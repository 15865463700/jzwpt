...
class Mdl_Rzpscompany_Comment extends Mdl_Table
{   
  
    protected $_table = 'rzps_company_comment';
    ...
    public function comment_count($val)
    {
        ...
        if($rs = $this->db->Execute($sql)){
            ...
            foreach($counts as $k=>$v){
                K::M('rzpscompany/company')->update($k, array('comments'=>$v), true);
                $count ++;
            }            
        }
        return $count;
	...
	{
		if($company = K::M('rzpscompany/company')->detail($data['company_id'])){
			...
			K::M('rzpscompany/company')->update($data['company_id'], $update, true);
		}
	}
