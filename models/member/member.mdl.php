...
{    
    ...
    protected $_pk = 'uid';
    protected $_cols = 'uid,uname,passwd,from,mail,gender,city_id,group_id,mobile,credits,realname,face,face_80,face_32,Y,M,D,verify,uc_uid,cart,regip,closed,lastlogin,jifen,yajin,lasactive,dateline,truste_money,company,yezhu';
    ...
    protected $_from_list = array('member'=>'业主','company'=>'公司',  'designer'=>'设计师', 'shop'=>'商家' ,'mechanic'=>'技工', 'gz'=>'项目经理', 'rzps'=>'布艺窗帘', 'gybj'=>'工艺摆件', 'afzn'=>'安防智能', 'jtyjy'=>'家庭影剧院');
    ...
    public function get($v=""){
        ...
        $sql = "SELECT f.*,m.* FROM ".$this->table($this->_table)." m LEFT JOIN ".$this->table('member_fields')." f ON m.uid=f.uid WHERE m.token='{$v}'";
        if($row = $this->db->GetRow($sql)){
            $row = $this->_format_row($row);
        }
        return $row;
    ...
    protected function _format_row($row)
    {
        ...
        }else if(in_array($row['from'],array('afzn'))){
            $row['ucenter_url'] = '/afzn/member-info.html';
        }else if(in_array($row['from'],array('rzps'))){
            $row['ucenter_url'] = '/rzps/member-info.html';
        }else if(in_array($row['from'],array('jtyjy'))){
            $row['ucenter_url'] = '/jtyjy/member-info.html';
        }else if(in_array($row['from'],array('gybj'))){
            $row['ucenter_url'] = '/gybj/member-info.html';
        }else{
 ...
 //修改用户积分
    public function update1($pk, $data, $checked=false)
        {
            $data=array(
                'jifen'=>$data
                );
    ...
    protected function _check($data, $uid=null)
    {
        ...
        unset($data['uid'], $data['yajin'], $data['regip'], $data['dateline']);
        if($uid = (int)$uid){
    ...
    //验证手机号码唯一
    public function only($mobile)
    {
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE mobile=$mobile";
    ...
    public function getByMobile($mobile){
        $sql = "SELECT f.*,m.* FROM ".$this->table($this->_table)." m LEFT JOIN ".$this->table('member_fields')." f ON m.uid=f.uid WHERE m.mobile='$mobile'";
        if($row = $this->db->GetRow($sql)){
            $row = $this->_format_row($row);
        }
        return $row;
