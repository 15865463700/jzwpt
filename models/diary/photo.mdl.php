...
class Mdl_Diary_Photo extends Mdl_Table
{   
  
    protected $_table = 'diary_photo';
    protected $_pk = 'photo_id';
    protected $_cols = 'photo_id,city_id,diary_id,title,photo,size,views,orderby,closed,clientip,dateline';
    ...
    public function upload($diary_id, $attach)
    {
        ...
        $oImg = K::M('image/gd');
        $thumbs = $size = array();
        $size['photo'] = $cfg['diaryphoto']['photo'] ? $cfg['diaryphoto']['photo'] : '720';
        $size['thumb'] = $cfg['diaryphoto']['thumb'] ? $cfg['diaryphoto']['thumb'] : '200';
        $size['small'] = $cfg['diaryphoto']['small'] ? $cfg['diaryphoto']['small'] : '60X60';
        $thumbs = array($size['photo']=>"{$D}/{$fname}",$size['thumb']=>"{$D}/{$fname}_thumb.jpg", $size['small']=>"{$D}/{$fname}_small.jpg");
        $oImg->thumbs($F, $thumbs);
        if($cfg['diaryphoto']['watermark']){
            $uname = $attach['uname'] ? $attach['uname'] : 'IJH';
            $oImg->watermark("{$D}/{$fname}", $uname);
        }
        $data = array();
        $data['diary_id'] = (int)$diary_id;
        ...
        if($photo_id =$this->db->insert($this->_table, $data, true)){
            ...
            K::M('diary/diary')->update_last($diary_id, $attach['size'], 1);
            return $data;
        }
        return false;
    }

    public function items_by_diary($diary_id, $p=1, $l=50, &$count=0)
    {
        if(!$diary_id = (int)$diary_id){
            return false;
        }
        return $this->items(array('diary_id'=>$diary_id,'closed'=>0), $this->_orderby, $p, $l, $count);
    }

    public function count_by_diary($diary_id)
    {
        if(!$diary_id = (int)$album_id){
            return false;
        }
        $sql = "SELECT diary_id, COUNT(1) photos, SUM(`size`) size FROM ".$this->table($this->_table)." WHERE diary_id='$diary_id' AND closed=0";
    ...
    public function delete($pids, $force=false)
    {
        ...
        foreach($items as $item){
            $albums[$item['diary_id']]['num'] += 1;
            $albums[$item['diary_id']]['size'] += $item['size'];
        ...
        if(!empty($photo_ids)){
            ...
            if($ret){
                foreach($albums as $k=>$v){
                    K::M('diary/diary')->update_last($k, -$v['size'], -$v['num']);
                }
            }
        }
        return $ret;      
	...
	function phone_count($val){
	   $count = 0;
	   $sql = "SELECT diary_id, COUNT(1) c FROM ".$this->table($this->_table)." WHERE ". self::field('diary_id', $val)." and closed = 0 GROUP BY diary_id";
        if($rs = $this->db->Execute($sql)){
            while($row = $rs->fetch()){
                K::M('diary/diary')->update($row['diary_id'], array('phones'=>$row['c']), true);
                $count ++;
            }
        }
        return $count;
    ...
    public function _check($data, $photo_id=null)
    {
        ...
        if(isset($data['diary_id'])){
            $data['diary_id'] = (int)$data['diary_id'];
        }
        if(isset($data['size'])){
