...
class Mdl_Gybjcompany_Company extends Mdl_Table
{   
  
    protected $_table = 'gybj_company';
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        if($attrs = $filter['attrs']){
            ...
            $attr_sql = "SELECT company_id FROM ".$this->table('gybj_company_attr')." WHERE attr_value_id IN($attr_ids) GROUP BY company_id HAVING SUM(attr_value_id)=$attr_count";
        }
    ...
    public function company_by_uid($uid, $closed=false)
    {
        ...
        $sql = "SELECT f.*,c.* FROM ".$this->table($this->_table)." c LEFT JOIN ".$this->table('gybj_company_fields')." f ON c.company_id=f.company_id WHERE $where";
        if($row = $this->db->GetRow($sql)){
            $row = $this->_format_row($row);
        }
        return $row;
    ...
    protected function _company_url($row)
    {
        ...
        $link = K::M('helper/link')->mklink('gybjcompany',array($row['company_id']), array(), 'city:'.$row['city_id'], $site['rewrite']);
        return $link;
    }    
