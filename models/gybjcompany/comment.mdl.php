...
class Mdl_Gybjcompany_Comment extends Mdl_Table
{   
  
    protected $_table = 'gybj_company_comment';
    ...
    public function comment_count($val)
    {
        ...
        if($rs = $this->db->Execute($sql)){
            ...
            foreach($counts as $k=>$v){
                K::M('gybjcompany/company')->update($k, array('comments'=>$v), true);
                $count ++;
            }            
        }
        return $count;
	...
	{
		if($company = K::M('gybjcompany/company')->detail($data['company_id'])){
			...
			K::M('gybjcompany/company')->update($data['company_id'], $update, true);
		}
	}
