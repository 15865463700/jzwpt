...
{   
    ...
    protected $_cols = 'article_id,city_id,cat_id,from,page,title,thumb,desc,linkurl,views,favorites,allow_comment,comments,photos,hidden,orderby,audit,closed,dateline,tag';
    ...
    public function items_by_cate_page($cat_id,$page,$city_id=0,$order, &$count=0,$limit=""){
    ...
    public function items_by_cate_tag_page($cat_id,$page,$city_id=0,$order){
        if(empty($page)){
            return false;
        }else if(!preg_match('/^[\w]+$/', $page)){
            return false;
...
//        }
    ...
    public function items_all($filter,$limit){
        $limit = $this->limit(1,10);
        //var_dump($filter);die;
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE cat_id in(8,106,107,133) $limit";
        $result=array();
        if($rs = $this->db->Execute($sql)) {
            while ($row = $rs->fetch()) {
                array_push($result, $row);
    ...
    public function tuijian($cat_id,$limit){
        $limit = $this->limit(1,3);
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE cat_id=$cat_id order by views desc $limit";
        $result=array();
        if($rs = $this->db->Execute($sql)) {
            while ($row = $rs->fetch()) {
                array_push($result, $row);
    ...
    public function rows($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        $where="";
        $attr_sql="";
