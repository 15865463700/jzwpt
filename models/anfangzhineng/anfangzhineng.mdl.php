...
class Mdl_Anfangzhineng_Anfangzhineng extends Mdl_Table
{
  
    protected $_table = 'afzn';
    protected $_pk = 'afzn_id';
    protected $_cols = 'afzn_id,afzn_title,add_time,afzn_thumb,afzn_user_id,afzn_user_name,afzn_user_img,afzn_video,info,detail,lianxiren,phone,mobile,qq,address';
    protected $_orderby = array('afzn_id'=>'DESC');
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        if($attrs = $filter['attrs']){
            ...
            $attr_sql = "SELECT afzn_id FROM ".$this->table('afzn_attr')." WHERE attr_value_id IN($attr_ids) GROUP BY afzn_id HAVING SUM(attr_value_id)=$attr_count";
        }

        unset($filter['attrs']);
        $where = $this->where($filter);
        if($attr_sql){
            $where .= " AND afzn_id IN($attr_sql)";
        }
