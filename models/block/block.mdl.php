...
{   
    ...
    protected static $_allow_from = array('mechanic'=>'技工','gz'=>'工长', 'designer'=>'设计师','company'=>'公司', 'shop'=>'商家','youhui'=>'优惠信息','news'=>'公司新闻','product'=>'商品','coupon'=>'优惠券','home'=>'小区','tuan'=>'小区团装','site'=>'工地','case'=>'案例','diary'=>'日记','ask'=>'问答','article'=>'文章','activity'=>'活动',"rzpscompany"=>"布艺窗帘","gybjcompany"=>"工艺摆件","afzncompany"=>"安防智能","jtyjycompany"=>"家庭影剧院");
    ...
    public function load_mdl($from)
    {
        static $_allow_mdl = array(
            ...
            ,'rzpscompany'=>'rzpscompany/company'
            ,'gybjcompany'=>'gybjcompany/company'
            ,'afzncompany'=>'afzncompany/company'
            ,'jtyjycompany'=>'jtyjycompany/company'
        );
    ...
    public function block($params, $tmpl, $smarty)
    {
		...
		if($items = $this->block_city_items($block, $city_id, $limit)){
            foreach($items as $k=>$v){
                if($params['type']){
                    $items[$k]['link']=str_replace("/article-detail-","/{$params['type']}/",$v['link']);
                    $items[$k]['link']=str_replace("-","/",$items[$k]['link']);
                }
            }
            $content = '';
            $index = 0;
