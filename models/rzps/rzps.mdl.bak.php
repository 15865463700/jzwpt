...
class Mdl_Rzps_Rzps extends Mdl_Table
{   
  
    protected $_table = 'rzps_company';
    protected $_pk = 'company_id';
    protected $_cols = 'company_id,rzps_title,add_time,rzps_thumb,rzps_user_id,rzps_city_id,rzps_user_name,rzps_user_img,rzps_video,info,detail,lianxiren,phone,mobile,closed,qq,address';
    ...
    public function sign($tenders_id, $uid)
    {
        ...
        if($member['from'] == 'company'){
            if($company = K::M('rzps/rzps')->company_by_uid($uid)){
    ...
    public function format_items_ext($items)
    {
        ...
        if($company_ids){
            $company_list = K::M('rzps/rzps')->items_by_ids($company_ids);
        }        
        foreach((array)$items as $k=>$v){
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        if($attrs = $filter['attrs']){
            ...
            $attr_sql = "SELECT rzps_id FROM ".$this->table('rzps_attr')." WHERE attr_value_id IN($attr_ids) GROUP BY rzps_id HAVING SUM(attr_value_id)=$attr_count";
        }
        unset($filter['attrs']);
        $where = $this->where($filter);
        if($attr_sql){
            $where .= " AND rzps_id IN($attr_sql)";
        }
