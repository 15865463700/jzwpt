...
class Mdl_Gybjcase_Case extends Mdl_Table
{   
  
    protected $_table = 'gybj_case';
	...
	{
	    if($case = K::M('gybjcase/case')->detail($val)){
	...
	public function company_count($val)
	{
        ...
        if($rs = $this->db->Execute($sql)){
            ...
            foreach($counts as $k=>$v){
                K::M('gybjcompany/company')->update($k, array('case_num'=>$v), true);
                $count ++;
            }            
        }
		K::M('gybjcompany/company')->update($val, array('last_case' => __TIME));
        return $count;
	}
                ...
                if($ret = parent::delete($case_ids, $force)){
                    ...
                    if($company_ids){
                        foreach($company_ids as $k=>$v) {
                            K::M('gybjcompany/company')->update_count($k, 'case_num', -(int)$v);
                        }
                    }
                    if($home_ids){
        ...
        if(isset($data['home_id']) || isset($data['company_id']) || isset($data['designer_id'])){
            ...
            if(isset($data['company_id'])){
                if($case['company_id'] != $data['company_id']){
                    if($case['company_id']){
                        K::M('gybjcompany/company')->update_count($case['company_id'], 'case_num', -1);
                    }
                    if($data['company_id']){
                        K::M('gybjcompany/company')->update_count($data['company_id'], 'case_num', 1);
                    }
                }
            }
    ...
    public function update_last($case_id, $size=0, $num=1)
    {
        ...
        $photo = '';
        if($items = K::M('gybjcase/photo')->items($filter, array('photo_id'=>'DESC'), 1, 10)){
            foreach($items as $v){
    ...
    public function reset_count($case_id)
    {
        if(!$case_id = (int)$case_id){
            return false;
        }else if(!$data = K::M('gybjcase/photo')->count_by_case($case_id)){
    ...
    public function format_items_ext($items)
    {
        ...
        if($photo_ids){
            $photo_list = K::M('gybjcase/photo')->items_by_ids($photo_ids);
        }
        if($home_ids){
        ...
        if($company_ids){
            $company_list = K::M('gybjcompany/company')->items_by_ids($company_ids);
        }
        if($case_ids){
            $attr_list = K::M('gybjcase/attr')->items(array('case_id'=>$case_ids), null, 1, 500);
        }
        foreach((array)$items as $k=>$v){
