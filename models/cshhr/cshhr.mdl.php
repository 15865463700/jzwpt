...
class Mdl_Cshhr_Cshhr extends Mdl_Table
{   
  
    protected $_table = 'cshhr';
    protected $_pk = 'cshhr_id';
    protected $_cols = 'cshhr_id,cshhr_title,cshhr_thumb,lianxiren,phone,mobile,qq,address,add_time,info,detail,province_id,city_id,area_id,code';
    protected $_orderby = array('cshhr_id'=>'DESC');
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        if($attrs = $filter['attrs']){
            ...
            $attr_sql = "SELECT cshhr_id FROM ".$this->table('cshhr_attr')." WHERE attr_value_id IN($attr_ids) GROUP BY cshhr_id HAVING SUM(attr_value_id)=$attr_count";
        }

        unset($filter['attrs']);
        $where = $this->where($filter);
        if($attr_sql){
            $where .= " AND cshhr_id IN($attr_sql)";
        }
