...
class Mdl_Jtyjycompany_Comment extends Mdl_Table
{   
  
    protected $_table = 'jtyjy_company_comment';
    ...
    public function comment_count($val)
    {
        ...
        if($rs = $this->db->Execute($sql)){
            ...
            foreach($counts as $k=>$v){
                K::M('jtyjycompany/company')->update($k, array('comments'=>$v), true);
                $count ++;
            }            
        }
        return $count;
	...
	{
		if($company = K::M('jtyjycompany/company')->detail($data['company_id'])){
			...
			K::M('jtyjycompany/company')->update($data['company_id'], $update, true);
		}
	}
