...
{   
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        if($attrs = $filter['attrs']){
            ...
            $attr_sql = "SELECT company_id FROM ".$this->table('jtyjy_company_attr')." WHERE attr_value_id IN($attr_ids) GROUP BY company_id HAVING SUM(attr_value_id)=$attr_count";
        }
    ...
    public function company_by_uid($uid, $closed=false)
    {
        ...
        $sql = "SELECT f.*,c.* FROM ".$this->table($this->_table)." c LEFT JOIN ".$this->table('jtyjy_company_fields')." f ON c.company_id=f.company_id WHERE $where";
        if($row = $this->db->GetRow($sql)){
            $row = $this->_format_row($row);
        }
        return $row;
        ...
        if($uid = (int)$data['uid']){
            if($member = K::M('member/member')->detail($uid)){
                if($member['from'] != 'jtyjy'){
                    $this->err->add("所属会员必须为安防智能公司类型", 451);
                    return false;
    ...
    protected function _format_row($row)
    {
        ...
        if($row['comments']){
            ...
            $row['avg_score'] = $row['avg_scores']['score'] = round($row['score'], 1);
            $row['avg_scores']['score1'] = round($row['score1'], 1);
            $row['avg_scores']['score2'] = round($row['score2'], 1);
            $row['avg_scores']['score3'] = round($row['score3'], 1);
            $row['avg_scores']['score4'] = round($row['score4'], 1);
            $row['avg_scores']['score5'] = round($row['score5'], 1);
		}
    ...
    protected function _company_url($row)
    {
        ...
        $link = K::M('helper/link')->mklink('jtyjycompany',array($row['company_id']), array(), 'city:'.$row['city_id'], $site['rewrite']);
        return $link;
    }    
