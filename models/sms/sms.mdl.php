...
{   
    ...
    protected function _send($mobile, $content)
    {
        if(!$this->sms->send($mobile, $content)){
            ...
            if($cfg['show_error'] == '1'){
                if(__CFG::DEBUG){
                    $this->err->add('短信接口错误:['.$this->sms->lastcode.':'.$this->sms->lastmsg.']', 450);
                }else{
                    $this->err->add('发送短信失败['.$this->sms->lastcode.']', 450);
                }
            }
            K::M('sms/log')->create(array('mobile'=>$mobile, 'content'=>$content, 'sms'=>$this->sms->tag, 'status'=>0));
            return false;
        }
        K::M('sms/log')->create(array('mobile'=>$mobile, 'content'=>$content, 'sms'=>$this->sms->tag, 'status'=>1));
        return true;        
    }
