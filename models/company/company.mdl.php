...
{   
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        ...
        if(isset($filter['company_ids'])){
            $attr_sql=empty($attr_sql)?$filter['company_ids']:"{$attr_sql},{$filter['company_ids']}";
        }
        unset($filter['attrs']);
        unset($filter['company_ids']);
        ...
        $sql="select sign_company_id,count(tenders_id) tenders_num from ".$this->table("tenders")." where sign_uid>0 and sign_company_id in ({$company_ids}) group by sign_company_id";
        if($rs = $this->db->query($sql)){
            while($row = $rs->fetch()){
                if($row["sign_company_id"]){
                    $items[$row["sign_company_id"]]["tenders_num"]=$row["tenders_num"];
                }
            }
        }
        return $items;
    ...
    public function detail($company_id, $closed=false)
    {
        ...
        $row['tenders_num']=0;
        $sql="select sign_company_id,count(tenders_id) tenders_num from ".$this->table("tenders")." where sign_uid>0 and sign_company_id ={$company_id}";
        if($rs = $this->db->GetRow($sql)){
            $row['tenders_num']=$rs["tenders_num"];
        }
        return $row;
    }
