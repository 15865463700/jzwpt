        ...
        if($items = $detail['items']){
            ...
            if($adv['from'] == 'script'){
				...
				foreach($item_list as $k => $item){
					$content .= str_replace(array('<{$item.a_attr}>','<{$pager.img}>','<{$item.clickurl}>','<{$item.link}>','<{$item.title}>','<{$item.thumb}>','<{$item.item_attr}>','<{$item.desc}>'), array('target='."'".$item['target']."'",$cfg['url'],$item['link'],$item['link'],$item['title'], $item['thumb'], $item['item_attr'], $item['desc']), $match[1]);
				}
