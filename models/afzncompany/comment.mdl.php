...
class Mdl_Afzncompany_Comment extends Mdl_Table
{   
  
    protected $_table = 'afzn_company_comment';
    ...
    public function comment_count($val)
    {
        ...
        if($rs = $this->db->Execute($sql)){
            ...
            foreach($counts as $k=>$v){
                K::M('afzncompany/company')->update($k, array('comments'=>$v), true);
                $count ++;
            }            
        }
        return $count;
	...
	{
		if($company = K::M('afzncompany/company')->detail($data['company_id'])){
			...
			K::M('afzncompany/company')->update($data['company_id'], $update, true);
		}
	}
