...
class Mdl_Xw_Photo extends Mdl_Table{
    protected $_table = 'xw_photo';
    protected $_pk = 'photo_id';
    protected $_cols = 'photo_id,city_id,xw_id,title,photo,size,views,orderby,closed,clientip,dateline';
    ...
    public function upload($xw_id, $attach)
    {
        ...
        if($photo_id =$this->db->insert($this->_table, $data, true)){
            ...
            K::M('xw/xw')->update_last($xw_id, $attach['size'], 1);
            return $data;
        }
        return false; 
    }

    public function items_by_case($xw_id, $p=1, $l=50, &$count=0)
    {
        if(!$xw_id = (int)$xw_id){
            return false;
        }
        return $this->items(array('xw_id'=>$xw_id,'closed'=>0), $this->_orderby, $p, $l, $count);
    }

    public function count_by_case($xw_id)
    {
        if(!$xw_id = (int)$album_id){
            return false;
        }
        $sql = "SELECT xw_id, COUNT(1) photos, SUM(`size`) size FROM ".$this->table($this->_table)." WHERE xw_id='$xw_id' AND closed=0";
    ...
    public function delete($pids, $force=false)
    {
        ...
        foreach($items as $item){
            $albums[$item['xw_id']]['num'] += 1;
            $albums[$item['xw_id']]['size'] += $item['size'];
        ...
        if(!empty($photo_ids)){
            ...
            if($ret){
                foreach($albums as $k=>$v){
                    K::M('xw/xw')->update_last($k, -$v['size'], -$v['num']);
                }
            }
        }
        return $ret;      
	...
	function phone_count($val){
	   $count = 0;
	   $sql = "SELECT xw_id, COUNT(1) c FROM ".$this->table($this->_table)." WHERE ". self::field('xw_id', $val)." and closed = 0 GROUP BY xw_id";
        if($rs = $this->db->Execute($sql)){
            while($row = $rs->fetch()){
                K::M('xw/xw')->update($row['xw_id'], array('phones'=>$row['c']), true);
                $count ++;
            }
        }
        return $count;
