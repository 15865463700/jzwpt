...
class Mdl_Vrqj_Vrqj extends Mdl_Table
{   
  
    protected $_table = 'vrqj';
    protected $_pk = 'vrqj_id';
    protected $_cols = 'vrqj_id,vrqj_title,city_id,add_time,vrqj_thumb,vrqj_user_id,vrqj_user_name,vrqj_user_img,vrqj_video';
    protected $_orderby = array('vrqj_id'=>'DESC');

//    protected $_from_list = array('TZX'=>'招标','ZXB'=>'装修保','TBJ'=>'报价','TLF'=>'量房','TSJ'=>'设计','TJC'=>'建材','TBJ'=>'报价');
    ...
    public function items($filter=array(), $orderby=null, $p=1, $l=20, &$count=0)
    {
        if($attrs = $filter['attrs']){
            ...
            $attr_sql = "SELECT vrqj_id FROM ".$this->table('vrqj_attr')." WHERE attr_value_id IN($attr_ids) GROUP BY vrqj_id HAVING SUM(attr_value_id)=$attr_count";
        }

        unset($filter['attrs']);
        $where = $this->where($filter);
        if($attr_sql){
            $where .= " AND vrqj_id IN($attr_sql)";
        }
