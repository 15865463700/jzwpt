...
class Ctl_Rzpscase_Photo extends Ctl
{
    ...
    public function upload()
    {
        if(!$case_id = (int)$this->GP('case_id')){
            ...
            if($data = K::M('rzpscase/photo')->upload($case_id, $attach)){
                $cfg = $this->system->config->get('attach');
                $this->err->set_data('photo', $cfg['attachurl'].'/'.$data['photo']);
				K::M('rzpscase/photo')->phone_count($case['case_id']);
                $this->err->add('上传图片成功');
            }
        }
        $this->err->json();
    ...
    public function delete($photo_id=null)
    {
		if($photo_id = (int)$photo_id){
            if($photo = K::M('rzpscase/photo')->detail($photo_id)){
				if(!$case = K::M('rzpscase/case')->detail($photo['case_id'])){
					 $this->err->add('该案例不存在或已经删除', 403);
				}else if(!$this->check_city($case['city_id'])){
					 $this->err->add('不可越权操作', 403);
				}else if(K::M('rzpscase/photo')->delete($photo_id)){
					K::M('rzpscase/photo')->phone_count($case['case_id']);
                    $this->err->add('删除成功');
                }
            }
        }else if($ids = $this->GP('photo_id')){
           if($items = K::M('rzpscase/photo')->items_by_ids($ids)){
                $aids  =  array();
                ...
                if(!$case = K::M('rzpscase/case')->detail($case_id)){
                    ...
                    if($aids && K::M('rzpscase/photo')->delete($aids)){
						K::M('rzpscase/photo')->phone_count($case_ids);
                        $this->err->add('批量删除成功');
                    }
                }                
            }
    ...
    public function update($case_id)
    {
        if($this->checksubmit('data')){
            if($data = $this->GP('data')){
				$detail = K::M('rzpscase/case')->detail($case_id);
