...
class Ctl_Rzpscase_Comment extends Ctl {
    ...
    public function index($page = 1)
    {
        ...
        if ($items = K::M('rzpscase/comment')->items($filter, null, $page, $limit, $count)) {
            $uids = $caseids = array();
        ...
        $caseList=K::M('rzpscase/case')->items_by_ids($caseids);
        $this->pagedata['caseList'] = $caseList;
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:rzpscase/comment/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:rzpscase/comment/so.html';
    }
    ...
    public function create()
    {
        if ($this->checksubmit()) {
            if (!$data = $this->GP('data')) {
                ...
                if ($comment_id = K::M('rzpscase/comment')->create($data)) {
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', '?rzpscase/comment-index.html');
                }
            }
        }
        else {
            $this->tmpl = 'admin:rzpscase/comment/create.html';
        }
    ...
    public function edit($comment_id = null)
    {
        if (!($comment_id = (int) $comment_id) && !($comment_id = $this->GP('comment_id'))) {
        ...
        }else if (!$detail = K::M('rzpscase/comment')->detail($comment_id)) {
            ...
            if (!$data = $this->GP('data')) {
                ...
                if (K::M('rzpscase/comment')->update($comment_id, $data)) {
                    $this->err->add('修改内容成功');
                }
            }
        }
            ...
            $this->tmpl = 'admin:rzpscase/comment/edit.html';
        }
    }
    ...
    public function doaudit($comment_id = null)
    {
		if($comment_id = (int)$comment_id){
            if($comment = K::M('rzpscase/comment')->detail($comment_id)){
                if(!$this->check_city($comment['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('rzpscase/comment')->batch($comment_id, array('audit' => 1))){
                    $this->err->add('审核内容成功');
                }
            }
        }else if($ids = $this->GP('comment_id')){
            if($items = K::M('rzpscase/comment')->items_by_ids($ids)){
                $aids  = array();
                foreach($items as $v){
                ...
                if($aids && K::M('rzpscase/comment')->batch($ids, array('audit' => 1))){
    ...
    public function delete($comment_id = null)
    {
		...
		if($comment_id = (int)$comment_id){
            if($comment = K::M('rzpscase/comment')->detail($comment_id)){
                if(!$this->check_city($comment['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('rzpscase/comment')->delete($comment_id)){
                    $this->err->add('删除评论成功');
                }
            }
        }else if($ids = $this->GP('comment_id')){
            if($items = K::M('rzpscase/comment')->items_by_ids($ids)){
                ...
                if($aids && K::M('rzpscase/comment')->delete($aids)){
