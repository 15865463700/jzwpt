...
class Ctl_Diary_Diary extends Ctl {
    ...
    //日记图片
     public function photo() {
        $this->tmpl = 'admin:diary/diary/photo.html';
    }
    //日记图片添加
    public function upload(){
        if(!$diary_id = (int)$this->GP('diary_id')){
            ...
            if($data = K::M('diary/diary')->upload($diary_id, $attach)){
    ...
    public function detail($diary_id=null){
        if(!$diary_id = (int)$diary_id){
            $this->err->add('非指定日记ID', 211);
        }else if(!$diary = K::M('diary/diary')->detail($diary_id)){
            $this->err->add('日记不存在或已经删除', 212);
        }else if(!$this->check_city($diary['city_id'])){
            $this->err->add('不可越权操作', 403);
        }else{
            $pager = array('diary_id'=>$diary_id);
            $pager['page'] = (int)$page;
            $pager['limit'] = $limit = 50;
            $pager['count'] = $count = 0;
            $this->pagedata['detail'] = $detail;
            if($items = K::M('diary/photo')->items_by_diary($diary_id, $page, $limit, $count)){
                $this->pagedata['items'] = $items;
                $pager['count'] = $count;
                $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("diary/diary:detail", array('{page}')));
            }
            $this->pagedata['pager'] = $pager;
            $this->pagedata['diary'] = $diary;
            $this->tmpl = 'admin:diary/diary/detail.html';
        }
    }
