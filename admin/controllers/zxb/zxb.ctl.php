...
class Ctl_Zxb_Zxb extends Ctl
{
    ...
    public function index($page=1,$type=0)
    {
        ...
        if($SO = $this->GP('SO')){
            ...
            if ($SO['dealline']) {
			    if($SO['dealline']==10000000){
                    $filter['back'] = 10000000;
                    $filter['status'] = 6;
                }else{
                    $filter['dealline'] = "<=:".(time()+$SO['dealline']*24*60*60);
                    $filter['back'] = "!=:10000000";
                    $filter['status'] = 6;
                }
            }
	...
	public function change_zxb($tenders_id){
		if(!$tenders_id = (int)$tenders_id){
            ...
            $this->err->add('此标还没有指定装修业主!', 212);
        }else if(!$detail['sign_company_id']){
            $this->err->add('此标还没有装修公司中标!', 212);
        }else{
