...
class Ctl_Xw_Photo extends Ctl
{
    
    public function upload()
    {
        if(!$xw_id = (int)$this->GP('xw_id')){
    ...
    public function delete($photo_id=null)
    {
		if($photo_id = (int)$photo_id){
            if($photo = K::M('xw/photo')->detail($photo_id)){
				if(!$case = K::M('xw/xw')->detail($photo['xw_id'])){
					 $this->err->add('该案例不存在或已经删除', 403);
				}else if(!$this->check_city($case['city_id'])){
					 $this->err->add('不可越权操作', 403);
				}else if(K::M('xw/photo')->delete($photo_id)){
					K::M('xw/photo')->phone_count($case['xw_id']);
                    $this->err->add('删除成功');
                }
            }
        }else if($ids = $this->GP('photo_id')){
           if($items = K::M('xw/photo')->items_by_ids($ids)){
                $aids  =  array();
                ...
                if(!$case = K::M('xw/xw')->detail($xw_id)){
                    ...
                    if($aids && K::M('xw/photo')->delete($aids)){
						K::M('xw/photo')->phone_count($xw_ids);
                        $this->err->add('批量删除成功');
                    }
                }                
            }
    ...
    public function update($xw_id)
    {
        if($this->checksubmit('data')){
            if($data = $this->GP('data')){
				$detail = K::M('xw/xw')->detail($xw_id);
                $obj = K::M('xw/photo');
                foreach($data as $k=>$v){
