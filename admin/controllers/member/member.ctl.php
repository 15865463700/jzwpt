...
class Ctl_Member_Member extends Ctl
{
    ...
    {
        $member_auth=K::M('member/auth')->manager($uid);
...
//             $siteurl=$cfg['siteurl'];
            ...
            if($member['from'] == 'shop' || $member['from'] == 'company'){
                 $link = $siteurl.'/index.php?scenter/'.$member['from'].'-index.html';
            }else if($member['from'] == 'gz' || $member['from'] == 'designer' || $member['from'] == 'mechanic'){
                 $link = $siteurl.'/index.php?dcenter/'.$member['from'].'-index.html';
            }else if($member['from'] == "afzn"){
                 $link = $siteurl.'/index.php?afzn/member-info.html';
            }else if($member['from'] == "rzps"){
                 $link = $siteurl.'/index.php?rzps/member-info.html';
            }else if($member['from'] == "jtyjy"){
                 $link = $siteurl.'/index.php?jtyjy/member-index.html';
            }else{
                 $link = $siteurl.'/index.php?ucenter/member-index.html';
            }
    ...
    public function gold($uid=null)
    {
	 ...
	 public function yajin($uid=null)
		{
			if(!($uid = (int)$uid) && !($uid = (int)$this->GP('uid'))){
				$this->err->add('未指定要修改的用户ID', 211);
			}else if(!$detail = K::M('member/member')->detail($uid)){
				...
				if(!$data = $this->GP('data')){
					$this->err->add('非法的数据提交', 213);
				}else if(!$yajin = (int)$data['yajin']){
					$this->err->add('非法的充值押金', 214);
				}else if(!$log = trim($data['log'])){
					$this->err->add('没有填写充值备注', 215);
				}else{
                    $kefu="";
                    ...
                    if(K::M('member/yajin')->update($uid, $yajin, $log,$kefu)){
                        $this->err->add('充值押金成功');
                    }
                }
			}else{
				$this->pagedata['detail'] = $detail;
				$this->tmpl = 'admin:member/member/yajin.html';
			}
		}
