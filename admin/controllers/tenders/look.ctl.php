...
class Ctl_Tenders_Look extends Ctl {
        ...
        if (!$look_id = (int) $look_id) {
            ...
            if ($zxb_id = (int) $tenders['zxb_id']) {
                if (!$company = K::M('company/company')->company_by_uid($look['uid'])) {
                    ...
                    $smsdata=array();
                    K::M('sms/sms')->send($mobile, "sms_tender_sign", $smsdata);
        ...
        if (empty($tenders_id)) {
            ...
            if ($this->checksubmit()) {
                if (!$data = $this->GP('data')) {
                    ...
                    switch ($data['from']) {
                        case 'company':
                            $tmdl = K::M('company/company');
                            break;
                        case 'designer':
                            $tmdl = K::M('designer/designer');
                            break;
                        case 'gz':
                            $tmdl = K::M('gz/gz');
                            break;
                        case 'mechanic':
                            $tmdl = K::M('mechanic/mechanic');
                            break;
                        case 'shop':
                            $tmdl = K::M('shop/shop');
                            break;
                    ...
                    if (!$tmember = $tmdl->detail($data['tid'])) {
                        ...
                        if ($tenders_look == true && $look_id = K::M('tenders/look')->create($datas)) {
                            ...
                            switch ($member['from']) {
                                case 'gz':
                                    K::M('gz/gz')->update_count($member['uid'], 'tenders_num');
                                    break;
                                case 'designer':
                                    K::M('designer/designer')->update_count($member['uid'], 'tenders_num');
                                    break;
                                case 'mechanic':
                                    K::M('mechanic/mechanic')->update_count($member['uid'], 'tenders_num');
                                    break;
                                case 'company':
                                    $company = K::M('company/company')->update_count($company['company_id'], 'tenders_num');
                                    break;
                                case 'shop':
    ...
    public function make($tenders_id) {
        if (empty($tenders_id)) {
            $this->err->add('未指定要修改的内容ID', 211);
        } else if (!$detail = K::M('tenders/tenders')->detail($tenders_id)) {
            $this->err->add('您要修改的内容不存在或已经删除', 212);
            ...
            if ($this->checksubmit()) {
                ...
                $this->pagedata['tenders_id'] = $tenders_id;
                $this->pagedata['tenders'] = $detail;
                $this->tmpl = 'admin:tenders/look/make.html';
            }
        }
    }
