...
class Ctl_Dp_Cate extends Ctl {

    public function index($page = 1)
    {

        if ($items = K::M('dp/cate')->fetch_all()) {
            ...
            $strtmpl = '<tr>
                    ...
                    <td>{$spacer}{$title}</td><td>{$py}</td><td>{$orderby}</td><td>{$auditLabel}</td>
        ...
        $this->tmpl = 'admin:dp/cate/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:dp/cate/so.html';
    }
    ...
    public function create()
    {
        if ($this->checksubmit()) {
            if (!$data = $this->GP('data')) {
                ...
                if ($cat_id = K::M('dp/cate')->create($data)) {
                    $this->err->add('添加内容成功');
            ...
            $this->tmpl = 'admin:dp/cate/create.html';
        }
    }
    ...
    public function edit($cat_id = null)
    {
        if (!($cat_id = (int) $cat_id) && !($cat_id = $this->GP('cat_id'))) {
        ...
        else if (!$detail = K::M('dp/cate')->detail($cat_id)) {
            ...
            if (!$data = $this->GP('data')) {
                ...
                if (K::M('dp/cate')->update($cat_id, $data)) {
                    $this->err->add('修改内容成功');
                }
            }
        }
            ...
            $this->tmpl = 'admin:dp/cate/edit.html';
        }
    }
    ...
    public function doaudit($cat_id = null)
    {
        if ($cat_id = (int) $cat_id) {
            if (K::M('dp/cate')->batch($cat_id, array('audit' => 1))) {
                $this->err->add('审核内容成功');
            }
        }
        else if ($ids = $this->GP('cat_id')) {
            if (K::M('dp/cate')->batch($ids, array('audit' => 1))) {
    ...
    {
        if ($cat_id = (int) $cat_id) {
            if (K::M('dp/cate')->delete($cat_id)) {
                $this->err->add('删除成功');
            }
        }
        else if ($ids = $this->GP('cat_id')) {
            if (K::M('dp/cate')->delete($ids)) {
                $this->err->add('批量删除成功');
            }
        }
        else {
    ...
    public function children($pid = null)
    {
        ...
        $cats = array();
        if ($childrens = K::M('dp/cate')->childrens($pid)) {
