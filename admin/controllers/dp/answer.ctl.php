 ...
 * $Id: answer.ctl.php 3053 2014-01-15 02:00:13Z youyi $
 */
if (!defined('__CORE_DIR')) {
    exit("Access Denied");
}

class Ctl_Dp_Answer extends Ctl
{
    ...
    public function index($ask_id = 0, $page = 1)
    {
        ...
        if($ask_id = (int)$ask_id){
			 ...
			 $this->pagedata['detail'] = K::M('dp/ask')->detail($ask_id);
        }
		
        if ($items = K::M('dp/answer')->items($filter, null, $page, $limit, $count)) {
            $askids = $uids =  array();
            foreach($items as $v){
            ...
            if(!empty($askids)){
                $this->pagedata['asks'] = K::M('dp/ask')->items_by_ids($askids);
            }
            if (!empty($uids)) {
        ...
        $this->tmpl = 'admin:dp/answer/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:dp/answer/so.html';
    }
    ...
    public function create($ask_id = 0)
    {
		if (!($ask_id = (int) $ask_id) && !($ask_id = $this->GP('ask_id'))) {
		...
		}else if (!$detail = K::M('dp/ask')->detail($ask_id)) {
			...
			if ($this->checksubmit()) {
				if (!$data = $this->GP('data')) {
					...
					$data['ask_id'] = $ask_id;
					if ($answer_id = K::M('dp/answer')->create($data)) {
						$this->err->add('添加内容成功');
				...
				$this->tmpl = 'admin:dp/answer/create.html';
			}
		}
    }
    ...
    public function edit($answer_id = null)
    {
        if (!($answer_id = (int) $answer_id) && !($answer_id = $this->GP('answer_id'))) {
            $this->err->add('未指定要修改的内容ID', 211);
        }else if (!$detail = K::M('dp/answer')->detail($answer_id)) {
            $this->err->add('您要修改的内容不存在或已经删除', 212);
        }else if (!$ask = K::M('dp/ask')->detail($detail['ask_id'])) {
            ...
            if (!$data = $this->GP('data')) {
				...
				unset($data['ask_id']);
                if (K::M('dp/answer')->update($answer_id, $data)) {
                    $this->err->add('修改内容成功');
            ...
            $this->tmpl = 'admin:dp/answer/edit.html';
        }
    }
	...
	public function good($ask_id,$answer_id){
		if (!($ask_id = (int) $ask_id) && !($ask_id = $this->GP('ask_id'))) {
            ...
            $this->err->add('未指定要修改的内容ID', 211);
        }else if (!$answer = K::M('dp/answer')->detail($answer_id)) {
            $this->err->add('您要修改的内容不存在或已经删除', 212);
        }else{
			if(K::M('dp/ask')->update($ask_id, array('answer_id' => $answer_id))){
    ...
    public function doaudit($answer_id = null)
    {
        if ($answer_id = (int) $answer_id) {
            if (K::M('dp/answer')->batch($answer_id, array('audit' => 1))) {
                $this->err->add('审核内容成功');
            }
        }
        else if ($ids = $this->GP('answer_id')) {
            if (K::M('dp/answer')->batch($ids, array('audit' => 1))) {
    ...
    {
        if ($answer_id = (int) $answer_id) {
            if (K::M('dp/answer')->delete($answer_id)) {
                $this->err->add('删除成功');
            }
        }
        else if ($ids = $this->GP('answer_id')) {
            if (K::M('dp/answer')->delete($ids)) {
                $this->err->add('批量删除成功');
            }
        }
        else {
