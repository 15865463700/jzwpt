 ...
 * $Id: supply.ctl.php 3053 2014-01-15 02:00:13Z youyi $
 */
if (!defined('__CORE_DIR')) {
    exit("Access Denied");
}

class Ctl_Dp_Supply extends Ctl {
    ...
    public function index($ask_id = 0, $page = 1)
    {
        ...
        if ($items = K::M('dp/supply')->items($filter, null, $page, $limit, $count)) {
            $askids = $uids =  array();
            foreach($items as $v){
        ...
        $this->tmpl = 'admin:dp/supply/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:dp/supply/so.html';
    }
    ...
    public function create($ask_id=0)
    {
        if ($this->checksubmit()) {
            if (!$data = $this->GP('data')) {
                ...
                if ($supply_id = K::M('dp/supply')->create($data)) {
                    $this->err->add('添加内容成功');
            ...
            $this->tmpl = 'admin:dp/supply/create.html';
        }
    }
    ...
    public function edit($supply_id = null)
    {
        if (!($supply_id = (int) $supply_id) && !($supply_id = $this->GP('supply_id'))) {
        ...
        else if (!$detail = K::M('dp/supply')->detail($supply_id)) {
            ...
            if (!$data = $this->GP('data')) {
                ...
                if (K::M('dp/supply')->update($supply_id, $data)) {
                    $this->err->add('修改内容成功');
                }
            }
        }
            ...
            $this->tmpl = 'admin:dp/supply/edit.html';
        }
    }
    ...
    public function doaudit($supply_id = null)
    {
        if ($supply_id = (int) $supply_id) {
            if (K::M('dp/supply')->batch($supply_id, array('audit' => 1))) {
                $this->err->add('审核内容成功');
            }
        }
        else if ($ids = $this->GP('supply_id')) {
            if (K::M('dp/supply')->batch($ids, array('audit' => 1))) {
    ...
    {
        if ($supply_id = (int) $supply_id) {
            if (K::M('dp/supply')->delete($supply_id)) {
                $this->err->add('删除成功');
            }
        }
        else if ($ids = $this->GP('supply_id')) {
            if (K::M('dp/supply')->delete($ids)) {
                $this->err->add('批量删除成功');
            }
        }
        else {
