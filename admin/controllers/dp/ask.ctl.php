 ...
 * $Id: ask.ctl.php 5649 2014-06-25 11:13:56Z youyi $
 */
if (!defined('__CORE_DIR')) {
    exit("Access Denied");
}

class Ctl_Dp_Ask extends Ctl {
    ...
    public function index($page = 1)
    {
        ...
        if ($items = K::M('dp/ask')->items($filter, array('ask_id'=>'desc'), $page, $limit, $count)) {
        ...
        $this->pagedata['cates'] = K::M('dp/cate')->fetch_all();
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:dp/ask/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:dp/ask/so.html';
    }
    ...
    public function create()
    {
        if ($this->checksubmit()) {
            ...
            $this->tmpl = 'admin:dp/ask/create.html';
        }
    }
    ...
    public function edit($ask_id = null)
    {
        if (!($ask_id = (int) $ask_id) && !($ask_id = $this->GP('ask_id'))) {
            ...
            if (!$data = $this->GP('data')) {
                ...
                if (K::M('dp/ask')->update($ask_id, $data)) {
                    $this->err->add('修改内容成功');
                }
            }
        }
        else {
            $detail['cat_pids'] = K::M('dp/cate')->parent_ids($detail['cat_id'], ',', true);
            if ($uid = (int) $detail['uid']) {
                $this->pagedata['member'] = K::M('member/member')->detail($uid);
            }
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'admin:dp/ask/edit.html';
        }
    }
    ...
    public function doaudit($ask_id = null)
    {
        if ($ask_id = (int) $ask_id) {
            if (K::M('dp/ask')->batch($ask_id, array('audit' => 1))) {
                $this->err->add('审核内容成功');
            }
        }
        else if ($ids = $this->GP('ask_id')) {
            if (K::M('dp/ask')->batch($ids, array('audit' => 1))) {
    ...
    {
        if ($ask_id = (int) $ask_id) {
            if (K::M('dp/ask')->delete($ask_id)) {
                $this->err->add('删除成功');
            }
        }
        else if ($ids = $this->GP('ask_id')) {
            if (K::M('dp/ask')->delete($ids)) {
                $this->err->add('批量删除成功');
            }
        }
        else {
