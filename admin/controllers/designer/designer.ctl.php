...
class Ctl_Designer_Designer extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('designer/designer')->items($filter, null, $page, $limit, $count)){
            ...
            $company_ids=$rzpscompany_ids=$afzncompany_ids=$jtyjycompany_ids= array();
            foreach($items as $k=>$v){
                switch($v["company_type"]){
                    case 5:
                        $rzpscompany_ids[$v['company_id']] = $v['company_id'];
                        break;
                    case 6:
                        $afzncompany_ids[$v['company_id']] = $v['company_id'];
                        break;
                    case 7:
                        $jtyjycompany_ids[$v['company_id']] = $v['company_id'];
                        break;
                    default:
...
//            $this->pagedata['company_list'] = K::M('company/company')->items_by_ids($company_ids);
            ...
            $rzpscompany_list = K::M('rzpscompany/company')->items_by_ids($rzpscompany_ids);
            $afzncompany_list = K::M('afzncompany/company')->items_by_ids($afzncompany_ids);
            $jtyjycompany_list = K::M('jtyjycompany/company')->items_by_ids($jtyjycompany_ids);
            foreach($items as $k=>$v){
                switch($v["company_type"]){
                    case 5:
                        $items[$k]["company_name"]=$rzpscompany_list[$v['company_id']]["name"]."(软装配饰)";
                        break;
                    case 6:
                        $items[$k]["company_name"]=$afzncompany_list[$v['company_id']]["name"]."(安防智能)";
                        break;
                    case 7:
                        $items[$k]["company_name"]=$jtyjycompany_list[$v['company_id']]["name"]."(家庭影剧院)";
                        break;
                    default:
                        if($v['company_id']>0){
                            $items[$k]["company_name"]=$company_list[$v['company_id']]["name"]."(装修公司)";
                        }else{
                            $items[$k]["company_name"]="未设置";
                        }
                        break;
                }
            }
