...
class Ctl_Article_Article extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'fenzhan:article/article/items.html';
    }
    ...
    public function so($target=null, $multi=null)
    {
        ...
        $this->pagedata['pager'] = $pager;   
        $this->tmpl = 'fenzhan:article/article/so.html';
    }
    ...
    public function dialog($multi=1, $page=1)
    {
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'fenzhan:article/article/dialog.html';       
    }
        ...
        if($this->checksubmit()){
            if(!$data = $this->GP('data')){
                ...
                if($article_id = K::M('article/article')->create($data)){
                    ...
                    if($root=K::M('article/cate')->cat_root($data['cat_id'])){
                        $cat_ids=K::M('article/cate')->children_ids($root['cat_id']);
                        K::M('article/tag')->check($data['tag'],$root['cat_id'],$cat_ids);
                    }
                    $this->err->add('添加文章成功');
            ...
            $pager['from'] = $this->article_from;
            $this->pagedata['pager'] = $pager;            
            $this->tmpl = 'fenzhan:article/article/create.html';
        }
    }
    ...
    public function edit($article_id=null)
    {
        if(!($article_id = (int)$article_id) && !($article_id = (int)$this->GP('article_id'))){
        	...
        	$this->tmpl = 'fenzhan:article/article/edit.html';
        }
    }
