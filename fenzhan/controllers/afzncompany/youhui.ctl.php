...
class Ctl_Afzncompany_Youhui extends Ctl {
    ...
    public function index($page = 1) {
        ...
        if ($items = K::M('afzncompany/youhui')->items($filter, null, $page, $limit, $count)) {
            foreach ($items as $k => $v) {
                ...
                if (!empty($v['afzncompany_id']))
                    $afzncompanyIds[$v['afzncompany_id']] = $v['afzncompany_id'];
        ...
        $this->tmpl = 'admin:afzncompany/youhui/items.html';
    }

    public function so() {
        $this->tmpl = 'admin:afzncompany/youhui/so.html';
    }
    
    public function audit($youhui_id = null)
    {
        if ($youhui_id = (int) $youhui_id) {
			if(!$detail = K::M('afzncompany/youhui')->detail($youhui_id)){
				$this->err->add('您要审核内容不存在或已经删除', 212);
			}else if(!$this->check_city($detail['city_id'])){
				 $this->err->add('不可越权操作', 403);
			}else{
				if (K::M('afzncompany/youhui')->update($youhui_id,array('audit'=>1))) {
					$this->err->add('审核成功');
				}
			}
        } else if ($ids = $this->GP('youhui_id')) {

			 if($items = K::M('afzncompany/youhui')->items_by_ids($ids)){
                $aids = array();
                foreach($items as $v){
                ...
                if($aids && K::M('afzncompany/youhui')->batch($aids,array('audit'=>1))){
                   $this->err->add('审核成功');
                }
            }
        } else {
    ...
    public function create() {
        if ($this->checksubmit()) {
            if (!$data = $this->GP('data')) {
                ...
                if ($youhui_id = K::M('afzncompany/youhui')->create($data)) {
					K::M('afzncompany/youhui')->youhui_count($afzncompany['afzncompany_id']);
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', '?afzncompany/youhui-index.html');
                }
            }
        } else {
            $this->tmpl = 'admin:afzncompany/youhui/create.html';
        }
    }
    ...
    public function edit($youhui_id = null) {
        if (!($youhui_id = (int) $youhui_id) && !($youhui_id = $this->GP('youhui_id'))) {
        ...
        } else if (!$detail = K::M('afzncompany/youhui')->detail($youhui_id)) {
            ...
            if (!$data = $this->GP('data')) {
				...
				if (K::M('afzncompany/youhui')->update($youhui_id, $data)) {
                    $this->err->add('修改内容成功');
					$this->err->set_data('forward', '?afzncompany/youhui-index.html');
                }
            }
            ...
            $this->tmpl = 'admin:afzncompany/youhui/edit.html';
        }
    }
    ...
    {
        if($youhui_id = (int)$youhui_id){
            if(K::M('afzncompany/youhui')->batch($youhui_id, array('audit'=>1))){
                $this->err->add('审核内容成功');
            }
        }else if($ids = $this->GP('youhui_id')){
            if(K::M('afzncompany/youhui')->batch($ids, array('audit'=>1))){
    ...
    public function delete($youhui_id = null)
    {
		...
		if($youhui_id = (int)$youhui_id){
            if($youhui = K::M('afzncompany/youhui')->detail($youhui_id)){
                if(!$this->check_city($youhui['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('afzncompany/youhui')->delete($youhui_id)){
                    K::M('afzncompany/youhui')->youhui_count($youhui['afzncompany_id']);
                    $this->err->add('删除成功');
                }
            }
        }else if($ids = $this->GP('youhui_id')){
            if($items = K::M('afzncompany/youhui')->items_by_ids($ids)){
                ...
                if($aids && K::M('afzncompany/youhui')->delete($aids)){
                    K::M('afzncompany/youhui')->youhui_count($afzncompany_ids);
                    $this->err->add('批量删除成功');
                }
            }
        }else{
