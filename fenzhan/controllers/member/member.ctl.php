...
class Ctl_Member_Member extends Ctl
{
    public function index($page=1){
        ...
        $this->tmpl = 'fenzhan:member/member/items.html';
    }
    ...
    public function so($target=null, $multi=null, $from='member')
    {
        ...
        $this->pagedata['pager'] = $pager;   
        $this->tmpl = 'fenzhan:member/member/so.html';
    }
    ...
    public function dialog($from='member', $page=1)
    {
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'fenzhan:member/member/dialog.html';   
    }
    ...
    public function ucard($uid=null)
    {
        ...
        }else if($member['city_id'] != CITY_ID){
            $this->err->add('没有权限操作', 212);
        }else{
            if($member['from'] == 'shop'){
    ...
    public function manager($uid)
    {
		...
		$rule  = "/\/\/(\w+)./";  
        preg_match($rule,$request['url'],$result); 
		foreach($city_list as $k => $v){
			if($v['pinyin'] == $result[1]){
				$pinyin = $v['pinyin'];
			}
		}
        if(K::M('member/auth')->manager($uid)) {
            $cfg = $this->system->config->get('site');
//             $link = 'http://'.$pinyin.'.'.$cfg['domain'].'/index.php?ucenter/member-index.html';
            ...
            }else if($member['from'] == "afzn"){
                $link = $cfg['siteurl'].'/index.php?afzn/member-info.html';
            }else if($member['from'] == "rzps"){
                $link = $cfg['siteurl'].'/index.php?rzps/member-info.html';
            }else{
    ...
    public function edit($uid=null)
    {
        if(!($uid = (int)$uid) && !($uid = (int)$this->GP('uid'))){
        ...
        }else if($detail['city_id'] != CITY_ID){
            $this->err->add('没有权限操作', 212);
        	...
        	$this->tmpl = 'fenzhan:member/member/edit.html';
        }
    }
	...
	public function gold($uid=null)
    {
        if(!($uid = (int)$uid) && !($uid = (int)$this->GP('uid'))){
            ...
            if(!$data = $this->GP('data')){
                ...
                $kefu="";
                $FZTOKEN=$this->cookie->get('FZTOKEN');
                $FZTOKEN_ARR=explode('-',$FZTOKEN);
                if($FZTOKEN_ARR[0]){
                    $kefu.=$FZTOKEN_ARR[0];
                }
                if($this->cookie->get('FZADMIN')){
                    $kefu.=empty($kefu)?$this->cookie->get('FZADMIN'):":".$this->cookie->get('FZADMIN');
                }
                if(K::M('member/gold')->update($uid, $gold, $log,$kefu)){
                    $this->err->add('充值金币成功');
                }
            }
        }else{
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'fenzhan:member/member/gold.html';
        }
    }
    ...
    public function delete($uid=null, $force=false)
    {
        if($uid = (int)$uid){
            if($member = K::M('member/member')->detail($uid)){
                if($member['city_id'] != CITY_ID){
                    $this->err->add('不可越权操作', 403);
                }else  if(K::M('member/handler')->delete($uid, $force)){
                    $this->err->add('删除用户成功');
                }
            }
        }else if($ids = $this->GP('uid')){
            if($items = K::M('member/member')->items_by_ids($ids)){
                $aids = $uids = array();
                foreach($items as $v){
				...
				if(K::M('member/handler')->delete($uids, $force)){
					$this->err->add('批量删除成功');
				}
            }
        }else{
    ...
    public function create()
    {
        if($this->checksubmit()){
            ...
            $this->pagedata['from_list'] = K::M('member/member')->from_list();
            $this->tmpl = 'fenzhan:member/member/create.html';
        }
    }
