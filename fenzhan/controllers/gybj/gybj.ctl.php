...
class Ctl_Gybj_Gybj extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('gybj/gybj')->items($filter, null, $page, $limit, $count)){
        	$pager['count'] = $count;
        	$pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array('{page}')), array('SO'=>$SO));
        }
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:gybj/gybj/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:gybj/gybj/so.html';
    }
    ...
    public function detail($tenders_id = null)
    {
        if(!$tenders_id = (int)$tenders_id){
            ...
            $this->tmpl = 'admin:gybj/gybj/detail.html';
        }
    }
    ...
    public function create()
    {
        if($this->checksubmit()){
            if(!$data = $this->GP('data')){
                ...
                $detail = K::M('designer/designer')->detail($data["gybj_user_id"]);
                $data["gybj_user_name"]=$detail["uname"];
                ...
                $data["add_time"]=time();
                $gybj_id=K::M('gybj/gybj')->create($data);
                if($attr=  $this->GP('attr')){
                    K::M('gybj/attr')->update($gybj_id,$attr);
                }
                if($gybj_id){
                    $this->err->add('添加内容成功！');
                    $this->err->set_data('forward', '?gybj/gybj-index.html');
                }else{
                    $this->err->add('添加内容失败！');
                }
            } 
           ...
           $this->tmpl = 'admin:gybj/gybj/create.html';
        }
    }

    public function edit($gybj_id=null)
    {
        if(!($gybj_id = (int)$gybj_id) && !($gybj_id = $this->GP('gybj_id'))){
            $this->err->add('未指定要修改的内容ID', 211);
        }
        else if(!$detail = K::M('gybj/gybj')->detail($gybj_id)){
            ...
            if(!$data = $this->GP('data')){
                ...
                if(K::M('gybj/gybj')->update($gybj_id, $data)){
                    if($attr = $this->GP('attr')){
                        K::M('gybj/attr')->update($gybj_id,$attr);
                    }
                    $this->err->add('修改内容成功');
        	...
        	$this->pagedata['detail'] = $detail;
        	$this->tmpl = 'admin:gybj/gybj/edit.html';
        }
    }
    ...
    public function delete($gybj_id=null)
    {
        if($gybj_id = (int)$gybj_id){
            if(K::M('gybj/gybj')->delete($gybj_id)){
                $this->err->add('删除成功');
            }
        }else if($ids = $this->GP('gybj_id')){
            if(K::M('gybj/gybj')->delete($ids)){
                $this->err->add('批量删除成功');
            }
        }else{
