...
class Ctl_Afzncase_Photo extends Ctl
{
    ...
    public function upload()
    {
        if(!$case_id = (int)$this->GP('case_id')){
            ...
            if($data = K::M('afzncase/photo')->upload($case_id, $attach)){
				...
				K::M('afzncase/photo')->phone_count($case['case_id']);
                $this->err->add('上传图片成功');
            }
        }
        $this->err->json();
    ...
    public function delete($photo_id=null)
    {
		if($photo_id = (int)$photo_id){
            if($photo = K::M('afzncase/photo')->detail($photo_id)){
				if(!$case = K::M('afzncase/case')->detail($photo['case_id'])){
					 $this->err->add('该案例不存在或已经删除', 403);
				}else if(!$this->check_city($case['city_id'])){
					 $this->err->add('不可越权操作', 403);
				}else if(K::M('afzncase/photo')->delete($photo_id)){
					K::M('afzncase/photo')->phone_count($case['case_id']);
                    $this->err->add('删除成功');
                }
            }
        }else if($ids = $this->GP('photo_id')){
           if($items = K::M('afzncase/photo')->items_by_ids($ids)){
                $aids  =  array();
                ...
                if(!$case = K::M('afzncase/case')->detail($case_id)){
                    ...
                    if($aids && K::M('afzncase/photo')->delete($aids)){
						K::M('afzncase/photo')->phone_count($case_ids);
                        $this->err->add('批量删除成功');
                    }
                }                
            }
    ...
    public function update($case_id)
    {
        if($this->checksubmit('data')){
            if($data = $this->GP('data')){
				$detail = K::M('afzncase/case')->detail($case_id);
