...
class Ctl_Afzncase_Comment extends Ctl {
    ...
    public function index($page = 1)
    {
        ...
        if ($items = K::M('afzncase/comment')->items($filter, null, $page, $limit, $count)) {
            $uids = $caseids = array();
        ...
        $caseList=K::M('afzncase/case')->items_by_ids($caseids);
        $this->pagedata['caseList'] = $caseList;
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:afzncase/comment/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:afzncase/comment/so.html';
    }
    ...
    public function create()
    {
        if ($this->checksubmit()) {
            if (!$data = $this->GP('data')) {
                ...
                if ($comment_id = K::M('afzncase/comment')->create($data)) {
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', '?afzncase/comment-index.html');
                }
            }
        }
        else {
            $this->tmpl = 'admin:afzncase/comment/create.html';
        }
    ...
    public function edit($comment_id = null)
    {
        if (!($comment_id = (int) $comment_id) && !($comment_id = $this->GP('comment_id'))) {
        ...
        }else if (!$detail = K::M('afzncase/comment')->detail($comment_id)) {
            ...
            if (!$data = $this->GP('data')) {
                ...
                if (K::M('afzncase/comment')->update($comment_id, $data)) {
                    $this->err->add('修改内容成功');
                }
            }
        }
            ...
            $this->tmpl = 'admin:afzncase/comment/edit.html';
        }
    }
    ...
    public function doaudit($comment_id = null)
    {
		if($comment_id = (int)$comment_id){
            if($comment = K::M('afzncase/comment')->detail($comment_id)){
                if(!$this->check_city($comment['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('afzncase/comment')->batch($comment_id, array('audit' => 1))){
                    $this->err->add('审核内容成功');
                }
            }
        }else if($ids = $this->GP('comment_id')){
            if($items = K::M('afzncase/comment')->items_by_ids($ids)){
                $aids  = array();
                foreach($items as $v){
                ...
                if($aids && K::M('afzncase/comment')->batch($ids, array('audit' => 1))){
    ...
    public function delete($comment_id = null)
    {
		...
		if($comment_id = (int)$comment_id){
            if($comment = K::M('afzncase/comment')->detail($comment_id)){
                if(!$this->check_city($comment['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('afzncase/comment')->delete($comment_id)){
                    $this->err->add('删除评论成功');
                }
            }
        }else if($ids = $this->GP('comment_id')){
            if($items = K::M('afzncase/comment')->items_by_ids($ids)){
                ...
                if($aids && K::M('afzncase/comment')->delete($aids)){
