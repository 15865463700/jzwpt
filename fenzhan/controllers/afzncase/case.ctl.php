...
class Ctl_Afzncase_Case extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $items = K::M('afzncase/case')->items($filter, array('case_id'=>'desc'), $page, $limit, $count);
        if($items){
        	$pager['count'] = $count;
        ...
        $this->tmpl = 'admin:afzncase/case/items.html';
    }
	...
	public function dialog($page=1)
    {
        ...
        $this->tmpl = 'admin:afzncase/case/dialog.html';
    }
    ...
    public function audit($page=1)
    {
        ...
        $items = K::M('afzncase/case')->items($filter, null, $page, $limit, $count);
        if($items){
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array('{page}')), array('SO'=>$SO));
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:afzncase/case/audit.html';
    }
    ...
    public function update($from='audit', $case_id=null)
    {
        ...
        if($data){
            if($case_id = (int)$case_id){
                if(K::M('afzncase/case')->update($case_id,  $data)){
                    $this->err->add($msg.'成功');
                }
            }else if($ids = $this->GP('case_id')){
                foreach((array)$ids as $id){
                    K::M('afzncase/case')->update((int)$id,  $data);
                }
    ...
    public function so()
    {
        $this->tmpl = 'admin:afzncase/case/so.html';
    }
    ...
    public function detail($case_id=null)
    {
        if(!$case_id = (int)$case_id){
        ...
        }else if(!$case = K::M('afzncase/case')->detail($case_id)){
            ...
            if($items = K::M('afzncase/photo')->items_by_case($case_id, $page, $limit, $count)){
                $this->pagedata['items'] = $items;
                $pager['count'] = $count;
            ...
            $this->tmpl = 'admin:afzncase/case/detail.html';
        }
    }
    ...
    {	
        if($this->checksubmit()){
            if(!$data = $this->GP('data')){
                ...
                if($case_id = K::M('afzncase/case')->create($data)){
                    if(!$attr = $this->GP('attr')){
                        $attr = array();
                    }
                    K::M('afzncase/attr')->update($case_id, $attr);
					K::M('afzncase/case')->case_count($case_id);
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('case/case:index'));
                }
            } 
        }else{
           $this->tmpl = 'admin:afzncase/case/create.html';
        }
    }
    ...
    public function edit($case_id=null)
    {
        if(!($case_id = (int)$case_id) && !($case_id = $this->GP('case_id'))){
        ...
        }else if(!$detail = K::M('afzncase/case')->detail($case_id)){
            ...
            if($attrs = K::M('afzncase/attr')->attrs_by_case($case_id)){
            ...
            $this->pagedata['detail'] = $detail;            
        	$this->tmpl = 'admin:afzncase/case/edit.html';
        }
    }
    ...
    {
        if($case_id = (int)$case_id){
            if(K::M('afzncase/case')->batch($case_id, array('audit'=>1))){
                $this->err->add('审核内容成功');
            }
        }else if($ids = $this->GP('case_id')){
            if(K::M('afzncase/case')->batch($ids, array('audit'=>1))){
		...
		if($case_id = (int)$case_id){
            if($case = K::M('afzncase/case')->detail($case_id)){
                if(!$this->check_city($case['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('afzncase/case')->delete($case_id)){
					K::M('afzncase/case')->down_count($case);
                    $this->err->add('删除案例成功');
                }
            }
        }else if($ids = $this->GP('case_id')){
            if($items = K::M('afzncase/case')->items_by_ids($ids)){
                ...
                if($aids && K::M('afzncase/case')->delete($aids)){
					K::M('afzncase/case')->downs_count($items);
                    $this->err->add('批量删除成功');
                }
            }
        }else{
