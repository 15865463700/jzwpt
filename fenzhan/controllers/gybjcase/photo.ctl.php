...
class Ctl_Gybjcase_Photo extends Ctl
{
    ...
    public function upload()
    {
        if(!$case_id = (int)$this->GP('case_id')){
            ...
            if($data = K::M('gybjcase/photo')->upload($case_id, $attach)){
				...
				K::M('gybjcase/photo')->phone_count($case['case_id']);
                $this->err->add('上传图片成功');
            }
        }
        $this->err->json();
    ...
    public function delete($photo_id=null)
    {
		if($photo_id = (int)$photo_id){
            if($photo = K::M('gybjcase/photo')->detail($photo_id)){
				if(!$case = K::M('gybjcase/case')->detail($photo['case_id'])){
					 $this->err->add('该案例不存在或已经删除', 403);
				}else if(!$this->check_city($case['city_id'])){
					 $this->err->add('不可越权操作', 403);
				}else if(K::M('gybjcase/photo')->delete($photo_id)){
					K::M('gybjcase/photo')->phone_count($case['case_id']);
                    $this->err->add('删除成功');
                }
            }
        }else if($ids = $this->GP('photo_id')){
           if($items = K::M('gybjcase/photo')->items_by_ids($ids)){
                $aids  =  array();
                ...
                if(!$case = K::M('gybjcase/case')->detail($case_id)){
                    ...
                    if($aids && K::M('gybjcase/photo')->delete($aids)){
						K::M('gybjcase/photo')->phone_count($case_ids);
                        $this->err->add('批量删除成功');
                    }
                }                
            }
    ...
    public function update($case_id)
    {
        if($this->checksubmit('data')){
            if($data = $this->GP('data')){
				$detail = K::M('gybjcase/case')->detail($case_id);
