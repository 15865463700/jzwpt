...
class Ctl_Gybjcase_Case extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $items = K::M('gybjcase/case')->items($filter, array('case_id'=>'desc'), $page, $limit, $count);
        if($items){
        	$pager['count'] = $count;
        ...
        $this->tmpl = 'admin:gybjcase/case/items.html';
    }
	...
	public function dialog($page=1)
    {
        ...
        $this->tmpl = 'admin:gybjcase/case/dialog.html';
    }
    ...
    public function audit($page=1)
    {
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:gybjcase/case/audit.html';
    }
        ...
        if($data){
            if($case_id = (int)$case_id){
                ...
                foreach((array)$ids as $id){
                    K::M('gybjcase/case')->update((int)$id,  $data);
                }
    ...
    public function so()
    {
        $this->tmpl = 'admin:gybjcase/case/so.html';
    }
    ...
    public function detail($case_id=null)
    {
        if(!$case_id = (int)$case_id){
            ...
            $this->tmpl = 'admin:gybjcase/case/detail.html';
        }
    }
        ...
        if($this->checksubmit()){
            if(!$data = $this->GP('data')){
                ...
                if($case_id = K::M('gybjcase/case')->create($data)){
					...
					K::M('gybjcase/case')->case_count($case_id);
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('case/case:index'));
                }
            } 
        }else{
           $this->tmpl = 'admin:gybjcase/case/create.html';
        }
    }
    ...
    public function edit($case_id=null)
    {
        if(!($case_id = (int)$case_id) && !($case_id = $this->GP('case_id'))){
        ...
        }else if(!$detail = K::M('gybjcase/case')->detail($case_id)){
            ...
            $this->pagedata['detail'] = $detail;            
        	$this->tmpl = 'admin:gybjcase/case/edit.html';
        }
    }
    ...
    {
        if($case_id = (int)$case_id){
            if(K::M('gybjcase/case')->batch($case_id, array('audit'=>1))){
                $this->err->add('审核内容成功');
            }
        }else if($ids = $this->GP('case_id')){
            if(K::M('gybjcase/case')->batch($ids, array('audit'=>1))){
		...
		if($case_id = (int)$case_id){
            if($case = K::M('gybjcase/case')->detail($case_id)){
                if(!$this->check_city($case['city_id'])){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('gybjcase/case')->delete($case_id)){
					K::M('gybjcase/case')->down_count($case);
                    $this->err->add('删除案例成功');
                }
            }
        }else if($ids = $this->GP('case_id')){
            if($items = K::M('gybjcase/case')->items_by_ids($ids)){
                ...
                if($aids && K::M('gybjcase/case')->delete($aids)){
					K::M('gybjcase/case')->downs_count($items);
                    $this->err->add('批量删除成功');
                }
            }
        }else{
