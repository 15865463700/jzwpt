...
// class Ctl_Rzps_Company_Case extends Ctl_Rzps_Ruanzhuangpeishi
class Ctl_Rzps_Company_Case extends Ctl
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'rzps/company/case/items.html';
    }
        ...
        if (K::M('member/integral')->check('case', $this->MEMBER) === false) {
            ...
            if ($data = $this->checksubmit('data')) {
                if (! $data = $this->check_fields($data, $this->_allower_fields)) {
                    ...
                    if ($case_id = K::M('rzpscase/case')->create($data)) {
                        if (! $attr = $this->GP('attr')) {
                            $attr = array();
                        }
                        K::M('rzpscase/attr')->update($case_id, $attr);
                        K::M('rzpscompany/company')->update_count($company_id, 'case_num', 1);
                        ...
                        $this->err->add('添加案例成功');
                        $this->err->set_data('forward', $this->mklink('rzps/company/case:detail', array(
                            $case_id
                        ), array(), true));
                    }
                }
                ...
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'rzps/company/case/create.html';
            }
    }
    ...
    public function edit($case_id = null)
    {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('rzpscase/case')->detail($case_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    if (! $data = $this->check_fields($data, $this->_allower_fields)) {
                        ...
                        if (K::M('rzpscase/case')->update($case_id, $data)) {
                    ...
                    if ($attrs = K::M('rzpscase/attr')->attrs_by_case($case_id)) {
                    ...
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'rzps/company/case/edit.html';
                }
    }
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('rzpscase/case')->detail($case_id)) {
                ...
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    $pager['count'] = $count = 0;
                    $this->pagedata['detail'] = $detail;
                    if ($items = K::M('rzpscase/photo')->items_by_case($case_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("rzps/company/case:detail", array(
                            $case_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'rzps/company/case/detail.html';
                }
    }
                            ...
                            if (UPLOAD_ERR_OK != $attach['error']) {
                                ...
                                if ($data = K::M('case/photo')->upload($case_id, $attach)) {
                                    ...
                                    if ($allow_case != $case['audit'] && empty($case['photos'])) {
                                        K::M('rzpscase/case')->update($case_id, array(
                                            'audit' => $allow_case
                                        ));
                                        K::M('member/integral')->commit('case', $this->MEMBER, '发布案例');
                                    }
            ...
            if (! $detail = K::M('rzpscase/case')->detail($case_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    if (empty($photo_ids)) {
                        ...
                        if (! $photoinfos = K::M('case/photo')->items_by_ids($photo_ids)) {
                            $this->err->add('没有您要更新的内容', 212);
                        } else {
                            $obj = K::M('rzpscase/photo');
                            foreach ($data as $k => $v) {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $case = K::M('rzpscase/case')->detail($case_id)) {
                ...
                if ($case['company_id'] != $company['company_id']) {
                    ...
                    if (K::M('rzpscase/case')->delete($case_id)) {
                        K::M('rzpscompany/company')->update_count($company['company_id'], 'case_num', - 1);
                        $this->err->add('删除案例成功');
                    }
    }
    ...
    public function defaultphoto($photo_id = null)
    {
        ...
        if (! ($photo_id = (int) $photo_id) && ! ($photo_id = (int) $this->GP('photo_id'))) {
            ...
            if (! $detail = K::M('case/photo')->detail($photo_id)) {
                $this->err->add('效果图不存在或已经删除', 211);
            } else 
                if (! $case = K::M('rzpscase/case')->detail($detail['case_id'])) {
                    $this->err->add('案例不存在或已经删除', 212);
                } else 
                    if ($case['company_id'] != $company['company_id']) {
                        $this->err->add('不许越权管理别人的内容', 213);
                    } else {
                        if (K::M('rzpscase/case')->update($detail['case_id'], array(
                            'photo' => $detail['photo']
                        ))) {
                            $this->err->add('修改封面成功');
                        }
    ...
    public function deletephoto($photo_id = null)
    {
        ...
        if (! ($photo_id = (int) $photo_id) && ! ($photo_id = (int) $this->GP('photo_id'))) {
            ...
            if (! $detail = K::M('rzpscase/photo')->detail($photo_id)) {
                $this->err->add('效果图不存在或已经删除', 211);
            } else 
                if (! $case = K::M('rzpscase/case')->detail($detail['case_id'])) {
                    $this->err->add('案例不存在或已经删除', 212);
                    ...
                    if ($case['company_id'] != $company['company_id']) {
                        ...
                        if (K::M('rzpscase/photo')->delete($photo_id)) {
    ...
    {
        $this->company = K::M('rzpscompany/company')->company_by_uid($this->uid);
        if ($this->company) {
            ...
            if ($this->request['ctl'] == 'rzps/company' && $this->request['act'] == 'info') {
                $this->pagedata['company_no_open'] = true;
                return false;
            } else {
                $this->pagedata['company_no_open'] = true;
                $this->tmpl = 'rzps/company/info.html';
            }
        $this->response();
    }
}