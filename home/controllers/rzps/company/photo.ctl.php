...
// class Ctl_Rzps_Company_Photo extends Ctl_Rzps_Ruanzhuangpeishi
class Ctl_Rzps_Company_Photo extends Ctl
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'rzps/company/photo/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                ...
                if ($pic_id = K::M('company/photo')->create($data)) {
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('rzps/company/photo:index'));
                }
            }
        } else {
            $this->pagedata['type_list'] = K::M('company/photo')->get_type_means();
            $this->tmpl = 'rzps/company/photo/create.html';
        }
    }
                    ...
                    if ($a = $data[$k]) {
                        if ($v['company_id'] == $company['company_id']) {
                            if ($a['title'] != $v['title'] || $a['type'] != $v['type']) {
                                K::M('company/photo')->update($k, array(
                                    'type' => $a['type'],
                                    'title' => $a['title']
                                ));
                            }
