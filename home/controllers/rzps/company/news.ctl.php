...
// class Ctl_Rzps_Company_News extends Ctl_Rzps_Ruanzhuangpeishi
class Ctl_Rzps_Company_News extends Ctl
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'rzps/company/news/items.html';
    }
                ...
                if ($allow_news < 0) {
                    ...
                    if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                        ...
                        if ($news_id = K::M('rzpscompany/news')->create($data)) {
                            K::M('rzpscompany/news')->news_count($company['company_id']);
                            K::M('system/integral')->commit('company_news', $this->MEMBER, '添加公司新闻');
                            $this->err->add('添加公司新闻成功');
                            $this->err->set_data('forward', $this->mklink('rzps/company/news:index'));
                        }
                    }
            } else {
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'rzps/company/news/create.html';
            }
    }
    ...
    public function edit($news_id = null)
    {
        ...
        if (! ($news_id = (int) $news_id) && ! ($news_id = (int) $this->GP('news_id'))) {
            ...
            if (! $detail = K::M('rzpscompany/news')->detail($news_id)) {
                ...
                if ($company['company_id'] != $detail['company_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'rzps/company/news/edit.html';
                    }
    }
        ...
        if (! ($news_id = (int) $news_id) && ! ($news_id = $this->GP('news_id'))) {
            ...
            if (! $detail = K::M('rzpscompany/news')->detail($news_id)) {
                ...
                if ($company['company_id'] != $detail['company_id']) {
                    ...
                    K::M('rzpscompany/news')->delete($news_id);
                    $this->err->add('删除资讯成功');
                }
    }
