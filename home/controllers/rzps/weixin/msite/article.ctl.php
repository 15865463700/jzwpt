...
class Ctl_Rzps_Weixin_Msite_Article extends Ctl_Rzps_Ruanzhuangpeishi
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'rzps/weixin/msite/article/items.html';
    }
            ...
            if ($this->MEMBER['group']['allow_msite'] < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($article_id = K::M('weixin/msite/article')->create($data)) {
                        $this->err->add('添加文章成功');
                        $this->err->set_data('forward', $this->mklink('rzps/weixin/msite/article:index'));
                    }
                }
        } else {
            $this->pagedata['cate_list'] = $cate_list;
            $this->tmpl = 'rzps/weixin/msite/article/create.html';
        }
    }
            ...
            if (! $detail = K::M('weixin/msite/article')->detail($article_id)) {
                ...
                if ($weixin['wx_id'] != $detail['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'rzps/weixin/msite/article/edit.html';
                    }
    }
