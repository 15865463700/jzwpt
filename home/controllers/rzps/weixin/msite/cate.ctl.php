...
class Ctl_Rzps_Weixin_Msite_Cate extends Ctl_Rzps_Ruanzhuangpeishi
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'rzps/weixin/msite/cate/items.html';
    }
            ...
            if ($this->MEMBER['group']['priv']['allow_msite'] < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($article_id = K::M('weixin/msite/cate')->create($data)) {
                        $this->err->add('添加分类成功');
                        $this->err->set_data('forward', $this->mklink('rzps/weixin/msite/cate:index'));
                    }
                }
        } else {
            $this->tmpl = 'rzps/weixin/msite/cate/create.html';
        }
    }
            ...
            if (! $detail = K::M('weixin/msite/cate')->detail($cat_id)) {
                ...
                if ($weixin['wx_id'] != $detail['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'rzps/weixin/msite/cate/edit.html';
                    }
    }
