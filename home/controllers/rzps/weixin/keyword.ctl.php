 ...
 * $Id$
 */
class Ctl_Rzps_Weixin_Keyword extends Ctl_Rzps_Ruanzhuangpeishi
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'rzps/weixin/keyword/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, 'keyword,reply_id,content')) {
                ...
                if ($kw_id = K::M('weixin/keyword')->create($data)) {
                    $this->err->set_data('forward', $this->mklink('rzps/weixin/keyword:index'));
                    $this->err->add('添加关键字成功');
                }
            }
        } else {
            $this->tmpl = 'rzps/weixin/keyword/create.html';
        }
    }
        ...
        if (! ($kw_id = (int) $kw_id) && ! ($kw_id = (int) $this->GP('kw_id'))) {
            ...
            if (! $detail = K::M('weixin/keyword')->detail($kw_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $this->tmpl = 'rzps/weixin/keyword/edit.html';
                }
    }
