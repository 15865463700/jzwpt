 ...
 * Class Ctl_Rzps
 */
class Ctl_Rzps_Ruanzhuangpeishi extends Ctl
{
    ...
    {
        if ($this->MEMBER['from'] != '软装配饰') {
            $this->err->add('您的帐号不是软装配饰公司类型', 211);
            $this->response();
        } else 
            if ($this->company = K::M('rzpscompany/company')->company_by_uid($this->uid)) {
    ...
    // return $this->shop;
    // }else if($this->request['ctl'] == 'rzps/shop' && $this->request['act'] == 'info'){
