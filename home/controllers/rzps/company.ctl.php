...
// class Ctl_Rzps_Company extends Ctl_Rzps_Ruanzhuangpeishi{
// }
class Ctl_Rzps_Company extends Ctl
{

    protected $_allow_fields = 'city_id,area_id,title,info,name,slogan,contact,phone,addr,qq,mobile,lng,lat,video,banner';
    ...
    public function index()
    {
        ...
        $this->tmpl = 'rzps/company/index.html';
    }
    ...
    public function refresh()
    {
        ...
        if ($this->GP('fromid')) {
            ...
            if ($isrefresh && K::M('member/flush')->create($data)) {
                K::M('company/company')->update($company['company_id'], array(
                    'flushtime' => __TIME
                ));
                $this->err->add('刷新成功');
            }
            ...
            $this->tmpl = 'rzps/company/refresh/look.html';
        }
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                ...
                if (! $company['company_id']) {
                    ...
                    if ($company_id = K::M('rzpscompany/company')->create($data)) {
                        K::M('member/member')->update($this->uid, array(
                            'group_id' => $data['group_id']
                        ));
                        if ($attr = $this->GP('attr')) {
                            K::M('rzpscompany/attr')->update($company_id, $attr);
                        }
                        if ($fields = $this->GP('fields')) {
                            if ($fields = $this->check_fields($fields, array(
                                'info'
                            ))) {
                                K::M('rzpscompany/fields')->update($company_id, $fields);
                            }
                        }
                        $this->err->add('设置公司资料成功');
                    }
                } else 
                    if (K::M('rzpscompany/company')->update($company['company_id'], $data)) {
                        if ($attr = $this->GP('attr')) {
                            K::M('rzpscompany/attr')->update($company['company_id'], $attr);
                        }
                        if ($fields = $this->GP('fields')) {
                            if ($fields = $this->check_fields($fields, array(
                                'info'
                            ))) {
                                K::M('rzpscompany/fields')->update($company['company_id'], $fields);
                            }
                        }
                        $this->err->add('设置公司资料成功');
                    }
            ...
            // if($attrs = K::M('rzpscompany/attr')->attrs_by_company($company['company_id'])){
            ...
            if ($domain = $this->checksubmit('domain')) {
                if (! $company['group']['priv']['allow_domain']) {
                    ...
                    if ($company['domain']) {
                        ...
                        if (K::M('company/company')->update_domain($company['company_id'], $domain)) {
                            $this->err->add('设置个性域名成功');
                        }
            } else {
                $this->pagedata['domain_company'] = $CFG['domain']['company'] . '.' . $CFG['site']['domain'];
                $this->tmpl = 'rzps/company/domain.html';
            }
    }
        ...
        if ($skin = $this->checksubmit('skin')) {
            if ($audit_skin < 0) {
                ...
                if (! $cfg = $skins[$skin]) {
                    ...
                    if (K::M('company/fields')->update($company['company_id'], array(
                        'skin' => $skin
                    ), true)) {
            ...
            $this->tmpl = 'rzps/company/skin.html';
        }
    }
}