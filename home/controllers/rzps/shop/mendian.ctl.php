 ...
 * $Id$
 */
class Ctl_Rzps_Shop_mendian extends Ctl_Rzps_Ruanzhuangpeishi
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'rzps/shop/mendian/items.html';
    }
            ...
            if ($allow_mendian < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($mendian_id = K::M('shop/mendian')->create($data)) {
                        $this->err->add('添加内容成功');
                        $this->err->set_data('forward', $this->mklink('rzps/shop/mendian:index'));
                    }
                }
        } else {
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'rzps/shop/mendian/create.html';
        }
    }
            ...
            if (! $detail = K::M('shop/mendian')->detail($mendian_id)) {
                ...
                if ($shop['shop_id'] != $detail['shop_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'rzps/shop/mendian/edit.html';
                    }
    }
