...
class Ctl_Home2 extends Ctl
{
    ...
    public function items()
    {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('home:items', array(
                $area_id,
                implode('-', $vids)
            ));
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('home:items', array(
                    $area_id,
                    implode('-', $vids)
                ));
                $v['values'][$kk] = $vv;
        ...
        $area_all_link = $this->mklink('home:items', array(
            0,
        ...
        $order_list[0]['link'] = $this->mklink('home:items', array(
            $area_id,
            implode('-', $attr_value_ids),
            0,
            1
        ));
        $order_list[1]['link'] = $this->mklink('home:items', array(
            $area_id,
            implode('-', $attr_value_ids),
            1,
            1
        ));
        $order_list[2]['link'] = $this->mklink('home:items', array(
            $area_id,
            implode('-', $attr_value_ids),
            2,
            1
        ...
        if ($items = K::M('home/home')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('home:items', array(
                $area_id,
                implode('-', $attr_value_ids),
                $order,
                '{page}'
    ...
    public function cases($home_id, $page = 1)
    {
        ...
        if ($items = K::M('case/case')->items($filter, NULL, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('home:cases', array(
                $home_id,
                '{page}'
            )));
    ...
    public function photo($home_id, $type, $page)
    {
        ...
        if ($items = K::M('home/photo')->items($filter, NULL, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('home:photo', array(
                $home_id,
                $type,
                '{page}'
            )));
    ...
    public function site($home_id, $page = 1)
    {
        ...
        if ($items = K::M('home/site')->items($filter, NULL, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('home:site', array(
                $home_id,
                '{page}'
            )));
            $company_ids = $uids = array();
    ...
    public function caseDetail($home_id, $case_id, $page = 1)
    {
        ...
        if (! $case_id = (int) $case_id) {
            ...
            if (! $case = K::M('case/case')->detail($case_id)) {
                ...
                if ($items = K::M('case/photo')->items(array(
                    'case_id' => $case_id,
                    'closed = 0'
            ...
            if (is_numeric($SO['lng_start']) && is_numeric($SO['lng_end']) && is_numeric($SO['lat_start']) && is_numeric($SO['lat_end'])) {
                ...
                foreach ($items as $val) {
                    $data[$val['home_id']] = array(
                        'home_id' => $val['home_id'],
                        'link' => $this->mklink('home:detail', array(
                            $val['home_id']
                        ), array(), true),
                        'name' => $val['name'],
                        'thumb' => $val['thumb'],
    ...
    public function tuan($area_id = 0, $order = 0, $page = null)
    {
        ...
        $area_list = $this->request['area_list'];
        $area_all_link = $this->mklink('home:tuan', array(
            0,
            $order,
            1
        ));
        foreach ($area_list as $k => $v) {
            $v['link'] = $this->mklink('home:tuan', array(
                $k,
                $order,
                1
            ));
        ...
        $order_list[0]['link'] = $this->mklink('home:tuan', array(
            $area_id,
            0,
            1
        ));
        $order_list[1]['link'] = $this->mklink('home:tuan', array(
            $area_id,
            1,
            1
        ));
        $order_list[2]['link'] = $this->mklink('home:tuan', array(
            $area_id,
            2,
            1
        ));
        ...
        if ($items = K::M('home/tuan')->items($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('home:tuan', array(
                $area_id,
                $order,
                '{page}'
            ), $params));
    ...
    public function tuanDetail($tuan_id)
    {
        if (! ($tuan_id = (int) $tuan_id) && ! ($tuan_id = (int) $this->GP('tuan_id'))) {
            ...
            if (! $detail = K::M('home/tuan')->detail($tuan_id)) {
                $this->err->add('没有您要查看的团装', 212);
            } else {
                $package = K::M('home/package')->items(array(
                    'tuan_id' => $tuan_id
                ));
                $huxingIds = array();
                ...
                $this->pagedata['mobile_url'] = $this->mklink('mobile/home:tuanDetail', array(
                    $tuan_id
                ));
    ...
    protected function check_home($home_id)
    {
        ...
        $this->seo->init('home_detail', array(
            'home_name' => $home['name']
        ));
        return $home;
    }
