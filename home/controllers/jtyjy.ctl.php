...
class Ctl_Jtyjy extends Ctl{
    ...
    public function items($page = 1){
        ...
        $attr_values = K::M('data/attr')->attrs_by_from('zx:jtyjy', true);
        $uri = $this->request['uri'];
        if(preg_match('/items(-[\d\-]+)?(-(\d+)).html/i', $uri, $m)||preg_match('/jtyjy(-[\d\-]+)?(-(\d+))(\.html)*/i', $uri, $m)){
            $page = (int)$m[3];
            if($m[1]){
            ...
            if($v['filter']){
                ...
                foreach($attr_vids as $vv){
                    if($v['values'][$vv]){
                        ...
                        $_seo_repeat.=",{$this->request['city']['city_name']}{$v['values'][$vv]['title']}家庭影剧院公司";
                    }
                }
            }
        }
        ...
        foreach($attr_values as $k=>$v){
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('jtyjy:items', array($area_id, $group_id, implode('-', $vids)));
                $v['link']=preg_replace("/-items/","",$v['link']);
                $v['link']=preg_replace("/jtyjy-0-0-0-0-0-1\./","jtyjy.",$v['link']);
            ...
            foreach($v['values'] as $kk=>$vv){
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('jtyjy:items', array($area_id, $group_id, implode('-', $vids)));
                $vv['link']=preg_replace("/-items/","",$vv['link']);
                $vv['link']=preg_replace("/jtyjy-0-0-0-0-0-1\./","jtyjy.",$vv['link']);
        ...
        // $group_all_link = $this->mklink('jtyjy:items', array($area_id, 0, implode('-', $attr_value_ids), $order, 1));
        // foreach($group_list as $k=>$v){
        // $v['link'] = $this->mklink('jtyjy:items', array($area_id, $k, implode('-', $attr_value_ids), $order, 1));
        // $group_list[$k] = $v;
        // }
        // }
        $area_list = $this->request['area_list'];
        $area_all_link = $this->mklink('jtyjy:items', array(
            0,
            $group_id,
        ...
        $area_all_link=preg_replace("/jtyjy-0-0-0-0-0-1\./","jtyjy.",$area_all_link);
        $area_all_link=preg_replace("/\.html/","/",$area_all_link);
        foreach ($area_list as $k => $v) {
            $v['link'] = $this->mklink('jtyjy:items', array(
                $k,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
        ...
        $order_list[0]['link'] = $this->mklink('jtyjy:items', array($area_id, $group_id, implode('-', $attr_value_ids), 0, 1));
        $order_list[1]['link'] = $this->mklink('jtyjy:items', array($area_id, $group_id, implode('-', $attr_value_ids), 1, 1));
        $order_list[2]['link'] = $this->mklink('jtyjy:items', array($area_id, $group_id, implode('-', $attr_value_ids), 2, 1));
        $pager = $filter = array();
        ...
        $items = K::M('jtyjycompany/company')->items($filter, $orderby, $page, $limit, $count);
        foreach ($items as $key => $value) {
            $fields = K::M('jtyjycompany/fields')->detail($value["company_id"]);
        ...
        if ($items) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('jtyjy:items', array(
                $area_id,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
            ...
            $pager['pagebar']=preg_replace("/jtyjy-0-0-0-0-0-1\./","jtyjy.",$pager['pagebar']);
            $pager['pagebar']=preg_replace("/\.html/","/",$pager['pagebar']);
            foreach($items as $k=>$v){
                $items[$k]['company_url']=preg_replace("/\/jtyjycompany-/","/jtyjycompany/",$v['company_url']);
                $items[$k]['company_url']=  str_replace(Ctl_Jtyjy::$ctl_jtyjycompany_old, Ctl_Jtyjy::$ctl_jtyjycompany_new, $items[$k]['company_url']);
        ...
        if(is_main()){
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('jtyjy',$seo):$this->seo->init('jtyjy_attr',$seo);
        }else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_jtyjy',$seo):$this->seo->init('fenzhan_jtyjy_attr',$seo);
        }
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/jtyjy/";
        $this->tmpl = 'jtyjy/items.html';
    }
    ...
    public function map()
    {
        ...
        $this->pagedata['area_list'] = $area_list;
        $this->pagedata['attr_values'] = $attr_values;
        K::M('helper/seo')->init('jtyjy_maps', array());
        $this->tmpl = 'jtyjy/maps.html';
    }
    ...
    public function yuyue($company_id)
    {
        if (! ($company_id = (int) $company_id) && ! ($company_id = (int) $this->GP('company_id'))) {
            ...
            if (! $detail = K::M('jtyjycompany/company')->detail($company_id)) {
                $this->err->add('家庭影剧院公司不存在或已经删除', 212);
            } else
                    ...
                    if ($this->checksubmit('data')) {
                        if (! $data = $this->GP('data')) {
                            ...
                            if ($verifycode_success) {
                                ...
                                $data['content'] = "预约家庭影剧院";
                                $data['city_id'] = $this->request['city_id'];
                                if ($yuyue_id = K::M('jtyjycompany/yuyue')->create($data)) {
                                    K::M('jtyjycompany/yuyue')->yuyue_count($company_id);
                                    $this->err->add('预约家庭影剧院公司成功！');
                                }
                            }
                        }
                    } else {
