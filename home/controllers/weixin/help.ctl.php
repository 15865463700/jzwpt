...
class Ctl_Weixin_Help extends Ctl_Weixin
{
    ...
    public function preview($help_id = null)
    {
        if (! ($help_id = (int) $help_id) && ! ($help_id = $this->GP('help_id'))) {
            ...
            if (! $detail = K::M('weixin/help')->detail($help_id)) {
                ...
                $list = K::M('weixin/helpsn')->items(array(
                    'help_id' => $help_id,
                    'openid' => $openid
                ));
                foreach ($list as $k => $v) {
                ...
                if ($sn_list = K::M('weixin/helpsn')->items($filter, array(
                    'zhuli' => 'desc'
                ), $page, '10', $count)) {
                ...
                if ($my_sn_list) {
                    $this->pagedata['url'] = $this->mklink('weixin/help/show', array(
                        $my_sn_list['sn_id']
                    ));
                }
    ...
    public function show($sn_id)
    {
        if (! ($sn_id = (int) $sn_id) && ! ($sn_id = $this->GP('sn_id'))) {
            ...
            if (! $helpsn = K::M('weixin/helpsn')->detail($sn_id)) {
                $this->err->add('该用户不存在', 212);
            } else 
                if (! $detail = K::M('weixin/help')->detail($helpsn['help_id'])) {
                    $this->err->add('该助力不存在或已经删除', 212);
                } else {
                    ...
                    if ($openid == $helpsn['openid']) {
                        ...
                        $list = K::M('weixin/helpsn')->items(array(
                            'help_id' => $helpsn['help_id'],
                            'openid' => $helpsn['openid']
                        ));
                        foreach ($list as $k => $v) {
                            $this->pagedata['my_sn_list'] = $my_sn_list = $v;
                        }
                        ...
                        $this->pagedata['url2'] = $this->mklink('weixin/help/preview', array(
                            $helpsn['help_id']
                        ));
                        ...
                        $this->pagedata['detail'] = $detail;
                        if ($list_sn = K::M('weixin/helplist')->items(array(
                            'openid' => $helpsn['openid'],
                            'zhuliid' => $openid
                        ))) {
