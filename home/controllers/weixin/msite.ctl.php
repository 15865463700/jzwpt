...
class Ctl_Weixin_Msite extends Ctl_Weixin
{
    ...
    public function index($wx_id)
    {
        if (! $wx_id = (int) $wx_id) {
            ...
            if (! $msite = K::M('weixin/msite')->detail($wx_id)) {
                $this->error(404);
            } else {
    ...
    public function article($article_id)
    {
        if (! $article_id = (int) $article_id) {
            ...
            if (! $detail = K::M('weixin/msite/article')->detail($article_id)) {
                ...
                if (! $msite = K::M('weixin/msite')->detail($detail['wx_id'])) {
                    $this->error(404);
                } else 
                    if ($detail['link'] && strpos($detail['link'], 'http') !== false) {
    ...
    public function cate($cat_id, $page = 1)
    {
        if (! $cat_id = (int) $cat_id) {
            ...
            if (! $cate = K::M('weixin/msite/cate')->detail($cat_id)) {
                $this->error(404);
            } else 
                if (! $msite = K::M('weixin/msite')->detail($cate['wx_id'])) {
                    $this->error(404);
                } else 
                    if (K::M('verify/check')->url($cate['link'])) {
