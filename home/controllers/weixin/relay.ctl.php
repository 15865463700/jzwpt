...
class Ctl_Weixin_Relay extends Ctl_Weixin
{
    ...
    public function preview($relay_id = null)
    {
        if (! ($relay_id = (int) $relay_id) && ! ($relay_id = $this->GP('relay_id'))) {
            ...
            if (! $detail = K::M('weixin/relay')->detail($relay_id)) {
                ...
                $list = K::M('weixin/relaysn')->items(array(
                    'relay_id' => $relay_id,
                    'openid' => $openid
                ));
                foreach ($list as $k => $v) {
                ...
                if ($sn_list = K::M('weixin/relaysn')->items($filter, array(
                    'gold' => 'desc'
                ), $page, '10', $count)) {
                ...
                if ($my_sn_list) {
                    $this->pagedata['url'] = $this->mklink('weixin/relay/show', array(
                        $my_sn_list['sn_id']
                    ));
                }
    ...
    public function show($sn_id = null)
    {
        if (! ($sn_id = (int) $sn_id) && ! ($sn_id = $this->GP('sn_id'))) {
            ...
            if (! $relaysn = K::M('weixin/relaysn')->detail($sn_id)) {
                $this->err->add('该用户不存在', 212);
            } else 
                if (! $detail = K::M('weixin/relay')->detail($relaysn['relay_id'])) {
                    $this->err->add('该接力不存在或已经删除', 212);
                } else {
                    if (empty($openid)) {
                        $openid = $this->access_openid();
                    ...
                    if ($openid == $relaysn['openid']) {
                        ...
                        // mine
                        $my_list = K::M('weixin/relaylist')->items(array(
                            'relay_id' => $relay_id,
                            'openid' => $relaysn['openid'],
                            'jieliid' => $openid
                        ), null, 1, 100, $my_count);
                        ...
                        $this->pagedata['url2'] = $this->mklink('weixin/relay/preview', array(
                            $detail['relay_id']
                        ));
                        $member = K::M('member/weixin')->detail_by_openid($openid);
