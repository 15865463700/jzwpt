...
class Ctl_Weixin_Lottery extends Ctl_Weixin
{
    ...
    public function show($lottery_id)
    {
        if (! ($lottery_id = (int) $lottery_id) && ! ($lottery_id = $this->GP('lottery_id'))) {
            $this->err->add('没有指定大转盘ID', 211);
        } else 
            if (! $detail = K::M('weixin/lottery')->detail($lottery_id)) {
                $this->err->add('该大转盘不存在或已经删除', 212);
            } else {
                if (empty($openid)) {
                ...
                if (! empty($detail['stime']) && $detail['stime'] >= time()) {
                    ...
                    if (! empty($detail['ltime']) && $detail['ltime'] <= time()) {
                        ...
                        if ($detail['max_count'] > 0 && $detail['max_count'] <= count($list)) {
                            ...
                            if ($detail['follower_condtion'] == 1 && $wx_info['subscribe'] == 0) {
                ...
                $data['usenums'] = K::M('weixin/lotterysn')->count(array(
                    'openid' => $openid
                ));
                ...
                foreach ($record as $k => $v) {
                    switch ($v['prize']) {
                        case 1:
                            $record[$k]['award'] = $data['fist'];
                            break;
                        case 2:
                            $record[$k]['award'] = $data['second'];
                            break;
                        case 3:
                            $record[$k]['award'] = $data['third'];
                            break;
                        case 4:
                            $record[$k]['award'] = $data['four'];
                            break;
                        case 5:
                            $record[$k]['award'] = $data['five'];
                            break;
                        case 6:
                            $record[$k]['award'] = $data['six'];
                            break;
                    }
    ...
    public function prizeHandle($uid, $wx_id, $Lottery)
    {
        ...
        if ($record = K::M('weixin/lotterysn')->items(array(
            'openid' => $openid
        ...
        K::M('weixin/lotterysn')->items(array(
            'openid' => $openid
        ), null, null, null, $count);
    ...
    protected function get_prize($Lottery)
    {
        ...
        $prize_arr = array(
            ...
            '6' => array(
                'id' => 7,
                'prize' => '谢谢参与',
                'v' => (intval($Lottery['predict_num'])) * $multi - ($firstNum + $secondNum + $thirdNum + $fourthNum + $fifthNum + $sixthNum),
                'start' => $firstNum + $secondNum + $thirdNum + $fourthNum + $fifthNum + $sixthNum,
                'end' => intval($Lottery['predict_num']) * $multi
            )
        );
        //
