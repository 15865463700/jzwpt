...
class Ctl_Bssjs extends Ctl{
    public static $ctl_old="designer";
    public static $ctl_new="bssjs";
    public static $tpl_bssjs="bssjs";
    ...
    public function items($page = 1){
        if(strpos($this->request['uri'],"esigner")){
            $uri=str_replace(Ctl_Bssjs::$ctl_old, Ctl_Bssjs::$ctl_new, $this->request['uri']);
            header("Location:{$this->request['city']['siteurl']}/{$uri}");die;
        }
        $this->request['uri']=  str_replace(Ctl_Bssjs::$ctl_new, Ctl_Bssjs::$ctl_old, $this->request['uri']);
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $v["link"]=str_replace(Ctl_Bssjs::$ctl_old,Ctl_Bssjs::$ctl_new,$v["link"]);
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vv["link"]=str_replace(Ctl_Bssjs::$ctl_old,Ctl_Bssjs::$ctl_new,$vv["link"]);
                $v['values'][$kk] = $vv;
            }
            $attr_values[$k] = $v;
        }
        ...
        $area_all_link=str_replace(Ctl_Bssjs::$ctl_old,Ctl_Bssjs::$ctl_new,$area_all_link);
        ...
        if ($items = K::M('designer/designer')->items_by_attr($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['pagebar']=str_replace(Ctl_Bssjs::$ctl_old,Ctl_Bssjs::$ctl_new,$pager['pagebar']);
        ...
        foreach($order_list as $k=>$v){
            ...
            $order_list[$k]["link"]=str_replace(Ctl_Bssjs::$ctl_old,Ctl_Bssjs::$ctl_new,$order_list[$k]["link"]);
        }
...
//			empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('designer', $seo):$this->seo->init('designer_attr', $seo);
			empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('bshusjs_items', $seo):$this->seo->init('bshusjs_items_attr', $seo);
		}else{
//            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_designer', $seo):$this->seo->init('fenzhan_designer_attr', $seo);
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_bshusjs_items', $seo):$this->seo->init('fenzhan_bshusjs_items_attr', $seo);
		}
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/bssjs/";
        $this->tmpl = Ctl_Bssjs::$tpl_bssjs.'/items.html';
    }
            ...
            if (! $detail = K::M('designer/designer')->detail($uid)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($this->checksubmit('data') || $flag) {
                        ...
                        $this->tmpl = Ctl_Bssjs::$tpl_bssjs.'/yuyue.html';
                    }
                }
    }
