...
class Ctl_Ask extends Ctl{
    public function index()
    {
        ...
        $this->seo->init('ask');
            $this->pagedata['canonical'] = "www.jzwpt.com/ask/";
        $this->tmpl = 'ask/ask.html';
    }

    public function catetreev($cat_id = 0, $type = 0, $page = 1)
    {
        $list=K::M('ask/cate')->childrens($cat_id);
        foreach($list as $k=>$v){
            $list[$k]['subs']=K::M('ask/cate')->childrens($v['cat_id']);
        }
        $this->pagedata['items'] = $list;
        $this->tmpl = 'ask/catetreev.html';
    }
    public function items($cat_id = 0, $type = 0, $page = 1){
        ...
        if('catetreev'==$_REQUEST['a']){
            $this->catetreev($cat_id,$type,$page);
        }else{
            $this->items2($cat_id,$type,$page);
        }
    }
    public function items2($cat_id = 0, $type = 0, $page = 1)
    {
        $session = K::M('system/session')->start();
        if($session->get("substation")){
            $this->request['city']=$session->get("substation");
        }
        if($_REQUEST["a"]&&'items'!=$_REQUEST["a"]){
            $cat_id=$_REQUEST["a"];
        ...
        if ($items = K::M('ask/ask')->items($filter, null, $page, $limit, $count)) {
            ...
            $pager['pagebar']=preg_replace("/ask-items-([a-zA-Z0-9]*)/","ask/{$this->pagedata['cate']['py']}",$pager['pagebar']);
            ...
            if (! $detail = K::M('ask/ask')->detail($ask_id)) {
                ...
                if (! $detail['audit']) {
                    ...
                    foreach($member_list as $k=>$v){
                        $uname=$member_list[$k]['uname'];
                        if(K::M('verify/check')->phone($uname)){
                            $member_list[$k]['uname']=substr_replace($uname,"*****",3,5);
                        }
                    }
                    ...
                    $this->pagedata['canonical'] = "www.jzwpt.com/ask/{$ask_id}.html";
    ...
    public function yes($answer_id = null)
    {
        if (! $this->check_login()) {
            ...
            K::M('ask/ask')->update($detail['ask_id'], array(
                'answer_id' => $answer_id
            ));
            $this->err->add('操作成功！');
        }
