...
class Ctl_Gs extends Ctl{
    ...
    public function items($page = 1){
        ...
        if (preg_match('/items(-[\d\-]+)?(-(\d+)).html/i', $uri, $m)||preg_match('/gs(-[\d\-]+)?(-(\d+))(\.html)*/i', $uri, $m)) {
            $page = (int) $m[3];
            if ($m[1]) {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('gs:items', array(
                $area_id,
                $group_id,
                implode('-', $vids)
            ));
            ...
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('gs:items', array(
                    $area_id,
                    $group_id,
                    implode('-', $vids)
                ));
        ...
        if ($group_list = K::M('member/group')->items_by_from('company')) {
            $group_all_link = $this->mklink('gs:items', array(
                $area_id,
                0,
        ...
        $area_all_link = $this->mklink('gs:items', array(
            0,
            $group_id,
            implode('-', $attr_value_ids),
            $order,
        ...
        $order_list[0]['link'] = $this->mklink('gs:items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            0,
        ...
        $order_list[1]['link'] = $this->mklink('gs:items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            1,
        ...
        $order_list[2]['link'] = $this->mklink('gs:items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            2,
        ...
        if ($items = K::M('company/company')->items($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('gs:items', array(
                $area_id,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
            ...
            foreach($items as $k=>$v){
                ...
                $items[$k]['company_url']=  str_replace(Ctl_Gs::$ctl_company_old, Ctl_Gs::$ctl_company_new, $items[$k]['company_url']);
            }
        ...
        foreach($order_list as $k=>$v){
            $order_list[$k]["link"] = preg_replace("/gs-items/","gs",$v["link"]);
            $order_list[$k]["link"] = preg_replace("/gs-0-0-0-0-0-1\./","gs.",$order_list[$k]["link"]);
        ...
        if(is_main()){
            ...
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_gs_items', $seo):$this->seo->init('seo_fenzhan_gs_items_attr', $seo);
		}
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/gs/";
