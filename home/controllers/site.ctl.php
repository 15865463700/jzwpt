...
class Ctl_Site extends Ctl
{
    ...
    public function items($area_id = 0, $status, $order = 0, $page = null)
    {
        ...
        if ($items = K::M('home/site')->items($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('site:items', array(
                $area_id,
                $order,
                '{page}'
            )));
        ...
        foreach ($order_list as $k => $v) {
            ...
            $order_list[$k]['link'] = $this->mklink('site:items', array(
                $area_id,
                $status,
                $k,
                $page
        ...
        if (! $site_id = (int) $site_id) {
            ...
            if (! $site = K::M('home/site')->detail($site_id)) {
                ...
                if (empty($site['audit'])) {
                    ...
                    $this->pagedata['mobile_url'] = $this->mklink('mobile/site:detail', array(
                        $site_id
                    ));
                    $this->pagedata['status'] = $status;
                    $this->pagedata['items'] = $items;
                    $this->seo->init('site_detail', array(
                        'title' => $site['title'],
            ...
            if (is_numeric($SO['lng_start']) && is_numeric($SO['lng_end']) && is_numeric($SO['lat_start']) && is_numeric($SO['lat_end'])) {
                ...
                foreach ($items as $val) {
                    $data[$val['site_id']] = array(
                        'site_id' => $val['site_id'],
                        'link' => $this->mklink('site:detail', array(
                            $val['site_id']
                        ), array(), true),
                        'title' => $val['title'],
    ...
    public function yuyue($site_id)
    {
        if (! ($site_id = (int) $site_id) && ! ($site_id = (int) $this->GP('site_id'))) {
            ...
            if (! $detail = K::M('home/site')->detail($site_id)) {
                ...
                if (! $company = K::M('company/company')->detail($detail['company_id'])) {
                            ...
                            if (! $data = $this->GP('data')) {
                                ...
                                if ($verifycode_success) {
                                    ...
                                    if ($yuyue_id = K::M('gz/yuyue')->create($data)) {
                                        K::M('gz/gz')->update($detail['uid'], array(
                                            'yuyue_num' => $detail['yuyue_num'] + 1
                                        ));
                                ...
                                if (! $data = $this->GP('data')) {
                                    ...
                                    if ($verifycode_success) {
                                        ...
                                        if ($yuyue_id = K::M('designer/yuyue')->create($data)) {
                                            K::M('designer/designer')->update($detail['uid'], array(
                                                'yuyue_num' => $detail['yuyue_num'] + 1
                                            ));
                                            $smsdata = $maildata = array(
                                                'contact' => $data['contact'] ? $data['contact'] : '业主',
                        ...
                        if (! $data = $this->GP('data')) {
                            ...
                            if ($verifycode_success) {
                                ...
                                if ($yuyue_id = K::M('company/yuyue')->create($data)) {
                                    K::M('company/company')->update($detail['company_id'], array(
                                        'yuyue_num' => $detail['yuyue_num'] + 1
                                    ));
                                    $smsdata = $maildata = array(
                                        'contact' => $data['contact'] ? $data['contact'] : '业主',
