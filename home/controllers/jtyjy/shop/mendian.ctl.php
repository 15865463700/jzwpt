 ...
 * $Id$
 */
class Ctl_Jtyjy_Shop_mendian extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'jtyjy/shop/mendian/items.html';
    }
            ...
            if ($allow_mendian < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($mendian_id = K::M('shop/mendian')->create($data)) {
                        $this->err->add('添加内容成功');
                        $this->err->set_data('forward', $this->mklink('jtyjy/shop/mendian:index'));
                    }
                }
        } else {
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'jtyjy/shop/mendian/create.html';
        }
    }
            ...
            if (! $detail = K::M('shop/mendian')->detail($mendian_id)) {
                ...
                if ($shop['shop_id'] != $detail['shop_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'jtyjy/shop/mendian/edit.html';
                    }
    }
