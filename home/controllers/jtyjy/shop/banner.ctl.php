...
class Ctl_Jtyjy_Shop_Banner extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'jtyjy/shop/banners.html';
    }
        ...
        if ($allow_banner < 0) {
            ...
            if (! $attach = $_FILES['Filedata']) {
                ...
                if (UPLOAD_ERR_OK != $attach['error']) {
                    ...
                    if (K::M('shop/banner')->count_by_shop($shop['shop_id']) > 5) {
    ...
    public function delete($banner_id = null)
    {
        ...
        if (! $banner_id = (int) $banner_id) {
            ...
            if (! $banner = K::M('shop/banner')->detail($banner_id)) {
                $this->err->add('您要删除的内容不存在或已经删除', 212);
            } else 
                if ($banner['shop_id'] != $shop['shop_id']) {
