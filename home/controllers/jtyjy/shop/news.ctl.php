 ...
 * $Id$
 */
class Ctl_Jtyjy_Shop_News extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'jtyjy/shop/news/items.html';
    }
            ...
            if ($allow_news < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($news_id = K::M('shop/news')->create($data)) {
                        $this->err->add('添加内容成功');
                        $this->err->set_data('forward', $this->mklink('jtyjy/shop/news:index'));
                    }
                }
        } else {
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'jtyjy/shop/news/create.html';
        }
    }
            ...
            if (! $detail = K::M('shop/news')->detail($news_id)) {
                ...
                if ($shop['shop_id'] != $detail['shop_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'jtyjy/shop/news/edit.html';
                    }
    }
