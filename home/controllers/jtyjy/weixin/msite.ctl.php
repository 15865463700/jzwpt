...
class Ctl_Jtyjy_Weixin_Msite extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    {
        $this->tmpl = 'jtyjy/weixin/msite/access.html';
    }
    ...
    public function index()
    {
        if ($data = $this->checksubmit('data')) {
            ...
            $this->tmpl = 'jtyjy/weixin/msite/index.html';
        }
    }
    ...
    public function tmpl($from = 'index')
    {
        ...
        if ($tmpl = $this->checksubmit('template')) {
            K::M('weixin/msite')->update($this->msite['wx_id'], array(
                "tmpl_{$from}" => $tmpl
            ));
            $this->err->add('更换模板成功');
        } else {
            ...
            $this->tmpl = 'jtyjy/weixin/msite/tmpl.html';
        }
    }
        ...
        while (false !== ($entry = $d->read())) {
            if ($entry != '.' && $entry != '..' && is_dir($tmpl_dir . '/' . $entry)) {
                if (file_exists($tmpl_dir . '/' . $entry . '/info.php')) {
                    $tmpl_list[substr($entry, 1)] = @include ($tmpl_dir . '/' . $entry . '/info.php');
                }
            }
        }
        $d->close();
        ksort($tmpl_list);
        foreach ($tmpl_list as $k => $v) {
            $t['V' . $k] = $v;
        }
        return $t;
    }
