 ...
 * $Id$
 */
class Ctl_Jtyjy_Weixin_Keyword extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'jtyjy/weixin/keyword/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, 'keyword,reply_id,content')) {
                ...
                if ($kw_id = K::M('weixin/keyword')->create($data)) {
                    $this->err->set_data('forward', $this->mklink('jtyjy/weixin/keyword:index'));
                    $this->err->add('添加关键字成功');
                }
            }
        } else {
            $this->tmpl = 'jtyjy/weixin/keyword/create.html';
        }
    }
        ...
        if (! ($kw_id = (int) $kw_id) && ! ($kw_id = (int) $this->GP('kw_id'))) {
            ...
            if (! $detail = K::M('weixin/keyword')->detail($kw_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $this->tmpl = 'jtyjy/weixin/keyword/edit.html';
                }
    }
