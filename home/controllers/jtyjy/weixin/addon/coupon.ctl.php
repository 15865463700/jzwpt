...
class Ctl_Jtyjy_Weixin_Addon_Coupon extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $pager['page'] = $limit = 30;
        $pager['page'] = $count = 0;
        if ($items = K::M('weixin/coupon')->items(array(
            'wx_id' => $weixin['wx_sid']
        ...
        $this->tmpl = 'jtyjy/weixin/addon/index.html';
    }
    ...
    public function create()
    {
        ...
        if ($data = $this->checksubmit('data')) {
            ...
            $this->tmpl = 'jtyjy/weixin/addon/create.html';
        }
    }
        ...
        if (! ($coupon_id = (int) $coupon_id) && ! ($coupon_id = $this->GP('coupon_id'))) {
            ...
            if (! $detail = K::M('weixin/coupon')->detail($coupon_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $this->tmpl = 'jtyjy/weixin/addon/edit.html';
                }
    }
    ...
    public function sign($coupon_id = null)
    {
        ...
        if (! ($coupon_id = (int) $coupon_id) && ! ($coupon_id = $this->GP('coupon_id'))) {
            ...
            if (! $detail = K::M('weixin/coupon')->detail($coupon_id)) {
                ...
                $list = K::M('weixin/couponsn')->items(array(
                    'coupon_id' => $coupon_id,
                    'openid' => $openid
                ));
                ...
                if (! empty($detail['ltime']) && $detail['ltime'] <= time()) {
                    ...
                    if ($detail['max_count'] > 0 && $detail['max_count'] <= count($list)) {
                        $error = '您的领取名额已用完啦';
                    } else 
                        if ($detail['num'] <= $detail['down_count']) {
                            $error = '优惠券已经领取光啦';
                        } else 
                            if ($detail['follower_condtion'] && $wx_info['subscribe'] == 0) {
                                ...
                                if ($detail['member_condtion'] == 1 && ! $member['uname']) {
                                    $error = '用户注册后才能领取';
                                } else {
                                    $data['sn'] = uniqid();
                                    $data['uid'] = $this->uid;
                ...
                if ($error) {
                    $this->tmpl = 'jtyjy/weixin/addon/over.html';
                }
            }
    }
    ...
    public function sn($coupon_id, $page_id)
    {
        if (! ($coupon_id = (int) $coupon_id) && ! ($coupon_id = $this->GP('coupon_id'))) {
            ...
            if (! $detail = K::M('weixin/coupon')->detail($coupon_id)) {
                ...
                $this->tmpl = 'jtyjy/weixin/addon/sn.html';
            }
    }
    ...
    public function show($sn)
    {
        ...
        if (! ($sn = (int) $sn) && ! ($sn = $this->GP('sn'))) {
            ...
            if (! $detail = K::M('weixin/couponsn')->detail($sn)) {
                $this->err->add('非法访问', 212);
            } else 
                if (! $coupon = K::M('weixin/coupon')->detail($detail['coupon'])) {
                    $this->err->add('非法访问', 212);
                } else {
                    $this->pagedata['detail'] = $detail;
                    $condition = array();
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'jtyjy/weixin/addon/show.html';
                }
    }
