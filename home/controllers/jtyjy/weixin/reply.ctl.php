...
class Ctl_Jtyjy_Weixin_Reply extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        if ($items = K::M('weixin/reply')->items(array(
            'wx_id' => $weixin['wx_id']
        ...
        $this->tmpl = 'jtyjy/weixin/reply/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, 'title,intro,content,jumpurl')) {
                ...
                if ($reply_id = K::M('weixin/reply')->create($data)) {
                    $this->err->set_data('forward', $this->mklink('jtyjy/weixin/reply:index'));
                    $this->err->add('添加微信素材成功');
                }
            }
        } else {
            $this->tmpl = 'jtyjy/weixin/reply/create.html';
        }
    }
            ...
            if (! $detail = K::M('weixin/reply')->detail($reply_id)) {
                ...
                if ($detail['wx_id'] != $weixin['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'jtyjy/weixin/reply/edit.html';
                    }
    }
    ...
    public function dialog($page = 1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/weixin/reply/dialog.html';
    }
}