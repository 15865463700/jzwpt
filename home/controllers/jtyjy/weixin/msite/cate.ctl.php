...
class Ctl_Jtyjy_Weixin_Msite_Cate extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $pager['page'] = $limit = 30;
        $pager['page'] = $count = 0;
        if ($items = K::M('weixin/msite/cate')->items(array(
            'wx_id' => $weixin['wx_id']
        ...
        $this->tmpl = 'jtyjy/weixin/msite/cate/items.html';
    }
            ...
            if ($this->MEMBER['group']['priv']['allow_msite'] < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($article_id = K::M('weixin/msite/cate')->create($data)) {
                        $this->err->add('添加分类成功');
                        $this->err->set_data('forward', $this->mklink('jtyjy/weixin/msite/cate:index'));
                    }
                }
        } else {
            $this->tmpl = 'jtyjy/weixin/msite/cate/create.html';
        }
    }
    ...
    public function edit($cat_id = null)
    {
        ...
        if (! ($cat_id = (int) $cat_id) && ! ($cat_id = (int) $this->GP('cat_id'))) {
            ...
            if (! $detail = K::M('weixin/msite/cate')->detail($cat_id)) {
                ...
                if ($weixin['wx_id'] != $detail['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'jtyjy/weixin/msite/cate/edit.html';
                    }
    }
