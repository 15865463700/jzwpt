...
class Ctl_Jtyjy_Weixin_Msite_Article extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $pager['page'] = $limit = 30;
        $pager['page'] = $count = 0;
        if ($items = K::M('weixin/msite/article')->items(array(
            'wx_id' => $weixin['wx_id']
        ...
        $this->tmpl = 'jtyjy/weixin/msite/article/items.html';
    }
            ...
            if ($this->MEMBER['group']['allow_msite'] < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($article_id = K::M('weixin/msite/article')->create($data)) {
                        $this->err->add('添加文章成功');
                        $this->err->set_data('forward', $this->mklink('jtyjy/weixin/msite/article:index'));
                    }
                }
        } else {
            $this->pagedata['cate_list'] = $cate_list;
            $this->tmpl = 'jtyjy/weixin/msite/article/create.html';
        }
    }
    ...
    public function edit($article_id = null)
    {
        ...
        if (! ($article_id = (int) $article_id) && ! ($article_id = (int) $this->GP('article_id'))) {
            ...
            if (! $detail = K::M('weixin/msite/article')->detail($article_id)) {
                $this->err->add('您要修改的内容不存在或已经删除', 212);
            } else 
                if ($weixin['wx_id'] != $detail['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'jtyjy/weixin/msite/article/edit.html';
                    }
    }
