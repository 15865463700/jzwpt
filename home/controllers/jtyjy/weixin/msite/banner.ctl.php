...
class Ctl_Jtyjy_Weixin_Msite_Banner extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'jtyjy/weixin/msite/banners.html';
    }
        ...
        if ($this->MEMBER['group']['priv']['allow_msite'] < 1) {
            ...
            if (! $attach = $_FILES['Filedata']) {
                ...
                if (UPLOAD_ERR_OK != $attach['error']) {
                    ...
                    if (K::M('weixin/msite/banner')->count(array(
                        'wx_id' => $weixin['wx_id']
                    )) > 5) {
    ...
    public function delete($photo_id = null)
    {
        ...
        if (! $photo_id = (int) $photo_id) {
            ...
            if (! $banner = K::M('weixin/msite/banner')->detail($photo_id)) {
                $this->err->add('您要删除的内容不存在或已经删除', 212);
            } else 
                if ($banner['wx_id'] != $weixin['wx_id']) {
