...
class Ctl_Jtyjy_Member_Yuyue extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function tenders($page = 1)
    {
        ...
        if ($items = K::M('tenders/tenders')->items($filter, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('yuyue:tenders', array(
                '{page}'
            )));
            $this->pagedata['items'] = $items;
        }
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/yuyue/tenders.html';
    }
        ...
        if (! $tenders_id = (int) $tenders_id) {
            ...
            if (! $detail = K::M('tenders/tenders')->detail($tenders_id)) {
                ...
                if ($detail['uid'] != $this->uid) {
                    ...
                    if ($look_list = K::M('tenders/look')->items_by_tenders($tenders_id)) {
                        ...
                        if ($uids) {
                            if ($member_list = K::M('member/member')->items_by_ids($uids)) {
                                ...
                                if ($company_uids) {
                                    $this->pagedata['company_list'] = K::M('jtyjycompany/company')->items_by_uids($company_uids);
                                }
                                if ($shop_uids) {
                    ...
                    $this->tmpl = 'jtyjy/member/yuyue/tendersDetail.html';
                }
    }
            ...
            if (! $detail = K::M('tenders/tenders')->detail($tenders_id)) {
                ...
                if ($detail['uid'] != $this->uid) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->pagedata['cates'] = K::M('tenders/cate')->items(array(
                            'closed' => '0',
                            'audit' => '1'
                        ));
                        $this->pagedata['detail'] = $detail;
                        $this->tmpl = 'jtyjy/member/yuyue/tendersEdit.html';
                    }
    }
                    ...
                    if ($tenders['uid'] != $this->uid) {
                        ...
                        if (empty($tenders['audit'])) {
                            ...
                            if ($tenders['sign_uid']) {
                                ...
                                if (! $member = K::M('member/member')->detail($look['uid'])) {
                                    $this->err->add('该投标用户不存在', 216);
                                } else 
                                    if (K::M('tenders/look')->sign($look_id)) {
                                        switch ($member['from']) {
                                            case 'gz':
                                                K::M('gz/gz')->update_count($look['uid'], 'tenders_sign');
                                                break;
                                            case 'designer':
                                                K::M('designer/designer')->update_count($look['uid'], 'tenders_sign');
                                                break;
                                            case 'mechanic':
                                                K::M('mechanic/mechanic')->update_count($look['uid'], 'tenders_sign');
                                                break;
                                            case 'company':
                                                $company = K::M('jtyjycompany/company')->items(array(
                                                    'uid' => $look['uid']
                                                ));
                                                foreach ($company as $k => $v) {
                                                    $this->company['company_id'] = $v['company_id'];
                                                }
                                                K::M('jtyjycompany/company')->update_count($this->company['company_id'], 'tenders_sign');
                                            case 'shop':
                                                $shop = K::M('shop/shop')->items(array(
                                                    'uid' => $look['uid']
                                                ));
                                                foreach ($shop as $k => $v) {
                                                    $this->company['shop_id'] = $v['shop_id'];
                                                }
                                                K::M('shop/shop')->update_count($this->shop['shop_id'], 'tenders_sign');
                                                break;
                                        }
                                        if ($zxb_id = (int) $tenders['zxb_id']) {
                                            if (! $company = K::M('jtyjycompany/company')->company_by_uid($look['uid'])) {
                                                $this->err->add('只家庭影剧院公司才能参加装修保', 216);
                                            } else {
    ...
    public function company($page = 1)
    {
        ...
        $pager['count'] = $count = 0;
        $filter['uid'] = $this->uid;
        if ($items = K::M('jtyjycompany/yuyue')->items($filter, $page, $limit, $count)) {
            $pager['count'] = $count;
            ...
            if ($company_ids) {
                $this->pagedata['company_list'] = K::M('jtyjycompany/company')->items_by_ids($company_ids);
            }
            $this->pagedata['items'] = $items;
        }
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/yuyue/company.html';
    }
    ...
    public function companyDetail($yuyue_id = null)
    {
        if (! $yuyue_id = (int) $yuyue_id) {
            ...
            if (! $detail = K::M('jtyjycompany/yuyue')->detail($yuyue_id)) {
                ...
                if ($detail['uid'] != $this->uid) {
                    ...
                    if ($detail['company_id']) {
                        $this->pagedata['company'] = K::M('jtyjycompany/company')->detail($detail['company_id']);
                    }
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'jtyjy/member/yuyue/companyDetail.html';
                }
    }
    ...
    public function designer($page = 1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/yuyue/designer.html';
    }
    ...
    public function designerDetail($yuyue_id = null)
    {
        if (! $yuyue_id = (int) $yuyue_id) {
            ...
            if (! $detail = K::M('designer/yuyue')->detail($yuyue_id)) {
                $this->err->add('您要查看的预约不存在或已经删除', 212);
            } else 
                if ($detail['uid'] != $this->uid) {
                    $this->err->add('您没有权限查看该预约', 213);
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'jtyjy/member/yuyue/designerDetail.html';
                }
    }
    ...
    public function gz($page = 1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/yuyue/gz.html';
    }
    ...
    public function gzDetail($yuyue_id = null)
    {
        if (! $yuyue_id = (int) $yuyue_id) {
            ...
            if (! $detail = K::M('gz/yuyue')->detail($yuyue_id)) {
                $this->err->add('您要查看的预约不存在或已经删除', 212);
            } else 
                if ($detail['uid'] != $this->uid) {
                    $this->err->add('您没有权限查看该预约', 213);
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'jtyjy/member/yuyue/gzDetail.html';
                }
    }
    ...
    public function mechanic($page = 1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/yuyue/mechanic.html';
    }
    ...
    public function mechanicDetail($yuyue_id = null)
    {
        if (! $yuyue_id = (int) $yuyue_id) {
            ...
            if (! $detail = K::M('mechanic/yuyue')->detail($yuyue_id)) {
                $this->err->add('您要查看的预约不存在或已经删除', 212);
            } else 
                if ($detail['uid'] != $this->uid) {
                    $this->err->add('您没有权限查看该预约', 213);
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'jtyjy/member/yuyue/mechanicDetail.html';
                }
    }
    ...
    public function shop($page = 1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/yuyue/shop.html';
    }
    ...
    public function shopDetail($yuyue_id = null)
    {
        if (! $yuyue_id = (int) $yuyue_id) {
            ...
            if (! $detail = K::M('shop/yuyue')->detail($yuyue_id)) {
                $this->err->add('您要查看的预约不存在或已经删除', 212);
            } else 
                if ($detail['uid'] != $this->uid) {
                    $this->err->add('您没有权限查看该预约', 213);
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'jtyjy/member/yuyue/shopDetail.html';
                }
    }
}