 ...
 * $Id$
 */
class Ctl_Jtyjy_Product extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'jtyjy/product/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($data['sale_sku'] > $data['store'] && $data['sale_type'] == 1) {
                        ...
                        if ($product_id = K::M('product/product')->create($data)) {
                            ...
                            $this->err->set_data('forward', $this->mklink('jtyjy/product'));
                            $this->err->add('添加商品成功');
                        }
                    }
        } else 
            if ($cat_id = (int) $cat_id) {
                if ($cate = K::M('shop/cate')->cate($cat_id)) {
                    ...
                    $this->tmpl = 'jtyjy/product/create.html';
                } else {
                    $this->err->add('选择的分类不存在或已经删除', 211);
                    $this->err->set_data('forward', $this->mklink(null, array(
                        $product_id
                    )));
                }
            } else {
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'jtyjy/product/select_cate.html';
            }
    }
    ...
    public function edit($product_id = null, $cat_id = null)
    {
        ...
        if (! ($product_id = (int) $product_id) && ! ($product_id = (int) $this->GP('product_id'))) {
            ...
            if (! $detail = K::M('product/product')->detail($product_id)) {
                ...
                if ($detail['shop_id'] != $shop['shop_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        if ($cat_id == 'edit') {
                            ...
                            $this->pagedata['pager'] = $pager;
                            $this->tmpl = 'jtyjy/product/edit.html';
                        }
    }
        ...
        if (! $product_id = (int) $this->GP('product_id')) {
            ...
            if (! $product = K::M('product/product')->detail($product_id)) {
                ...
                if ($product['shop_id'] != $shop['shop_id']) {
                    ...
                    if (K::M('product/photo')->count_by_product($product_id) >= 5) {
        ...
        if (! ($product_id = (int) $product_id) && ! ($product_id = (int) $this->GP('product_id'))) {
            ...
            if (! $product = K::M('product/product')->detail($product_id)) {
                ...
                if ($product['shop_id'] != $shop['shop_id']) {
                    ...
                    $this->tmpl = 'jtyjy/product/spec.html';
                }
    }
            ...
            if (! $product = K::M('product/product')->detail($product_id)) {
                ...
                if ($product['shop_id'] != $shop['shop_id']) {
                    ...
                    if ($this->checksubmit()) {
                        ...
                        $items = K::M('product/spec')->items(array(
                        ...
                        if ($items) {
                            ...
                            K::M('product/product')->update($product_id, array(
                                'store' => $sku
                            ));
                        }
    ...
    public function deletespec($spec_id = null)
    {
        ...
        if (! $spec_id = (int) $spec_id) {
            ...
            if (! $spec = K::M('product/spec')->detail($spec_id)) {
                $this->err->add('规格不存在或已经删除', 212);
            } else 
                if (! $product = K::M('product/product')->detail($spec['product_id'])) {
                    ...
                    if (K::M('product/spec')->delete($spec_id)) {
                        ...
                        if ($items) {
                            ...
                            K::M('product/product')->update($product['product_id'], array(
                                'store' => $sku
                            ));
                        }
                        $this->err->add('规格删除成功');
        ...
        if (! ($product_id = (int) $product_id) && ! ($product_id = (int) $this->GP('product_id'))) {
            ...
            if (! $product = K::M('product/product')->detail($product_id)) {
                ...
                if ($product['shop_id'] != $shop['shop_id']) {
                    ...
                    $this->pagedata['pager'] = $pager;
                    $this->pagedata['product'] = $product;
                    $this->tmpl = 'jtyjy/product/photo.html';
                }
    }
                            ...
                            foreach ($items as $k => $v) {
                                if ($a = $data[$k]) {
                                    if ($a['title'] != $v['title'] || $a['orderby'] != $v['orderby']) {
                                        K::M('product/photo')->update($k, array(
                                            'title' => $a['title'],
                                            'orderby' => $a['orderby']
                                        ));
                                    }
    ...
    public function deletephoto($photo_id = null)
    {
        ...
        if (! $photo_id = (int) $photo_id) {
            ...
            if (! $photo = K::M('product/photo')->detail($photo_id)) {
                $this->err->add('图片不存在或已经删除', 212);
            } else 
                if (! $product = K::M('product/product')->detail($photo['product_id'])) {
                    $this->err->add('你没有权限删除', 213);
                } else 
