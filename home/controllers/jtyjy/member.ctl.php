...
// class Ctl_Jtyjy_Member extends Ctl_Jtyjy_Jiatingyingjuyuan
class Ctl_Jtyjy_Member extends Ctl
{
    ...
    public function __construct(&$system)
    {
        ...
        $this->pagedata['ctlgroup'] = "member";
    }

    public function index()
    {
        $this->pagedata['order_count'] = K::M('trade/order')->count_by_uid($this->uid);
        $this->pagedata['yuyue_company_count'] = K::M('jtyjycompany/yuyue')->count(array(
            'uid' => $this->uid
        ));
        ...
        $items['yuyue_company_count'] = K::M('jtyjycompany/yuyue')->items(array(
            'uid' => $this->uid
        ));
        ...
        // die(var_dump($this->pagedata['menu_list'][0]['items'][0]['nav']));
        $this->tmpl = 'jtyjy/member/index.html';
    }
    ...
    public function info()
    {
        if ($account = $this->checksubmit('account')) {
            ...
            $this->tmpl = 'jtyjy/member/info.html';
        }
    }
    ...
    public function passwd()
    {
        if ($account = $this->checksubmit('account')) {
            if (md5($account['old_passwd']) != $this->MEMBER['passwd']) {
                ...
                if ($account['passwd'] != $account['confirm_passwd']) {
                    $this->err->add('两次输入的密码不相同', 212);
                } else 
                    if (K::M('member/account')->check_passwd($account['passwd'])) {
                        if ($this->auth->update_passwd($account['passwd'], false)) {
                            $this->err->add('修改密码成功');
                        }
                    }
        } else {
            $this->tmpl = 'jtyjy/member/passwd.html';
        }
    }

    public function passwds()
    {
        $this->tmpl = 'jtyjy/member/passwds.html';
    }
    ...
    public function mail()
    {
        if ($account = $this->checksubmit('account')) {
            if ($this->MEMBER['verify_mail'] && (md5($account['passwd']) != $this->MEMBER['passwd'])) {
                ...
                if (K::M('member/account')->check_mail($account['newmail'])) {
                    if ($this->auth->update_mail($account['newmail'])) {
                        $this->err->add('更换邮箱成功');
                    }
                }
        } else {
            $this->tmpl = 'jtyjy/member/mail.html';
        }
    }

    public function face()
    {
        $this->tmpl = 'jtyjy/member/face.html';
    }
    ...
    public function upload()
    {
        if (! $data = file_get_contents("php://input")) {
            ...
            if (K::M('member/face')->update_face($this->uid, null, $data)) {
    ...
    public function bindaccount()
    {
        ...
        $bind_list = K::M('connect/connect')->items(array(
            'uid' => $this->uid
        ), null, 1, 10);
        ...
        $this->tmpl = 'jtyjy/member/bindaccount.html';
    }
    ...
    public function gold()
    {
        ...
        $this->tmpl = 'jtyjy/member/gold.html';
    }
    ...
    public function truste($truste, $truste_id)
    {
        if ($truste < 1) {
            ...
            $this->pagedata['pay_list'] = K::M('payment/payment')->fetch_all();
            $this->tmpl = 'jtyjy/member/truste.html';
        }
    }
    ...
    public function logs($type = null, $page = 0)
    {
        ...
        if ($items = K::M('member/log')->items($filter, null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('jtyjy/member:logs', array(
                '{page}'
            )));
            $this->pagedata['items'] = $items;
        }
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/logs.html';
    }
    ...
    public function jflogs($type = null, $page = 0)
    {
        ...
        if ($items = K::M('fenxiao/log')->items($filter, null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('jtyjy/member:jflogs', array(
                '{page}'
            )));
            $this->pagedata['items'] = $items;
        }
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/jflogs.html';
    }
    ...
    public function home($uid)
    {
        ...
        // $this->error(404);
        // }
        $url = $this->mklink('jtyjy:detail', $this->uid);
        header("Location:{$url}");
        exit();
    }
    ...
    public function fenxiao()
    {
        ...
        $this->tmpl = 'jtyjy/member/fenxiao.html';
    }
    ...
    public function topics()
    {
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/member/topics/index.html';
    }
        ...
        if (! $topics_id = (int) $topics_id) {
            ...
            if (! $detail = K::M('topics/topics')->detail($topics_id)) {
                ...
                if ($detail['uid'] != $this->uid) {
                    ...
                    $this->tmpl = 'jtyjy/member/topics/detail.html';
                }
    }
}