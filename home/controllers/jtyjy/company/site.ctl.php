...
// class Ctl_Jtyjy_Company_Site extends Ctl_Jtyjy_Jiatingyingjuyuan
class Ctl_Jtyjy_Company_Site extends Ctl
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'jtyjy/company/site/items.html';
    }
                    ...
                    if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                        ...
                        if ($site_id = K::M('home/site')->create($data)) {
                            if ($company_id = (int) $data['company_id']) {
                                K::M('jtyjycompany/company')->update_count($company_id, 'site_num', 1);
                            }
                            if ($home_id = (int) $data['home_id']) {
                            ...
                            $this->err->add('添加工地成功');
                            $this->err->set_data('forward', $this->mklink('jtyjy/company/site:index'));
                        }
                    }
            } else {
                $pager['city_id'] = $company['city_id'];
                $this->pagedata['items'] = $this->getZxpm();
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'jtyjy/company/site/create.html';
            }
    }
    ...
    public function edit($site_id = null)
    {
        ...
        if (! ($site_id = (int) $site_id) && ! ($site_id = (int) $this->GP('site_id'))) {
            ...
            if (! $detail = K::M('home/site')->detail($site_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $pager['city_id'] = $company['city_id'];
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'jtyjy/company/site/edit.html';
                }
    }
    
    // 获取项目监理列表
    private function getZxpm()
    {
        $company = $this->ucenter_company();
        $filter['company_id'] = $company['company_id'];
        return K::M('jtyjycompany/zxpm')->items($filter);
    }
        ...
        if (! ($site_id = (int) $site_id) && ! ($site_id = $this->GP('site_id'))) {
            ...
            if (! $detail = K::M('home/site')->detail($site_id)) {
                ...
                if ($company['company_id'] != $detail['company_id']) {
                    ...
                    K::M('jtyjycompany/company')->update_count($company['company_id'], 'site_num', - 1);
    ...
    public function detail($site_id = null)
    {
        ...
        if (! $site_id = (int) $site_id) {
            ...
            if (! $site = K::M('home/site')->detail($site_id)) {
                ...
                if ($items = K::M('home/items')->items(array(
                    'site_id' => $site_id
                ))) {
                    $this->pagedata['items'] = $items;
                }
                ...
                $this->tmpl = 'jtyjy/company/site/diary/site.html';
            }
    }
}