...
// class Ctl_Jtyjy_Company_Case extends Ctl_Jtyjy_Jiatingyingjuyuan
class Ctl_Jtyjy_Company_Case extends Ctl
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'jtyjy/company/case/items.html';
    }
        ...
        if (K::M('member/integral')->check('case', $this->MEMBER) === false) {
            ...
            if ($data = $this->checksubmit('data')) {
                if (! $data = $this->check_fields($data, $this->_allower_fields)) {
                    ...
                    if ($case_id = K::M('jtyjycase/case')->create($data)) {
                        if (! $attr = $this->GP('attr')) {
                            $attr = array();
                        }
                        K::M('jtyjycase/attr')->update($case_id, $attr);
                        K::M('jtyjycompany/company')->update_count($company_id, 'case_num', 1);
                        ...
                        $this->err->add('添加案例成功');
                        $this->err->set_data('forward', $this->mklink('jtyjy/company/case:detail', array(
                            $case_id
                        ), array(), true));
                    }
                }
                ...
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'jtyjy/company/case/create.html';
            }
    }
    ...
    public function edit($case_id = null)
    {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('jtyjycase/case')->detail($case_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    if (! $data = $this->check_fields($data, $this->_allower_fields)) {
                        ...
                        if (K::M('jtyjycase/case')->update($case_id, $data)) {
                    ...
                    if ($attrs = K::M('jtyjycase/attr')->attrs_by_case($case_id)) {
                    ...
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'jtyjy/company/case/edit.html';
                }
    }
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('jtyjycase/case')->detail($case_id)) {
                ...
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    $pager['count'] = $count = 0;
                    $this->pagedata['detail'] = $detail;
                    if ($items = K::M('jtyjycase/photo')->items_by_case($case_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("jtyjy/company/case:detail", array(
                            $case_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'jtyjy/company/case/detail.html';
                }
    }
                    ...
                    if (! $case = K::M('jtyjycase/case')->detail($case_id)) {
                        ...
                        if (! $attach = $_FILES['Filedata']) {
                            ...
                            if (UPLOAD_ERR_OK != $attach['error']) {
                                ...
                                if ($data = K::M('jtyjycase/photo')->upload($case_id, $attach)) {
                                    // 发布案例第一张图的时候才判断审核！
                                    if ($allow_case != $case['audit'] && empty($case['photos'])) {
                                        K::M('jtyjycase/case')->update($case_id, array(
                                            'audit' => $allow_case
                                        ));
                                        K::M('member/integral')->commit('case', $this->MEMBER, '发布案例');
                                    }
            ...
            if (! $detail = K::M('jtyjycase/case')->detail($case_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    if (empty($photo_ids)) {
                        ...
                        if (! $photoinfos = K::M('jtyjycase/photo')->items_by_ids($photo_ids)) {
                            $this->err->add('没有您要更新的内容', 212);
                        } else {
                            $obj = K::M('jtyjycase/photo');
                            foreach ($data as $k => $v) {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $case = K::M('jtyjycase/case')->detail($case_id)) {
                ...
                if ($case['company_id'] != $company['company_id']) {
                    ...
                    if (K::M('jtyjycase/case')->delete($case_id)) {
                        K::M('jtyjycompany/company')->update_count($company['company_id'], 'case_num', - 1);
                        $this->err->add('删除案例成功');
                    }
    }
    ...
    public function defaultphoto($photo_id = null)
    {
        ...
        if (! ($photo_id = (int) $photo_id) && ! ($photo_id = (int) $this->GP('photo_id'))) {
            ...
            if (! $detail = K::M('jtyjycase/photo')->detail($photo_id)) {
                $this->err->add('效果图不存在或已经删除', 211);
            } else 
                if (! $case = K::M('jtyjycase/case')->detail($detail['case_id'])) {
                    $this->err->add('案例不存在或已经删除', 212);
                } else 
                    if ($case['company_id'] != $company['company_id']) {
                        $this->err->add('不许越权管理别人的内容', 213);
                    } else {
                        if (K::M('jtyjycase/case')->update($detail['case_id'], array(
                            'photo' => $detail['photo']
                        ))) {
                            $this->err->add('修改封面成功');
                        }
            ...
            if (! $detail = K::M('jtyjycase/photo')->detail($photo_id)) {
                ...
                if (! $case = K::M('jtyjycase/case')->detail($detail['case_id'])) {
                    ...
                    if ($case['company_id'] != $company['company_id']) {
                        ...
                        if (K::M('jtyjycase/photo')->delete($photo_id)) {
