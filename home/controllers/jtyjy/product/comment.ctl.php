...
class Ctl_Jtyjy_Product_Comment extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function shop($page = 1)
    {
        ...
        $this->tmpl = 'jtyjy/product/comment/items.html';
    }
    ...
    public function detail($comment_id = null)
    {
        ...
        if (! ($comment_id = (int) $comment_id) && ! ($comment_id = (int) $this->GP('comment_id'))) {
            ...
            if (! $detail = K::M('product/comment')->detail($comment_id)) {
                ...
                if ($shop['shop_id'] != $detail['shop_id']) {
                    ...
                    $this->tmpl = 'jtyjy/product/comment/detail.html';
                }
    }
    ...
    public function reply($comment_id = null)
    {
        ...
        if (! ($comment_id = (int) $comment_id) && ! ($comment_id = (int) $this->GP('comment_id'))) {
            ...
            if (! $comment = K::M('product/comment')->detail($comment_id)) {
                $this->err->add('评论不存在或已经删除', 212);
            } else 
                if (K::M('member/group')->check_priv($shop['group_id'], 'allow_reply') < 0) {
                    ...
                    if ($shop['shop_id'] != $comment['shop_id']) {
                        ...
                        if ($comment['replytime'] && $comment['reply']) {
                            ...
                            if ($this->checksubmit()) {
                                ...
                                $this->tmpl = 'jtyjy/shop/comment/reply.html';
                            }
    }
}