...
class Ctl_Jtyjy_Misc_Tenders extends Ctl_Jtyjy_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'jtyjy/misc/tenders/items.html';
    }
        ...
        if (! $tenders_id = (int) $tenders_id) {
            ...
            if (! $detail = K::M('tenders/tenders')->detail($tenders_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    K::M('tenders/tenders')->update_count($tenders_id, 'views');
                    $this->tmpl = 'jtyjy/misc/tenders/detail.html';
                }
    }
    ...
    public function looked($page = 1)
    {
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'jtyjy/misc/tenders/looks.html';
    }
    ...
    public function track($look_id = null, $page = 1)
    {
        if (! $look_id = (int) $look_id) {
            ...
            if (! $look = K::M('tenders/look')->detail($look_id)) {
                ...
                if (! $detail = K::M('tenders/tenders')->detail($look['tenders_id'])) {
                    ...
                    if (empty($detail['audit'])) {
                        ...
                        $pager['limit'] = $limit = 10;
                        $pager['count'] = $count = 0;
                        if ($track_list = K::M('tenders/track')->items(array(
                            'look_id' => $look_id
                        ), array(
                            'track_id' => 'DESC'
                        ), $page, $limit, $count)) {
                            $pager['count'] = $count;
                            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $count, $this->mklink(null, array(
                                $look_id,
                                '{page}'
                            )));
                        ...
                        $this->pagedata['pager'] = $pager;
                        $this->tmpl = 'jtyjy/misc/tenders/track.html';
                    }
    }
    ...
    public function look($tenders_id = null)
    {
        if (! ($tenders_id = (int) $tenders_id) && ! ($tenders_id = (int) $this->GP('tenders_id'))) {
            ...
            if (! $tenders = K::M('tenders/tenders')->detail($tenders_id)) {
                ...
                if (($tenders_look = K::M('member/group')->check_priv($this->MEMBER['group_id'], 'tenders_look')) < 0) {
                        ...
                        if ($tenders['status'] < 0) {
                            ...
                            if ($tenders['sign_uid']) {
                                ...
                                if ($tenders['looks'] >= $tenders['max_look']) {
                                    ...
                                    if (K::M('tenders/look')->is_looked($tenders_id, $this->uid)) {
                                        $this->err->add('您已经投过标了，不需要重复投标', 213);
                                    } else 
                                        if (($tenders['gold']) && ($this->MEMBER['gold'] < $tenders['gold'])) {
                                            ...
                                            if ($data = $this->checksubmit('data')) {
                                                    ...
                                                    if ($look_id = K::M('tenders/look')->create($datas)) {
                                                        ...
                                                        switch ($this->MEMBER['from']) {
                                                            ...
                                                            case 'mechanic':
                                                                ...
                                                                break;
                                                            case 'company':
                                                                K::M('jtyjycompany/company')->update_count($this->company['company_id'], 'tenders_num');
                                                                break;
                                                            case 'shop':
                                                ...
                                                $this->tmpl = 'jtyjy/misc/tenders/look.html';
                                            }
    }
}