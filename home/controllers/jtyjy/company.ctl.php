...
// class Ctl_Jtyjy_Company extends Ctl_Jtyjy_Jiatingyingjuyuan{
// }
class Ctl_Jtyjy_Company extends Ctl
{
    ...
    public function index()
    {
        ...
        // $this->pagedata['youhui_count'] = K::M('jtyjycompany/youhui')->count(array('company_id'=>$company['company_id']));
        // $this->pagedata['youhui_sign_count'] = K::M('jtyjycompany/sign')->count(array('company_id'=>$company['company_id']));
        //
        // $items['company_yuyue'] = K::M('jtyjycompany/yuyue')->items(array('company_id'=>$company['company_id']));
        // $items['company_sign'] = K::M('jtyjycompany/sign')->items(array('company_id'=>$company['company_id']));
        // $items['company_comment'] = K::M('jtyjycompany/comment')->items(array('company_id'=>$company['company_id']));
        //
        //
        ...
        // $items2['company_youhui'] = K::M('jtyjycompany/youhui')->items(array('company_id'=>$company['company_id']));
        //
        // $this->pagedata['data'] = $this->get_data($items);
        // $this->pagedata['data2'] = $this->get_data2($items2);
        $this->tmpl = 'jtyjy/company/index.html';
    }
    ...
    public function refresh()
    {
        ...
        if ($this->GP('fromid')) {
            ...
            if ($isrefresh && K::M('member/flush')->create($data)) {
                K::M('jtyjycompany/company')->update($company['company_id'], array(
                    'flushtime' => __TIME
                ));
                $this->err->add('刷新成功');
            }
            ...
            $this->tmpl = 'jtyjy/company/refresh/look.html';
        }
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                ...
                if (! $company['company_id']) {
                    ...
                    if ($company_id = K::M('jtyjycompany/company')->create($data)) {
                        K::M('member/member')->update($this->uid, array(
                            'group_id' => $data['group_id']
                        ));
                        if ($attr = $this->GP('attr')) {
                            K::M('jtyjycompany/attr')->update($company_id, $attr);
                        }
                        if ($fields = $this->GP('fields')) {
                            if ($fields = $this->check_fields($fields, array(
                                'info'
                            ))) {
                                K::M('jtyjycompany/fields')->update($company_id, $fields);
                            }
                        }
                        $this->err->add('设置公司资料成功');
                    }
                } else 
                    if (K::M('jtyjycompany/company')->update($company['company_id'], $data)) {
                        if ($attr = $this->GP('attr')) {
                            K::M('jtyjycompany/attr')->update($company['company_id'], $attr);
                        }
                        if ($fields = $this->GP('fields')) {
                            if ($fields = $this->check_fields($fields, array(
                                'info'
                            ))) {
                                K::M('jtyjycompany/fields')->update($company['company_id'], $fields);
                            }
                        }
                        $this->err->add('设置公司资料成功');
                    }
            ...
            // if($attrs = K::M('jtyjycompany/attr')->attrs_by_company($company['company_id'])){
            ...
            if ($domain = $this->checksubmit('domain')) {
                if (! $company['group']['priv']['allow_domain']) {
                    ...
                    if ($company['domain']) {
                        ...
                        if (K::M('jtyjycompany/company')->update_domain($company['company_id'], $domain)) {
                            $this->err->add('设置个性域名成功');
                        }
            } else {
                $this->pagedata['domain_company'] = $CFG['domain']['company'] . '.' . $CFG['site']['domain'];
                $this->tmpl = 'jtyjy/company/domain.html';
            }
    }
        ...
        if ($skin = $this->checksubmit('skin')) {
            if ($audit_skin < 0) {
                ...
                if (! $cfg = $skins[$skin]) {
                    ...
                    if (K::M('jtyjycompany/fields')->update($company['company_id'], array(
                        'skin' => $skin
                    ), true)) {
            ...
            $this->tmpl = 'jtyjy/company/skin.html';
        }
    }
}