...
class Ctl_Blog extends Ctl
{
    ...
    public function __construct(&$system)
    {
        ...
        if (preg_match('/blog-(\d+)(\.html)?/i', $this->request['uri'], $m)||preg_match('/blog\/(\d+)(\.html)?/i', $this->request['uri'], $m)) {
    ...
    public function detail($uid)
    {
        ...
        $comment_info = K::M("designer/comment")->items(array(
            'designer_id' => $uid,
            'closed' => 0
        ), array(
            'comment_id' => 'desc'
        ...
        $this->pagedata['mobile_url'] = $this->mklink('mobile/designer', array(
            $uid
        ));
        $this->pagedata['comment_list'] = $comment_info;
        $this->pagedata['designer'] = $designer;
         $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/blog/{$uid}.html";
    ...
    public function about($uid)
    {
        $designer = $this->check_designer($uid);
        $this->pagedata['company'] = K::M('company/company')->detail($designer['company_id']);
         $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/blog/about-{$uid}.html";
    ...
    public function cases($uid, $page = 1)
    {
        ...
        if ($items = K::M('case/case')->items($filter, null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('blog:cases', array(
                $uid,
                '{page}'
            )));
            $this->pagedata['items'] = $items;
        }
         $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/blog/cases-{$uid}.html";
    ...
    public function article($uid, $page = 1)
    {
        ...
        if ($items = K::M("designer/article")->items($filter, null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('blog:article', array(
                $uid,
                '{page}'
            )));
            $this->pagedata['items'] = $items;
        }
         $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/blog/article-{$uid}.html";
        $this->pagedata['pager'] = $pager;
    ...
    public function comments($uid, $page = 1)
    {
        ...
        if ($items = K::M("designer/comment")->items($filter, $order, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('blog:comment', array(
                $uid,
                '{page}'
            )));
            $this->pagedata['items'] = $items;
         ...
         $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/blog/comments-{$uid}.html";
    ...
    protected function check_designer($uid)
    {
        ...
        $seo = array(
            ...
            'uname' => $designer['name'],
            'designer_school' => $designer['school'],
            'designer_slogan' => $designer['slogan'],
            'designer_desc' => ''
        );
