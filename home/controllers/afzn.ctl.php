...
class Ctl_Afzn extends Ctl{
    ...
    public function items($page = 1){
        ...
        $attr_values = K::M('data/attr')->attrs_by_from('zx:afzn', true);
        $uri = $this->request['uri'];
        if(preg_match('/items(-[\d\-]+)?(-(\d+)).html/i', $uri, $m)||preg_match('/afzn(-[\d\-]+)?(-(\d+))(\.html)*/i', $uri, $m)) {
            $page = (int)$m[3];
            if($m[1]){
            ...
            if($v['filter']){
                ...
                foreach($attr_vids as $vv){
                    if($v['values'][$vv]){
                        ...
                        $_seo_repeat.=",{$this->request['city']['city_name']}{$v['values'][$vv]['title']}安防智能公司";
                    }
                }
            }
        }
        ...
        foreach($attr_values as $k=>$v){
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('afzn:items', array($area_id, $group_id, implode('-', $vids)));

            $v['link']=preg_replace("/-items/","",$v['link']);
            $v['link']=preg_replace("/afzn-0-0-0-0-0-1\./","afzn.",$v['link']);
            ...
            foreach($v['values'] as $kk=>$vv){
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('afzn:items', array($area_id, $group_id, implode('-', $vids)));
                $vv['link']=preg_replace("/-items/","",$vv['link']);
                $vv['link']=preg_replace("/afzn-0-0-0-0-0-1\./","afzn.",$vv['link']);
        ...
        $area_all_link = $this->mklink('afzn:items', array(
            0,
            $group_id,
        ...
        $area_all_link=preg_replace("/afzn-0-0-0-0-0-1\./","afzn.",$area_all_link);
        $area_all_link=preg_replace("/\.html/","/",$area_all_link);
        foreach ($area_list as $k => $v) {
            $v['link'] = $this->mklink('afzn:items', array(
                $k,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
        ...
        $order_list[0]['link'] = $this->mklink('afzn:items', array($area_id, $group_id, implode('-', $attr_value_ids), 0, 1));
        $order_list[1]['link'] = $this->mklink('afzn:items', array($area_id, $group_id, implode('-', $attr_value_ids), 1, 1));
        $order_list[2]['link'] = $this->mklink('afzn:items', array($area_id, $group_id, implode('-', $attr_value_ids), 2, 1));
        $pager = $filter = array();
        ...
        $items = K::M('afzncompany/company')->items($filter, $orderby, $page, $limit, $count);
        foreach ($items as $key => $value) {
            $fields = K::M('afzncompany/fields')->detail($value["company_id"]);
        ...
        if ($items) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('afzn:items', array(
                $area_id,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
            ...
            foreach($items as $k=>$v){
                $items[$k]['company_url']=preg_replace("/\/afzncompany-/","/afzncompany/",$v['company_url']);
                $items[$k]['company_url']=  str_replace(Ctl_Afzn::$ctl_afzncompany_old, Ctl_Afzn::$ctl_afzncompany_new, $items[$k]['company_url']);
        ...
        foreach($order_list as $k=>$v){
            ...
            $order_list[$k]['link']=preg_replace("/afzn-0-0-0-0-0-1\./","afzn.",$order_list[$k]['link']);
            $order_list[$k]['link']=preg_replace("/\.html/","/",$order_list[$k]['link']);
        }
        $this->pagedata['order_list'] = $order_list;
        foreach($area_all_link as $k=>$v){
            $area_all_link[$k]['link']=preg_replace("/-items/","",$v['link']);
            $area_all_link[$k]['link']=preg_replace("/afzn-0-0-0-0-0-1\./","afzn.",$area_all_link[$k]['link']);
            $area_all_link[$k]['link']=preg_replace("/\.html/","/",$area_all_link[$k]['link']);
        }
        ...
        if(is_main()){
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('afzn',$seo):$this->seo->init('afzn_attr',$seo);
        }else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_afzn',$seo):$this->seo->init('fenzhan_afzn_attr',$seo);
        }
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/afzn/";
        $this->tmpl = 'afzn/items.html';
    }
    ...
    public function result()
    {
        ...
        if (! empty($SO['lng_start']) && ! empty($SO['lng_end']) && ! empty($SO['lat_start']) && ! empty($SO['lat_end'])) {
            if (is_numeric($SO['lng_start']) && is_numeric($SO['lng_end']) && is_numeric($SO['lat_start']) && is_numeric($SO['lat_end'])) {
                ...
                $filter['closed'] = 0;
                $items = K::M('afzncompany/company')->items($filter, null, 1, 100, $count);
                $data = array();
    ...
    public function yuyue($company_id)
    {
        if (! ($company_id = (int) $company_id) && ! ($company_id = (int) $this->GP('company_id'))) {
            ...
            if (! $detail = K::M('afzncompany/company')->detail($company_id)) {
                $this->err->add('安防智能公司不存在或已经删除', 212);
            } else
                        ...
                        if (! $data = $this->GP('data')) {
                            ...
                            if ($verifycode_success) {
                                ...
                                if ($yuyue_id = K::M('afzncompany/yuyue')->create($data)) {
                                    K::M('afzncompany/yuyue')->yuyue_count($company_id);
                                    $this->err->add('预约安防智能公司成功！');
                                }
                            }
                        }
                    } else {
