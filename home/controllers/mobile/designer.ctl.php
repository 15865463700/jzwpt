...
class Ctl_Mobile_Designer extends Ctl_Mobile
{
    ...
    public function items($area_id = 0, $group_id = 0, $order = 0, $page = 1)
    {
        ...
        if ($items = K::M('designer/designer')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/designer:items', array(
                $area_id,
                $group_id,
                $order,
                '{page}'
    ...
    public function cases($uid, $page = 1)
    {
        ...
        $pager['limit'] = $limit = 5;
        if ($items = K::M('case/case')->items(array(
            'uid' => $uid,
            'audit' => 1,
            'closed' => 0
        ), null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/designer:cases', array(
                $uid,
                '{page}'
            )));
            $this->pagedata['items'] = $items;
        }
        $pager['backurl'] = $this->mklink('mobile/designer', array(
    ...
    public function article($uid, $page = 1)
    {
        ...
        $pager['page'] = max((int) $page, 1);
        $pager['limit'] = $limit = 10;
        if ($items = K::M('designer/article')->items(array(
            'uid' => $uid,
            'audit' => 1
        ), null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(array(
                'mobile/designer:article'
            ), array(
                $uid,
                '{page}'
            )));
                ...
                if (empty($designer['audit'])) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        if ($yuyue_id = K::M('designer/yuyue')->create($data)) {
                            ...
                            $this->err->set_data('forward', $this->mklink('mobile/designer:detail', array(
                                'uid' => $uid
                            )));
                        }
                    } else {
