...
class Ctl_Mobile_Zbdt extends Ctl_Mobile
{
    ...
    public function index()
    {
        ...
        if (preg_match('/index(-([\d\-]+)?)?-(\d+).html/i', $uri, $match)) {
            ...
            if (preg_match('/index-(\d+).html/i', $uri, $match)) {
                $page = $match[1];
            }
        ...
        foreach ($cate_list as $k => $v) {
            if ($v['parent_id'] == 0) {
                $v['link'] = $this->mklink('mobile/zbdt:index', array(
                    $k,
                    $status,
                    $order,
                    1
        ...
        if ($cate) {
            foreach ($cate_list as $k => $v) {
                if (empty($v['parent_id'])) {
                    ...
                    if ($top_cate['cat_id'] == $v['parent_id']) {
                        $v['link'] = $this->mklink('mobile/zbdt:index', array(
                            $k,
                            $status,
                            $order,
                        ...
                        if ($v['parent_id'] == $cate['parent_id'] || $v['parent_id'] == $cate['cat_id'] || $v['cat_id'] == $cate['cat_id']) {
                            $v['link'] = $this->mklink('mobile/zbdt:index', array(
                                $k,
                                $status,
                                $order,
        ...
        $cate_link = $this->mklink('mobile/zbdt:index', array(
            $top_cate['cat_id'],
            $status,
            $order,
            1
        ));
        $a_link = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            0,
            $order,
            1
        ));
        $b_link = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            1,
            $order,
            1
        ));
        $c_link = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            2,
            $order,
            1
        ));
        $order_alink = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            $status,
            0,
            1
        ));
        $order_blink = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            $status,
            1,
            1
        ));
        $order_clink = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            $status,
            2,
            1
        ...
        $price_link = $this->mklink('mobile/zbdt:index', array(
            $cat_id,
            $status,
            $order,
            1
        ...
        $all_parent_cat_link = $this->mklink('mobile/zbdt:index', array(
            0,
            $status,
            $order,
            1
        ...
        if ($items = K::M('truste/truste')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/zbdt:index', array(
                $cat_id,
                $status,
                $order,
                '{page}'
        ...
        $this->tmpl = 'mobile/zbdt/index.html';
    }
    ...
    public function detail($truste_id)
    {
        if (! $truste_id = (int) $truste_id) {
            ...
            if (! $detail = K::M('truste/truste')->detail($truste_id)) {
                ...
                if ($look_list = K::M('truste/look')->items(array(
                    'truste_id' => $truste_id
                ))) {
                    $uids = array();
                ...
                $this->seo->init('truste_detail', array(
                    'title' => $detail['title']
                ));
                $this->tmpl = 'mobile/zbdt/detail.html';
            }
    }
}