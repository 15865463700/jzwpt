...
class Ctl_Mobile_Shop extends Ctl_Mobile
{
    ...
    public function product($shop_id, $vcat_id = 0, $page = 1)
    {
        ...
        if ($items = K::M('product/product')->items($filter, null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/shop:product', array(
                $shop_id,
                $vcat_id,
                '{page}'
            )));
        ...
        $pager['backurl'] = $this->mklink('mobile/shop', array(
            $shop_id
        ));
        $this->pagedata['pager'] = $pager;
        $this->seo->set_shop($shop);
    ...
    public function coupon($shop_id, $page)
    {
        ...
        if ($items = K::M('shop/coupon')->items_by_shop($shop_id, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/shop:coupon', array(
                $shop_id,
                '{page}'
            )));
            $this->pagedata['items'] = $items;
    ...
    public function news($shop_id, $page)
    {
        ...
        if ($items = K::M('shop/news')->items_by_shop($shop_id, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/shop:news', array(
                $shop_id,
                '{page}'
            )));
            $this->pagedata['items'] = $items;
            ...
            if (! $detail = K::M('shop/news')->detail($news_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($shop = $this->check_shop($detail['shop_id'])) {
                        ...
                        $pager['backurl'] = $this->mklink('mobile/shop:news', array(
                            $detail['shop_id'],
                            1
                        ));
                        $this->pagedata['pager'] = $pager;
        ...
        if (! ($shop_id = (int) $shop_id) && ! ($shop_id = (int) $this->GP('shop_id'))) {
            ...
            if ($data = $this->checksubmit('data')) {
                ...
                if (K::M('shop/yuyue')->create($data)) {
                    ...
                    $this->err->set_data('forward', $this->mklink('mobile/shop', array(
                        $shop_id
                    )));
