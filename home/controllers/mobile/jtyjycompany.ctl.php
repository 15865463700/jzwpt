...
class Ctl_Mobile_Jtyjycompany extends Ctl_Mobile{

    public function __construct(&$system)
    {
        parent::__construct($system);
        $uri = $this->request['uri'];
        if (preg_match('/jtyjycompany-([\d]+).html/i', $uri, $match)) {
    ...
    public function items($area_id = 0, $group_id = 0, $order = 0, $page = 1)
    {
        ...
        $pager['count'] = $count = 0;
        if ($items = K::M('jtyjycompany/company')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/jtyjycompany:items', array(
                $area_id,
                $group_id,
                $order,
                '{page}'
        ...
        $this->tmpl = 'mobile/jtyjycompany/items.html';
    }

    public function detail($company_id)
    {
        $company = $this->check_jtyjycompany($company_id);
        $pager = array();
        $pager['backurl'] = '/mobile/rzzn/';
        $this->pagedata['pager'] = $pager;
        // $this->seo->set_company($company);
        // $this->seo->init('company');
        $this->tmpl = 'mobile/jtyjycompany/detail.html';
    }
    ...
    public function youhui($company_id, $page = 1)
    {
        ...
        $pager['page'] = $limit = 5;
        $pager['count'] = $count = 0;
        if ($items = K::M('jtyjycompany/youhui')->items_by_company($company_id, $page, $limit, $count)) {
            $pager['count'] = $count;
        ...
        $pager['backurl'] = $this->mklink('mobile/jtyjycompany', array(
            $company_id
        ));
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'mobile/jtyjycompany/youhui.html';
    }
    ...
    public function cases($company_id, $page = 1)
    {
        ...
        // $this->seo->set_company($company);
        // $this->seo->init('company');
        $this->tmpl = 'mobile/jtyjycompany/cases.html';
    }
    ...
    public function team($company_id, $page = 1)
    {
        ...
        // $this->seo->set_company($company);
        // $this->seo->init('company');
        $this->tmpl = 'mobile/jtyjycompany/team.html';
    }
    ...
    public function yuyue($company_id = null)
    {
        if (! ($company_id = (int) $company_id) && ! ($company_id = (int) $this->GP('company_id'))) {
            ...
            if (! $company = K::M('jtyjycompany/company')->detail($company_id)) {
                $this->error(404);
            } else 
                    ...
                    if ($data = $this->checksubmit('data')) {
                        if (! $data = $this->check_fields($data, 'contact,mobile')) {
                            ...
                            if ($yuyue_id = K::M('jtyjycompany/yuyue')->create($data)) {
                                K::M('jtyjycompany/company')->update_count($company_id, 'yuyue_num');
                                $this->err->add('预约公司成功');
                                $this->err->set_data('forward', $this->mklink('mobile/jtyjycompany', array(
                                    $company_id
                                )));
                            }
                        }
                        ...
                        $this->tmpl = 'mobile/jtyjycompany/yuyue.html';
                    }
    }
