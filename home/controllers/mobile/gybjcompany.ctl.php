...
class Ctl_Mobile_Gybjcompany extends Ctl_Mobile{

    public function __construct(&$system)
    {
        parent::__construct($system);
        $uri = $this->request['uri'];
        if (preg_match('/gybjcompany-([\d]+).html/i', $uri, $match)) {
    ...
    public function items($area_id = 0, $group_id = 0, $order = 0, $page = 1)
    {
        ...
        $pager['count'] = $count = 0;
        if ($items = K::M('gybjcompany/company')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/gybjcompany:items', array(
                $area_id,
                $group_id,
                $order,
                '{page}'
        ...
        $this->tmpl = 'mobile/gybjcompany/items.html';
    }

    public function detail($company_id)
    {
        $company = $this->check_gybjcompany($company_id);
        $pager = array();
        $pager['backurl'] = '/mobile/rzzn/';
        $this->pagedata['pager'] = $pager;
        // $this->seo->set_company($company);
        // $this->seo->init('company');
        $this->tmpl = 'mobile/gybjcompany/detail.html';
    }
    ...
    public function youhui($company_id, $page = 1)
    {
        ...
        $pager['page'] = $limit = 5;
        $pager['count'] = $count = 0;
        if ($items = K::M('gybjcompany/youhui')->items_by_company($company_id, $page, $limit, $count)) {
            $pager['count'] = $count;
        ...
        $pager['backurl'] = $this->mklink('mobile/gybjcompany', array(
            $company_id
        ));
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'mobile/gybjcompany/youhui.html';
    }
    ...
    public function cases($company_id, $page = 1)
    {
        ...
        // $this->seo->set_company($company);
        // $this->seo->init('company');
        $this->tmpl = 'mobile/gybjcompany/cases.html';
    }
    ...
    public function team($company_id, $page = 1)
    {
        ...
        // $this->seo->set_company($company);
        // $this->seo->init('company');
        $this->tmpl = 'mobile/gybjcompany/team.html';
    }
    ...
    public function yuyue($company_id = null)
    {
        if (! ($company_id = (int) $company_id) && ! ($company_id = (int) $this->GP('company_id'))) {
            ...
            if (! $company = K::M('gybjcompany/company')->detail($company_id)) {
                $this->error(404);
            } else 
                    ...
                    if ($data = $this->checksubmit('data')) {
                        if (! $data = $this->check_fields($data, 'contact,mobile')) {
                            ...
                            if ($yuyue_id = K::M('gybjcompany/yuyue')->create($data)) {
                                K::M('gybjcompany/company')->update_count($company_id, 'yuyue_num');
                                $this->err->add('预约公司成功');
                                $this->err->set_data('forward', $this->mklink('mobile/gybjcompany', array(
                                    $company_id
                                )));
                            }
                        }
                        ...
                        $this->tmpl = 'mobile/gybjcompany/yuyue.html';
                    }
    }
