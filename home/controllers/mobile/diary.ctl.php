...
class Ctl_Mobile_Diary extends Ctl_Mobile
{
...
//        foreach($order_list as $k=>$v){
//            $order_list[$k]['link'] = preg_replace("/mobiel/diary-index/","diary",$pager['link']);
...
//        $status_all_link = preg_replace("/mobiel/diary-index/","diary",$status_all_link);
...
//        $type_all_link = preg_replace("/mobiel/diary-index/","diary",$type_all_link);
...
//        $way_all_link = preg_replace("/mobiel/diary-index/","diary",$way_all_link);
...
//        $this->pagedata['canonical'] = "www.jzwpt.com/mobiel/diary/";
//        $this->tmpl = 'mobile/diary/index.html';
//    }
    {
        $status=isset($_REQUEST["a"])?$_REQUEST["a"]:$status;
        $type_id=isset($_REQUEST["b"])?$_REQUEST["b"]:$type_id;
        ...
        $status_list = K::M('home/site')->get_status();
        $status_all_link = $this->mklink('mobile/diary:index', array(
            0,
            $type_id,
            $way_id,
            $order,
        ...
        foreach ($status_list as $k => $v) {
            ...
            $a['link'] = $this->mklink('mobile/diary:index', array(
                $k,
                $type_id,
                $way_id,
                $order,
        ...
        $setting_type = K::M('tenders/setting')->get_type();
        $type_list = $way_list = array();
        $type_all_link = $this->mklink('mobile/diary:index', array($status, 0, $way_id, $order, 1));
        foreach ($setting_list[$setting_type['house_type']] as $k => $v) {
            $a = array(
                'title' => $v
            );
            $a['link'] = $this->mklink('mobile/diary:index', array($status,$k,$way_id,$order,1));
            $type_list[$k] = $a;
        }
        $way_all_link = $this->mklink('mobile/diary:index', array(
            $status,
            $type_id,
            0,
            $order,
        ...
        foreach ($order_list as $k => $v) {
            $v['link'] = $this->mklink('mobile/diary:index', array(
                $status,
                $type_id,
                $way_id,
                $k,
    ...
    public function items($diary_id, $page = 1){
        ...
        if (! $diary_id = (int) $diary_id) {
            ...
            if (! $detail = K::M('diary/diary')->detail($diary_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    $this->tmpl = 'mobile/diary/items.html';
                }
        $this->tmpl = 'mobile/diary/items.html';
    }
    public function comment($diary_id = null)
    {
       ...
       // echo 1;die;
        if($this->uid<=0){
            $this->err->add('您还没有登录，不能进行评论',201);
        }else
