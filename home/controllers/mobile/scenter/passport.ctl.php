...
class Ctl_Mobile_Scenter_Passport extends Ctl_Mobile
{
                ...
                if (! $uname = $data['uname']) {
                    ...
                    if (! $passwd = $data['passwd']) {
                        ...
                        if ($verifycode_success) {
                            ...
                            if ($member = $this->auth->loginshop($uname, $passwd, $a, false, $keep)) {
                                $this->err->add("{$member['uname']}，欢迎您回来!");
                            }
                        }
                    }
    ...
    public function qqcallback()
    {
        if (! $code = $this->GP('code')) {
            ...
            $forward = K::M('helper/link')->mklink('mobile/scenter:index', array(), array(), 'base');
    ...
    {
        $pager['backurl'] = $this->mklink('mobile/scenter/passport');
        $this->pagedata['pager'] = $pager;
        $this->pagedata['fromlist'] = K::M('member/view')->from_list();
        $this->tmpl = 'mobile/scenter/passport/signup.html';
    }
        ...
        if (! $this->checksubmit('data')) {
            ...
            if (! $data = $this->GP('data')) {
                ...
                if ($data['passwd'] != $this->GP('confirmpasswd')) {
                    ...
                    if ($verifycode_success) {
                        if ($uid = K::M('member/account')->create($data)) {
                            $this->err->set_data('forward', $this->mklink('mobile/scenter'));
