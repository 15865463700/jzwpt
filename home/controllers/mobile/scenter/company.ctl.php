...
class Ctl_Mobile_Scenter_Company extends Ctl_Mobile_Scenter
{
    ...
    public function zxb()
    {
        ...
        if ($company) {
            ...
            $this->tmpl = 'mobile/scenter/company/zxb.html';
        } else {
        ...
        if ($company) {
            if ($items = K::M('zxb/zxb')->detail($zxb_id)) {
                if (! $company_list = K::M('zxb/zxb')->items(array(
                    ...
                    $pager['backurl'] = $this->mklink('mobile/scenter/company-zxb');
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'mobile/scenter/company/zxb_lists.html';
                }
            } else {
    ...
    public function zxb_detail($zxb_id, $status)
    {
        ...
        if (! $company) {
            ...
            if ($items = K::M('zxb/zxb')->detail($zxb_id)) {
                $items['status_n']=$items['status']+1;
                if (! $company_list = K::M('zxb/zxb')->items(array(
                    'company_id' => $company['company_id']
                ))) {
                    $this->err->add('您没有权限查看该预约', 201);
                } else {
                    $status_arr = K::M('zxb/zxb')->get_status();
                    $this->pagedata['status'] = $status_arr;
                    $this->pagedata['item'] = $items;
                }
            } else {
                    ...
                    if ($detail['status'] < ($status - 1)) {
                        ...
                        if ($this->checksubmit('data')) {
                            if (! $data = $this->GP('data')) {
                                ...
                                $size=$attachs['hetong']['size'];  //截取图片大小
                                //echo $size;
                                ...
                                $data['zxb_id'] = $zxb_id;
                                if ($hetong_list) {
                                    if($size>=4194304){//$size==0 && $size>=4194304
                                        $this->err->add('文件不得超过4M，请重新上传', 201);
                                    }else{
                            ...
                            $pager['backurl'] = $this->mklink('mobile/scenter/company:zxb_lists', array(
                                $zxb_id
                            ));
                            $this->pagedata['pager'] = $pager;
                            $this->tmpl = 'mobile/scenter/company/hetong.html';
                        }
                    }
    }
                ...
                if ($detail['company_id'] < $company['company_id']) {
                    ...
                    if ($detail['status'] < ($status - 1)) {
                        ...
                        if ($this->checksubmit('data')) {
                            ...
                            $this->pagedata['pager'] = $pager;
                            $this->tmpl = 'mobile/scenter/company/detail.html';
                        }
                    }
    }
                ...
                if ($detail['company_id'] < $company['company_id']) {
                    ...
                    if ($detail['status'] < ($status - 1)) {
                        ...
                        if ($this->checksubmit('data')) {
                            ...
                            $this->pagedata['pager'] = $pager;
                            $this->tmpl = 'mobile/scenter/company/endstep.html';
                        }
                    }
                }
            }
    ...
    public function youhuiSign()
    {
        ...
        $pager['backurl'] = $this->mklink('mobile/scenter');
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'mobile/scenter/company/youhuiSign.html';
    }
}
