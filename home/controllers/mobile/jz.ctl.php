...
class Ctl_Mobile_Jz extends Ctl_Mobile
{
    ...
    public function items($area_id = 0, $group_id = 0, $order = 0, $page = 1)
    {
        ...
        if ($items = K::M('company/company')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/jz:items', array(
                $area_id,
                $group_id,
                $order,
                '{page}'
...
//        $this->seo->init('gs_items', $seo);
        $this->seo->init('jzhuang_items', $seo);
        $this->tmpl = 'mobile/jz/items.html';
    }
    ...
    public function team($company_id, $page = 1)
    {
        ...
        $this->seo->set_company($company);
        $this->seo->init('company');
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/company/team-{$company_id}.html";
