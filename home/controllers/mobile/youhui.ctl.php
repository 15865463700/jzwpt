...
class Ctl_Mobile_Youhui extends Ctl_Mobile
{
    ...
    public function index($page = 1)
    {
        ...
        if ($items = K::M('company/youhui')->items($filter, array(
            'youhui_id' => 'DESC'
        ), $page, $limit, $count)) {
            $counts = 0;
            ...
            if (! $detail = K::M('company/youhui')->detail($youhui_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if (! $company = K::M('company/company')->detail($detail['company_id'])) {
                        ...
                        $pager['backurl'] = $this->mklink('mobile/company:youhui', array(
                            $detail['company_id']
                        ));
                        $this->pagedata['pager'] = $pager;
                        $seo = array(
                ...
                if (empty($detail['audit'])) {
                    ...
                    if (! $data = $this->check_fields($data, 'contact,mobile')) {
                        ...
                        if ($id = K::M('company/sign')->create($data)) {
                            ...
                            $this->err->set_data('forward', $this->mklink('mobile/youhui/detail', array(
                                'youhui_id' => $youhui_id
                            )));
                        }
                    }
                    ...
                    $pager['tender_hide'] = 1;
                    $pager['backurl'] = $this->mklink('mobile/youhui/detail', array(
                        'youhui_id' => $youhui_id
                    ));
                    $this->pagedata['pager'] = $pager;
