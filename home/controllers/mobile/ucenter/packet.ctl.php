...
class Ctl_Mobile_Ucenter_Packet extends Ctl_Mobile_Ucenter
{
    ...
    public function items($type = 1, $page = 1)
    {
        ...
        $pager['backurl'] = $this->mklink('mobile/ucenter/index');
        $this->pagedata['cate'] = K::M('shop/cate')->items(array(
            'parent_id' => '0'
        ));
        $this->tmpl = 'mobile/ucenter/packet/index.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            ...
            if ($items = K::M('member/packet')->items(array(
                ...
                if (K::M('member/packet')->update($detail['id'], $data)) {
                    $this->err->add('添加红包成功');
                    $this->tmpl = 'mobile/ucenter/packet/items.html';
                }
            } else {
