...
//        $pager['backurl'] = $this->mklink('mobile/ucenter/sign-sign');
    ...
    public function zxb_lists($zxb_id)
    {
        ...
        } elseif (! $zxb = K::M('zxb/zxb')->items(array(
            'uid' => $uid,
            'zxb_id' => $zxb_id
        ))) {
                    ...
                    if ($detail['status'] < ($status - 1)) {
                        ...
                        if (! $member = K::M('member/member')->detail($this->uid)) {
                            ...
                            if ($this->checksubmit('data')) {
                                        ...
                                        if (! $hetong_id = $this->GP('hetong_id')) {
                                            ...
                                            if (! $step_list = K::M('zxb/step')->items(array(
                                                ...
                                                if (K::M('zxb/hetong')->update($hetong_id, $data)) {
                                                  ...
                                                  $mobile=$data['yezhu_phone'];
                                                  if(!K::M('verify/check')->phone($mobile)){
                                                     $this->err->add('手机号码有误', 201);
                                                  }
                                ...
                                $pager['backurl'] = $this->mklink('mobile/ucenter/member:zxb_lists', array(
                                    'zxb_id' => $zxb_id
                                ));
                                $this->pagedata['pager'] = $pager;
...
//                                    $size=$attachs ["yezhu_photo"]['size'];
//                                   if($size>4000000){
//                                       $this->err->add('文件不能大于4M，请重新上传', 201);
//                                   }
                                    $upload = K::M('magic/upload');
                                    foreach ($attachs as $k => $attach) {
                                        if ($attach['error'] == UPLOAD_ERR_OK) {
                            ...
                            if (! $data = $this->GP('data')) {
                                ...
                                if ($step_list['uid']) {
                                    ...
                                    if (! $step_id = $this->GP('step_id')) {
                                        ...
                                        if (K::M('zxb/step')->update($step_id, $data)) {
                                            $time=$_REQUEST["back"];
                                            $data['dealline']=$back_time=$time*86400+time();
                                            K::M('zxb/step')->update($step_id, $data);
                                            K::M('zxb/zxb')->update($zxb_id, array("back"=>$time,"dealline"=>$back_time));
                                            $time1=time();
                                            $this->pagedata['time1'] = $time1;
                                            $this->err->add('修改内容成功');
                                        }
                                    }
                                } else {
