...
class Ctl_Mobile_Ucenter_Tenders extends Ctl_Mobile_Ucenter
{
    ...
    {
        $pager['backurl'] = $this->mklink('mobile/ucenter/member:tenderDetail', array(
                                            ...
                                            if ($look_id = K::M('tenders/look')->create($data)) {
                                                ...
                                                switch ($this->MEMBER['from']) {
                                                    ...
                                                    case 'mechanic':
                                                        ...
                                                        break;
                                                    case 'company':
                                                        K::M('company/company')->update_count($this->company['company_id'], 'tenders');
                                                        break;
                                                    case 'shop':
                                                        K::M('shop/shop')->update_count($this->shop['shop_id'], 'tenders');
                                                        break;
                                                }
                                                $this->err->add('参加竞标成功！');
                                                $this->err->set_data('forward', $this->mklink('mobile/ucenter/tenders:tenderDetail', array(
                                                    'tenders_id' => $tenders_id
                                                )));
                                            }
                                        }
                                    } else {
                                        $pager['backurl'] = $this->mklink('mobile/ucenter/tenders-tendered', array(
                                            'tenders_id' => $tenders_id
                                        ));
                                        $this->pagedata['pager'] = $pager;
