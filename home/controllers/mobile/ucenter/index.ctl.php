...
class Ctl_Mobile_Ucenter_Index extends Ctl_Mobile_Ucenter
{
    ...
    public function index()
    {
        ...
        if (in_array($this->MEMBER['from'], array(
            'shop',
            'company'
        ))) {
        ...
        if ($account = $this->checksubmit('account')) {
            ...
            if (K::M('member/member')->update($this->uid, $account)) {
                if ($account['from'] == 'designer') {
                    K::M('designer/designer')->create(array(
                        'uid' => $this->uid,
                        'city_id' => $this->MEMBER['city_id']
                    ), null, true);
                } else 
                    if ($account['from'] == 'gz') {
                        K::M('gz/gz')->create(array(
                            'uid' => $this->uid,
                            'city_id' => $this->MEMBER['city_id']
                        ), null, true);
                    }
    ...
    public function mail()
    {
        if ($account = $this->checksubmit('account')) {
            if (md5($account['passwd']) != $this->MEMBER['passwd']) {
                ...
                if ($account['new_mail'] == $this->MEMBER['mail']) {
                    $this->err->add('新老邮箱一致不用修改', 211);
                } else 
                    if ($mail = K::M('member/account')->check_mail($account['new_mail'])) {
    ...
    public function appointment()
    {
        ...
        if ($form == 'member') {
            ...
            $form_list = $this->$temp();
            $filter = array(
                'company_id' => $form_list['company_id'],
                'closed' => 0
            );
