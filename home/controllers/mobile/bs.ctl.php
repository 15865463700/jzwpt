...
class Ctl_Mobile_Bs extends Ctl_Mobile
{
    ...
    public function items($area_id = 0, $group_id = 0, $order = 0, $page = 1)
    {
        ...
        if ($items = K::M('company/company')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/bs:items', array(
                $area_id,
                $group_id,
                $order,
                '{page}'
...
//        $this->seo->init('gs_items', $seo);
        $this->seo->init('bzhuang_items', $seo);
        $this->tmpl = 'mobile/bs/items.html';
    }
