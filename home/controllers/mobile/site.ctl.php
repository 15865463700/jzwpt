    ...
    public function index($page = 1)
    {
        ...
        if ($items = K::M('home/site')->items($filter, null, $page, $limit, $count)) {
            ...
            if ($member_list = K::M('member/member')->items_by_ids($uids)) {
                ...
                $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mobile/site:index', array(
                    '{page}'
                )));
            }
        }
                ...
                if ($data = $this->checksubmit('data')) {
                    if (! $data = $this->check_fields($data, 'contact,mobile')) {
                        ...
                        if (($site['company_id']) && ($company = K::M('company/company')->detail($site['company_id']))) {
                            ...
                            if ($member = K::M('member/member')->detail($site['uid'])) {
                                if ($member['from'] == 'gz') {
                        ...
                        if ($yuyue_result) {
                            $this->err->add('预约参观成功！');
                            $this->err->set_data('forward', $this->mklink('mobile/site:detail', array(
                                $site_id
                            )));
                        }
                    }
                    ...
                    $pager['tender_hide'] = 1;
                    $pager['backurl'] = $this->mklink('mobile/site:detail', array(
                        $site_id
                    ));
                    $this->pagedata['pager'] = $pager;
                    $this->pagedata['site'] = $site;
