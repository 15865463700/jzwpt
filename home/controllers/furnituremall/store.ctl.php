...
class Ctl_Furnituremall_Store2 extends Ctl
{
    ...
    public function index()
    {
        ...
        if ($items = K::M('shop/shop')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('furnituremall/store', array(
                $cat_id,
                $order,
                '{page}'
            ), $SO));
        ...
        $this->seo->init('mall_store', array(
        ...
        $this->tmpl = 'furnituremall/store/items.html';
    }
}