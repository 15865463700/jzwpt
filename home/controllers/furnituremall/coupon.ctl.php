...
class Ctl_Furnituremall_Coupon2 extends Ctl
{
    ...
    public function items($type = 0, $page = null)
    {
        ...
        if ($items = K::M('shop/coupon')->items($filter, null, $page, $limit, $count)) {
            foreach ($items as $k => $v) {
                if ($type == 1 && $v['ltime'] < __TIME && $v['ltime'] != 0) {
                    ...
                    if ($type == 2 && ($v['ltime'] > __TIME || $v['ltime'] == 0)) {
                        unset($items[$k]);
                    } else {
                        continue;
                    }
            }
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('furnituremall/coupon:items', array(
                $type,
                '{page}'
            )));
        ...
        $this->seo->init('coupon', array(
            'page' => ($page > 1) ? $page : 1
        ));
        $this->tmpl = 'furnituremall/coupon/items.html';
    }
        ...
        if (! $coupon_id = (int) $coupon_id) {
            ...
            if (! $detail = K::M('shop/coupon')->detail($coupon_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    $this->tmpl = 'furnituremall/coupon/detail.html';
