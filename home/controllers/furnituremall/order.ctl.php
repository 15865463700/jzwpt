...
class Ctl_Furnituremall_Order2 extends Ctl
{
    ...
    public function create()
    {
        ...
        if (! $data = $this->checksubmit('data')) {
            ...
            if (empty($cart['product_count'])) {
                ...
                $this->err->set_data('forward', $this->mklink('furnituremall/index'));
            } else 
                if ($order = K::M('trade/order')->create_by_cart($cart, $data)) {
                    $this->system->cookie->set('Order-Buy', 1, 3600);
                    $this->err->add('提交定单成功');
                    K::M('trade/cart')->clean();
                    $this->err->set_data('forward', $this->mklink('trade/order:success', array(
                        $order['order_no']
                    )));
                }
        }
