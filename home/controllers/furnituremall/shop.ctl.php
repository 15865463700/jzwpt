...
class Ctl_Furnituremall_Shop2 extends Ctl
{
    ...
    public function detail($shop_id)
    {
        ...
        $this->seo->init('shop', array(
            'cate_name' => $cate['title']
        ));
        $this->pagedata['mobile_url'] = $this->mklink('mobile/shop', array(
            $shop_id
        ));
    ...
    public function detail_c($cat_id)
    {
        ...
        $product = K::M('product/product')->items(array(
            'vcat_id' => $cat_id,
            'audit' => '1',
            'closed' => '0',
            'city_id' => $this->request['city_id']
    ...
    public function news($shop_id, $page = 1)
    {
        ...
        if ($items = K::M('shop/news')->items_by_shop($shop_id, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('furnituremall/shop:news', array(
                $shop_id,
                '{page}'
            ), array(
                'shop' => $shop
    ...
    public function mendian($shop_id, $page = 1)
    {
        ...
        if ($items = K::M('shop/mendian')->items_by_shop($shop_id, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('furnituremall/shop:mendian', array(
                $shop_id,
                '{page}'
            ), array(
                'shop' => $shop
        ...
        if (! $news_id = (int) $news_id) {
            ...
            if (! $detail = K::M('shop/news')->detail($news_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($shop = $this->check_shop($detail['shop_id'])) {
                        K::M('shop/news')->update_count($news_id, 'views', 1);
                        $this->pagedata['detail'] = $detail;
                        if ($prev = K::M('shop/news')->items(array(
                            'shop_id' => $shop_id,
                            'audit' => 1,
                            'news_id' => '<:' . $news_id
                        ), array(
                        ...
                        if ($next = K::M('shop/news')->items(array(
                            'shop_id' => $shop_id,
                            'audit' => 1,
                            'news_id' => '>:' . $news_id
                        ), array(
    ...
    public function mendiandetail($mendian_id)
    {
        if (! $mendian_id = (int) $mendian_id) {
            ...
            if (! $detail = K::M('shop/mendian')->detail($mendian_id)) {
                $this->error(404);
            } else 
                if (empty($detail['audit'])) {
                    $this->err->add('内容还未发布，暂不能访问', 211);
                    ...
                    if ($shop = $this->check_shop($detail['shop_id'])) {
                        ...
                        $this->pagedata['detail'] = $detail;
                        if ($prev = K::M('shop/mendian')->items(array(
                            'shop_id' => $shop_id,
                            'audit' => 1,
                        ...
                        if ($next = K::M('shop/mendian')->items(array(
                            'shop_id' => $shop_id,
                            'audit' => 1,
                            'mendian_id' => '>:' . $mendian_id
                        ), array(
    ...
    public function product($shop_id, $vcat_id = 0, $page = null)
    {
        ...
        if ($items = K::M('product/product')->items($filter, null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('furnituremall/shop:product', array(
                $shop_id,
                $vcat_id,
                '{page}'
            ), array(
        ...
        $this->pagedata['mobile_buy_url'] = $this->mklink('mobile/shop:product', array(
            $shop_id
        ));
        if ($shop['skin'] == 'default') {
    ...
    public function coupon($shop_id, $page = 1)
    {
        ...
        if ($items = K::M('shop/coupon')->items_by_shop($shop_id, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mall/shop:coupon', array(
                $shop_id,
                '{page}'
            ), array(
                'shop' => $shop
    ...
    public function savecomment($shop_id)
    {
        ...
        if ($this->check_login()) {
            ...
            if ($allow_comment < 0) {
                ...
                if ($this->uid == $shop['uid']) {
                    $this->err->add('不能点评自己的店铺', 101);
                } else 
                    if (! $data = $this->checksubmit('data')) {
                        $this->err->add('非法的数据提交', 214);
                    } else 
                        if (! $data = $this->check_fields($data, 'score,content')) {
                            $this->err->add('非法的数据提交', 214);
                        } else {
