...
class Ctl_Furnituremall_Product2 extends Ctl
{
    ...
    public function index()
    {
        ...
        if ($items = K::M('product/product')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('furnituremall/product', array(
                $cat_id,
                $attr_ids,
                $order,
                '{page}'
        ...
        $order_link = array();
        $order_link['default'] = $this->mklink('furnituremall/product', array(
            $cat_id,
            $brand_id,
            $attr_ids,
            0,
        ...
        $order_link['buy'] = $this->mklink('furnituremall/product', array(
            $cat_id,
            $brand_id,
            $attr_ids,
            1,
        ...
        $order_link['price'] = $this->mklink('furnituremall/product', array(
            $cat_id,
            $brand_id,
            $attr_ids,
            2,
        ...
        $this->tmpl = 'furnituremall/product/items.html';
    }
            ...
            if (! $detail = K::M('product/product')->detail($product_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($shop = $this->check_shop($detail['shop_id'])) {
                        ...
                        $this->pagedata['mobile_buy_url'] = $this->mklink('mobile/product:detail', array(
                            $product_id
                        ));
                        $this->tmpl = 'furnituremall/product/detail.html';
                    }
    }
    ...
    public function comment()
    {
        ...
        if (! $product_id = $this->GP('product_id')) {
            ...
            if (! $product = K::M('product/product')->detail($product_id, true)) {
                $this->err->add('商品不存在或已经删除', 212);
            } else 
                if (empty($product['audit'])) {
                    $this->err->add('商品还在审核中', 213);
                } else 
                    if (! $data = $this->checksubmit('data')) {
                        $this->err->add('非法的数据提交', 214);
                    } else 
                        if (! $data = $this->check_fields($data, array(
                            'score',
                            'content'
                        ))) {
                            $this->err->add('非法的数据提交', 214);
                        } else 
                            if (($allow_comment = K::M('member/group')->check_priv($this->MEMBER['group_id'], 'allow_comment')) < 0) {
