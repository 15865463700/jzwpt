 ...
 */
class Ctl_Furnituremall_Cart2 extends Ctl
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'furnituremall/order/cart.html';
    }
    ...
    public function checkout()
    {
        ...
        $this->pagedata['cart'] = $cart;
        $this->tmpl = 'furnituremall/order/checkout.html';
    }
    ...
    public function add($product_id = 0, $spec_id = 0, $num = 0)
    {
        if (! ($product_id = (int) $product_id) && ! ($product_id = (int) $this->GP('product_id'))) {
            ...
            if (! ($num = (int) $num) && ! ($num = (int) $this->GP('num'))) {
                $this->err->add('未定义操作', 211);
            } else 
                if (! $product = K::M('product/product')->detail($product_id, true)) {
                    ...
                    if (empty($product['audit'])) {
                        ...
                        if ($this->uid && $this->uid == $shop['uid']) {
                            ...
                            if (K::M('trade/cart')->add($product_id, $spec_id, $num)) {
    ...
    public function delete($product_id = null, $spec_id = 0)
    {
        ...
        if (! $product_id = (int) $product_id) {
            ...
            if (K::M('trade/cart')->delete($product_id, $spec_id)) {
