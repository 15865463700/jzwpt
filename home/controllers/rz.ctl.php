...
//        $attr_values = K::M('data/attr')->attrs_by_from('zx:bycl', true);
        ...
        if (preg_match('/items(-[\d\-]+)?(-(\d+)).html/i', $uri, $m) || preg_match('/rz(-[\d\-]+)?(-(\d+))(\.html)*/i', $uri, $m)) {
            $page = (int) $m[3];
            if ($m[1]) {
        ...
        if ($group_list = K::M('member/group')->items_by_from('company')) {
            $group_all_link = $this->mklink('rz:items', array($area_id, 0, implode('-', $attr_value_ids), $order, 1));
            foreach ($group_list as $k => $v) {
                $v['link'] = $this->mklink('rz:items', array($area_id, $k, implode('-', $attr_value_ids), $order, 1));
                $group_list[$k] = $v;
            }
        }
        $area_list = $this->request['area_list'];
        $area_all_link = $this->mklink('rz:items', array(
            0,
            $group_id,
...
//        $area_all_link = preg_replace("/\.html/", "/", $area_all_link);
        foreach ($area_list as $k => $v) {
            $v['link'] = $this->mklink('rz:items', array(
                $k,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
...
//            $v['link'] = preg_replace("/\.html/", "/", $v['link']);
        ...
        $order_list[0]['link'] = $this->mklink('rz:items', array($area_id, $group_id, implode('-', $attr_value_ids), 0, 1));
        $order_list[1]['link'] = $this->mklink('rz:items', array($area_id, $group_id, implode('-', $attr_value_ids), 1, 1));
        $order_list[2]['link'] = $this->mklink('rz:items', array($area_id, $group_id, implode('-', $attr_value_ids), 2, 1));
        $pager = $filter = array();
...
//        }
        ...
        $items = K::M('rz/rz')->items($filter, $orderby, $page, $limit, $count);
        ...
        if ($items) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('rz:items', array(
                        $area_id,
                        $order,
                        '{page}')));
...
//        $this->pagedata['group_list'] = $group_list;
        ...
        if(is_main()){
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('rz_items', $seo):$this->seo->init('rz_items_attr', $seo);
        }else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_rz_items', $seo):$this->seo->init('fenzhan_rz_items_attr', $seo);
        }
        $this->tmpl = 'rz/items.html';
    }
