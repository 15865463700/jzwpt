...
class Ctl_Ucenter_Misc_Select extends Ctl_Ucenter
{
    ...
    public function company($city_id = null, $page = null,$uid=0)
    {
        $filter = $pager = $params = array();
        $pager['page'] = max(intval($page), 1);
        $pager['limit'] = $limit = 10;
        ...
        if($uid){
            $list=K::M('zxb/zxb')->items(array("uid"=>$uid), null, 1, 100000, $count);
            if($list){
                $filter['company_ids']="";
                foreach($list as $k=>$v){
                    $filter['company_ids'].= empty($filter['company_ids'])?$v['company_id']:",{$v['company_id']}";
                }
            }
        }
