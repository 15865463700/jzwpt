...
class Ctl_Ucenter_Designer_Case extends Ctl_Ucenter
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'ucenter/designer/case/items.html';
    }
                ...
                if ($allow_case < 0) {
                    ...
                    if (! $detail = K::M('home/home')->detail($data['home_id'])) {
                        ...
                        if ($case_id = K::M('case/case')->create($data)) {
                            ...
                            $this->err->set_data('forward', $this->mklink('ucenter/designer/case:detail', array(
                                $case_id
                            )));
                            $this->err->add('添加案例成功');
                        }
            ...
            if (! $detail = K::M('case/case')->detail($case_id)) {
                ...
                if ($detail['uid'] != $designer['designer_id']) {
                    ...
                    if ($items = K::M('case/photo')->items_by_case($case_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("ucenter/designer/case:detail", array(
                            $case_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'ucenter/designer/case/detail.html';
                }
    }
