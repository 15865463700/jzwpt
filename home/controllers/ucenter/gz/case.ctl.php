 ...
 * $Id$
 */
class Ctl_Ucenter_Gz_Case extends Ctl_Ucenter
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'ucenter/gz/case/items.html';
    }
    ...
    public function create()
    {
        ...
        if (K::M('system/integral')->check('case', $this->MEMBER) === false) {
            ...
            if ($data = $this->checksubmit('data')) {
                ...
                if ($allow_case < 0) {
                    ...
                    if (! $detail = K::M('home/home')->detail($data['home_id'])) {
                        ...
                        if ($case_id = K::M('case/case')->create($data)) {
                            ...
                            $this->err->set_data('forward', $this->mklink('ucenter/gz/case:detail', array(
                                $case_id
                            )));
                            $this->err->add('添加案例成功');
                        }
                ...
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'ucenter/gz/case/create.html';
            }
    }
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('case/case')->detail($case_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $pager['city_id'] = $gz['city_id'];
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'ucenter/gz/case/edit.html';
                }
    }
            ...
            if (! $detail = K::M('case/case')->detail($case_id)) {
                ...
                if ($detail['uid'] != $gz['uid']) {
                    ...
                    if ($items = K::M('case/photo')->items_by_case($case_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("ucenter/gz/case:detail", array(
                            $case_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'ucenter/gz/case/detail.html';
                }
    }
