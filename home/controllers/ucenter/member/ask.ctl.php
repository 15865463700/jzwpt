...
class Ctl_Ucenter_Member_Ask extends Ctl_Ucenter
{
    ...
    public function index($page = 1)
    {
        ...
        $pager['count'] = $count = 0;
        if ($items = K::M('ask/ask')->items(array(
            'uid' => $this->uid
        ), null, $page, $limit, $count)){
            $items = K::M('ask/ask')->ask_answer_num($items);
            $pager['count'] = $count;
        ...
        $this->tmpl = 'ucenter/member/ask/asks.html';
    }
    ...
    public function answer($page = 1)
    {
        ...
        $pager['limit'] = $limit = 20;
        $pager['count'] = $count = 0;
        if ($items = K::M('ask/answer')->items(array(
            'uid' => $this->uid
        ), null, $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array(
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'ucenter/member/ask/answers.html';
    }
}