...
class Ctl_Ucenter_Company_Case extends Ctl_Ucenter
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'ucenter/company/case/items.html';
    }
            ...
            if ($data = $this->checksubmit('data')) {
                if (! $data = $this->check_fields($data, $this->_allower_fields)) {
                    ...
                    if ($case_id = K::M('case/case')->create($data)) {
                        ...
                        $this->err->add('添加案例成功');
                        $this->err->set_data('forward', $this->mklink('ucenter/company/case:detail', array(
                            $case_id
                        ), array(), true));
                    }
                }
            ...
            if (! $detail = K::M('case/case')->detail($case_id)) {
                ...
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    if ($items = K::M('case/photo')->items_by_case($case_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("ucenter/company/case:detail", array(
                            $case_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'ucenter/company/case/detail.html';
                }
    }
