...
class Ctl_Gz extends Ctl{
    public static $ctl_gz_old="gs";
    public static $ctl_gz_new="gz";
    public static $tpl_gz="gz";
    ...
    public function items($page = 1){
        ...
        if (preg_match('/items(-[\d\-]+)?(-(\d+)).html/i', $uri, $m)||preg_match('/'.Ctl_Gz::$ctl_gz_new.'(-[\d\-]+)?(-(\d+))(\.html)*/i', $uri, $m)) {
            $page = (int) $m[3];
            if ($m[1]) {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
                $area_id,
                $group_id,
                implode('-', $vids)
            ));
            ...
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
                    $area_id,
                    $group_id,
                    implode('-', $vids)
                ));
        ...
        if ($group_list = K::M('member/group')->items_by_from('company')) {
            $group_all_link = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
                $area_id,
                0,
        ...
        $area_all_link = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
            0,
            $group_id,
            implode('-', $attr_value_ids),
            $order,
        ...
        $order_list[0]['link'] = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            0,
        ...
        $order_list[1]['link'] = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            1,
        ...
        $order_list[2]['link'] = $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            2,
        ...
        if ($items = K::M('company/company')->items($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(Ctl_Gz::$ctl_gz_new.':items', array(
                $area_id,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
            ...
            foreach($items as $k=>$v){
                ...
                $items[$k]['company_url']=  str_replace(Ctl_Gz::$ctl_gz_old, Ctl_Gz::$ctl_gz_new, $items[$k]['company_url']);
            }
        ...
        foreach($order_list as $k=>$v){
            $order_list[$k]["link"] = preg_replace("/gs-items/",Ctl_Gz::$ctl_gz_new,$v["link"]);
            $order_list[$k]["link"] = preg_replace("/gs-0-0-0-0-0-1\./",Ctl_Gz::$ctl_gz_new.".",$order_list[$k]["link"]);
            $order_list[$k]["link"] = preg_replace("/\.html/","/",$order_list[$k]["link"]);
            $order_list[$k]["link"] = preg_replace(Ctl_Gz::$ctl_gz_old,Ctl_Gz::$ctl_gz_new,$order_list[$k]["link"]);
        }
        ...
        if(is_main()){
//            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('gs_items', $seo):$this->seo->init('gs_items_attr', $seo);
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('gzhuang_items', $seo):$this->seo->init('gzhuang_items_attr', $seo);
        }else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_gzhuang_items', $seo):$this->seo->init('fenzhan_gzhuang_items_attr', $seo);
        }
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/".Ctl_Gz::$ctl_gz_new."/";
        $this->tmpl = Ctl_Gz::$tpl_gz.'/items.html';
    }
    ...
    public function map()
    {
        ...
        $this->tmpl = Ctl_Gz::$tpl_gz.'/maps.html';
    }
            ...
            if (! $detail = K::M('company/company')->detail($company_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($this->checksubmit('data')) {
                        ...
                        $this->tmpl = Ctl_Gz::$tpl_gz.'/yuyue.html';
                    }
                }
    }
}