...
class Ctl_Gybjnews extends Ctl
{
    ...
    public function items($page = 1)
    {
        ...
        $filter['audit'] = 1;
        if ($items = K::M('gybjcompany/news')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
    ...
    public function detail($news_id)
    {
        ...
        if (! $news_id = (int) $news_id) {
            ...
            if (! $detail = K::M('gybjcompany/news')->detail($news_id)) {
                ...
                K::M('gybjcompany/news')->update_count($news_id, 'views', 1);
                $gybjcompany = $this->check_company($detail['company_id']);
                ...
                $this->tmpl = 'gybjnews/detail.html';
            }
    }
