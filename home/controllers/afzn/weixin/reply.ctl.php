...
class Ctl_Afzn_Weixin_Reply extends Ctl_Afzn_Anfangzhineng
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'afzn/weixin/reply/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, 'title,intro,content,jumpurl')) {
                ...
                if ($reply_id = K::M('weixin/reply')->create($data)) {
                    $this->err->set_data('forward', $this->mklink('afzn/weixin/reply:index'));
                    $this->err->add('添加微信素材成功');
                }
            }
        } else {
            $this->tmpl = 'afzn/weixin/reply/create.html';
        }
    }
            ...
            if (! $detail = K::M('weixin/reply')->detail($reply_id)) {
                ...
                if ($detail['wx_id'] != $weixin['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'afzn/weixin/reply/edit.html';
                    }
    }
    ...
    public function dialog($page = 1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'afzn/weixin/reply/dialog.html';
    }
}