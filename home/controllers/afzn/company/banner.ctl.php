...
// class Ctl_Afzn_Company_Banner extends Ctl_Afzn_Anfangzhineng
class Ctl_Afzn_Company_Banner extends Ctl
{
    ...
    public function index()
    {
        ...
        $count = 0;
        if ($items = K::M('afzncompany/banner')->items_by_company($company['company_id'])) {
        ...
        $this->tmpl = 'afzn/company/banners.html';
    }
        ...
        if ($allow_banner < 0) {
            ...
            if (! $attach = $_FILES['Filedata']) {
                ...
                if (UPLOAD_ERR_OK != $attach['error']) {
                    ...
                    if (K::M('afzncompany/banner')->count_by_company($company['company_id']) > 5) {
                        $this->err->add('轮转广告最多只可上传5张图片', 214);
                    } else {
                        if ($a = K::M('afzncompany/banner')->upload($company['company_id'], $attach)) {
    ...
    public function update()
    {
        ...
        if ($allow_banner < 0) {
            ...
            if ($data = $this->checksubmit('data')) {
                if ($banners = K::M('afzncompany/banner')->items_by_company($company['company_id'])) {
                    foreach ($banners as $k => $v) {
                        if ($a = $data[$k]) {
                            if ($a['title'] != $v['title'] || $a['link'] != $v['link'] || $a['orderby'] != $v['orderby']) {
                                K::M('afzncompany/banner')->update($k, $a);
                            }
                        }
                    }
                }
        ...
        if (! $banner_id = (int) $banner_id) {
            ...
            if (! $banner = K::M('afzncompany/banner')->detail($banner_id)) {
                ...
                if ($banner['company_id'] != $company['company_id']) {
                    ...
                    if (K::M('afzncompany/banner')->delete($banner_id)) {
