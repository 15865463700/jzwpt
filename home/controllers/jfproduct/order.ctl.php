 ...
 */
class Ctl_Jfproduct_Order extends Ctl
{
    ...
    public function save()
    {
        ...
        if (! $data = $this->checksubmit('data')) {
            ...
            if (! $product = K::M('jfproduct/jfproduct')->detail($data['product_id'])) {
                $this->err->add('商品不存在或已删除', 211);
              ...
              // }
                ...
                if (! $data['product_num']) {
                    ...
                    if ($this->MEMBER['jifen'] >= $data['jfamount']) {
                        ...
                        $member = $this->MEMBER;
                        // 可储积分
                        K::M('member/member')->update($member['uid'], array(
                            'jifen' => $member['jifen'] - $data['jfamount']
                        ), $checked = false);
                        ...
                        if ($order_id = K::M('trade/jforder')->create($data)) {
                            $this->err->add('提交订单成功');
                            $this->err->set_data('forward', $this->mklink('jfproduct/order:detail', array(
                                $order_id
                            )));
                        }
                    } else {
                    ...
                    if ($order['order_status'] < 0) {
                        ...
                        if ($order['order_status'] == 2) {
                            ...
                            if ($order['pay_status']) {
                                ...
                                // 更改兑换状态
                                k::M('trade/jforder')->update($order['order_id'], array(
                                    'pay_status' => 1
                                ), $checked = false);
                                $jifen = K::M('jfproduct/jfproduct')->detail($order['product_id']);
                                // 变更售出数量、库存。
                                K::M('jfproduct/jfproduct')->update($order['product_id'], array(
                                    'buys' => $jifen['buys'] + $order['product_num'],
                                    'kucun' => $jifen['kucun'] - $order['product_num']
                                ), $checkde = false);
                                // $url = 'http://hf.v6.jhcms.cn/jfproduct/jfproduct/';
                                // header("location: {$url}");
                                // die;
                                $this->err->add('兑换成功');
                                $this->err->set_data('forward', $this->mklink('jfproduct/jfproduct:index'));
                                // $this->err->set_data('forward', 'http://hf.v6.jhcms.cn/jfproduct/jfproduct/index');
                                // $url = "http://hf.v6.jhcms.cn/jfproduct/jfproduct/index";
                                // header("Location:{$url}");
                                // exit;
                            }
            }
