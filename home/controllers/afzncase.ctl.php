 ...
 * $Id$
 */
class Ctl_Afzncase extends Ctl
{
    ...
    public function items($page = 1)
    {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('afzncase:items', array(
                implode('-', $vids)
            ));
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('afzncase:items', array(
                    implode('-', $vids)
                ));
                $v['values'][$kk] = $vv;
            }
        ...
        $order_list[0]['link'] = $this->mklink('afzncase:items', array(
            implode('-', $attr_value_ids),
            0,
            1
        ));
        $order_list[1]['link'] = $this->mklink('afzncase:items', array(
            implode('-', $attr_value_ids),
            1,
            1
        ));
        $order_list[2]['link'] = $this->mklink('afzncase:items', array(
            implode('-', $attr_value_ids),
            2,
            1
        ));
        ...
        // $orderby = NULL;
        // }
        $items = K::M('afzncase/case')->items($filter, $orderby, $page, $limit, $count);
        if ($items) {
            $lastphotos = array();
            ...
            $this->pagedata['photos'] = K::M('afzncase/photo')->items_by_ids($lastphotos);
            // }
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('afzncase:items', array(
                implode('-', $attr_value_ids),
                $order,
                '{page}'
            ), $params));
        ...
        $this->seo->init('afzncase', $seo);
        $this->tmpl = 'afzncase/items.html';
    }
    ...
    public function album($page = 1)
    {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('afzncase:album', array(
                implode('-', $vids)
            ));
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('afzncase:album', array(
                    implode('-', $vids)
                ));
                $v['values'][$kk] = $vv;
            }
        ...
        $order_list[0]['link'] = $this->mklink('afzncase:album', array(
            implode('-', $attr_value_ids),
            0,
            1
        ));
        $order_list[1]['link'] = $this->mklink('afzncase:album', array(
            implode('-', $attr_value_ids),
            1,
            1
        ));
        $order_list[2]['link'] = $this->mklink('afzncase:album', array(
            implode('-', $attr_value_ids),
            2,
            1
        ));
        ...
        if ($items = K::M('afzncase/case')->items($filter, $orderby, $page, $limit, $count)) {
            $lastphotos = array();
            foreach ($items as $k => $val) {
                if ($val['lastphotos']) {
                    $lastphotos[] = $val['lastphotos'];
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('afzncase:album', array(
                implode('-', $attr_value_ids),
                $order,
                '{page}'
            ), $params));
    ...
    public function detail($case_id, $page = 1)
    {
        ...
        K::M('afzncase/case')->update_count($case_id, 'views', 1);
        if ($company_id = $case['company_id']) {
            $this->pagedata['company'] = K::M('afzncompany/company')->detail($case['company_id']);
        } else 
        ...
        if ($attr_values = K::M('afzncase/attr')->attrs_by_case($case_id)) {
            foreach ($attr_values as $k => $v) {
                $case['attrvalues'][$k] = $v['attr_value_id'];
            }
        }
        $this->pagedata['photos'] = K::M('afzncase/photo')->items_by_case($case_id, 1, 50);
        $filter = $pager = array();
        $pager['page'] = max(intval($page), 1);
        $pager['limit'] = $limit = 5;
        $filter['case_id'] = $case_id;
        if ($items = K::M('afzncase/comment')->items($filter, array(
            'dateline' => 'desc'
        ), $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('afzncase:detail', array(
                $case_id,
                '{page}'
            )));
            $uids = array();
        ...
        $this->tmpl = 'afzncase/detail.html';
    }
    ...
    public function comment($case_id)
    {
        if (! $this->check_login()) {
            ...
            if ($verifycode_success) {
                ...
                K::M('afzncase/comment')->create($data);
                $this->err->add('评论成功！');
            }
        }
    }
    ...
    protected function check_case($case_id)
    {
        if (! $case_id = (int) $case_id) {
            ...
            if (! $case = K::M('afzncase/case')->detail($case_id)) {
                $this->error(404);
            } elseif (! $case['audit']) {
        ...
        if (! $case_id = (int) $case_id) {
            ...
            if (! $case = K::M('afzncase/case')->detail($case_id)) {
                ...
                if (! $case['audit']) {
                    ...
                    if (K::M('afzncase/like')->is_like($case_id, __IP)) {
                        ...
                        K::M('afzncase/like')->create($data);
                        K::M('afzncase/case')->update_count($case_id, 'likes', 1);
                        $this->err->add('喜欢成功');
                    }
    }
}
