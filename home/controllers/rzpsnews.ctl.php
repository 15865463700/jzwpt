...
class Ctl_Rzpsnews extends Ctl
{
    ...
    public function items($page = 1)
    {
        ...
        $filter['audit'] = 1;
        if ($items = K::M('rzpscompany/news')->items($filter, $orderby, $page, $limit, $count)) {
            $pager['count'] = $count;
    ...
    public function detail($news_id)
    {
        ...
        if (! $news_id = (int) $news_id) {
            ...
            if (! $detail = K::M('rzpscompany/news')->detail($news_id)) {
                ...
                K::M('rzpscompany/news')->update_count($news_id, 'views', 1);
                $rzpscompany = $this->check_company($detail['company_id']);
                ...
                $this->tmpl = 'rzpsnews/detail.html';
            }
    }
