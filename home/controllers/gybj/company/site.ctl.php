...
// class Ctl_Gybj_Company_Site extends Ctl_Gybj_Jiatingyingjuyuan
class Ctl_Gybj_Company_Site extends Ctl
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'gybj/company/site/items.html';
    }
    ...
    public function create()
    {
        ...
        if (K::M('member/integral')->check('site', $this->MEMBER) === false) {
            ...
            if ($data = $this->checksubmit('data')) {
                    ...
                    if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                        ...
                        if ($site_id = K::M('home/site')->create($data)) {
                            if ($company_id = (int) $data['company_id']) {
                                K::M('gybjcompany/company')->update_count($company_id, 'site_num', 1);
                            }
                            if ($home_id = (int) $data['home_id']) {
                            ...
                            $this->err->add('添加工地成功');
                            $this->err->set_data('forward', $this->mklink('gybj/company/site:index'));
                        }
                    }
            } else {
                ...
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'gybj/company/site/create.html';
            }
    }
        ...
        if (! ($site_id = (int) $site_id) && ! ($site_id = (int) $this->GP('site_id'))) {
            ...
            if (! $detail = K::M('home/site')->detail($site_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $pager['city_id'] = $company['city_id'];
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'gybj/company/site/edit.html';
                }
    }
    ...
    private function getZxpm()
    {
        ...
        return K::M('gybjcompany/zxpm')->items($filter);
    }
        ...
        if (! ($site_id = (int) $site_id) && ! ($site_id = $this->GP('site_id'))) {
            ...
            if (! $detail = K::M('home/site')->detail($site_id)) {
                ...
                if ($company['company_id'] != $detail['company_id']) {
                    ...
                    K::M('gybjcompany/company')->update_count($company['company_id'], 'site_num', - 1);
    ...
    public function detail($site_id = null)
    {
        ...
        if (! $site_id = (int) $site_id) {
            ...
            if (! $site = K::M('home/site')->detail($site_id)) {
                ...
                $this->tmpl = 'gybj/company/site/diary/site.html';
            }
    }
}