...
class Ctl_Gybj_Company_Youhui extends Ctl_Gybj_Jiatingyingjuyuan
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'gybj/company/youhui/items.html';
    }
            ...
            if ($data = $this->checksubmit('data')) {
                ...
                if ($allow_youhui < 0) {
                    ...
                    if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                        ...
                        if ($coupon_id = K::M('gybjcompany/youhui')->create($data)) {
                            K::M('gybjcompany/youhui')->youhui_count($company['company_id']);
                            K::M('member/integral')->commit('youhui', $this->MEMBER, '添加优惠信息');
                            $this->err->add('添加优惠信息成功');
                            $this->err->set_data('forward', $this->mklink('gybj/company/youhui:index'));
                        }
                    }
            } else {
                $this->tmpl = 'gybj/company/youhui/create.html';
            }
    }
    ...
    public function refresh($youhui_id = null)
    {
        ...
        if ($fromid = $this->GP('fromid')) {
            ...
            if (K::M('member/flush')->create($data)) {
                K::M('gybjcompany/youhui')->update($fromid, array(
                    'flushtime' => __TIME
                ));
                $this->err->add('刷新成功');
            }
            ...
            if (! $detail = K::M('gybjcompany/youhui')->detail($youhui_id)) {
                ...
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'gybj/company/youhui/edit.html';
                    }
    }
        ...
        if (! ($youhui_id = (int) $youhui_id) && ! ($youhui_id = $this->GP('youhui_id'))) {
            ...
            if (! $detail = K::M('gybjcompany/youhui')->detail($youhui_id)) {
                ...
                if ($company['company_id'] != $detail['company_id']) {
                    ...
                    K::M('gybjcompany/youhui')->delete($youhui_id);
    ...
    public function sign($page = 1)
    {
        ...
        $pager['limit'] = $limit = 20;
        $pager['count'] = $count = 0;
        if ($items = K::M('gybjcompany/sign')->items(array(
            'company_id' => $company['company_id']
        ), $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array(
        ...
        $this->tmpl = 'gybj/company/youhui/sign.html';
    }
        ...
        if (! $youhui_id = (int) $youhui_id) {
            ...
            if (! $youhui = K::M('gybjcompany/youhui')->detail($youhui_id)) {
                ...
                if ($youhui['company_id'] != $company['company_id']) {
                    ...
                    $this->pagedata['youhui'] = $youhui;
                    $this->tmpl = 'gybj/company/youhui/youhuiSign.html';
                }
    }
    ...
    public function signDetail($sign_id = null)
    {
        ...
        if (! $sign_id = (int) $sign_id) {
            ...
            if (! $detail = K::M('gybjcompany/sign')->detail($sign_id)) {
                $this->err->add('您要查看的内容不存在或已经删除', 212);
            } else 
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'gybj/company/youhui/signDetail.html';
                }
    }
