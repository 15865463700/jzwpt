...
// class Ctl_Gybj_Company_Comment extends Ctl_Gybj_Jiatingyingjuyuan
class Ctl_Gybj_Company_Comment extends Ctl
{
    ...
    public function company($page = 1)
    {
        ...
        $this->tmpl = 'gybj/company/comment/items.html';
    }
    ...
    public function detail($comment_id = null)
    {
        ...
        if (!($comment_id = (int)$comment_id) && !($comment_id = (int)$this->GP('comment_id'))) {
            ...
            if (!$detail = K::M('gybjcompany/comment')->detail($comment_id)) {
                ...
                if ($company['company_id'] != $detail['company_id']) {
                    ...
                    $this->system->config->get('company_comment');
                    $this->tmpl = 'gybj/company/comment/detail.html';
                }
    }
    ...
    public function reply($comment_id = null)
    {
        ...
        if (!($comment_id = (int)$comment_id) && !($comment_id = (int)$this->GP('comment_id'))) {
            ...
            if (!$comment = K::M('gybjcompany/comment')->detail($comment_id)) {
                $this->err->add('评论不存在或已经删除', 212);
            } else
                        ...
                        if ($this->checksubmit()) {
                            if (!$reply_content = $this->GP('reply_content')) {
                                ...
                                if ($audit_reply < 0) {
                                    ...
                                    if (K::M('gybjcompany/comment')->reply($comment_id, $reply_content)) {
                                        $this->err->add('回复评论成功');
                                    }
                        } else {
                            if ($uid = $comment['uid']) {
                            ...
                            $this->tmpl = 'gybj/company/comment/reply.html';
                        }
    }
