 ...
 * $Id$
 */
class Ctl_Gybj_Shop_News extends Ctl_Gybj_Jiatingyingjuyuan
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'gybj/shop/news/items.html';
    }
            ...
            if ($allow_news < 0) {
                ...
                if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                    ...
                    if ($news_id = K::M('shop/news')->create($data)) {
                        $this->err->add('添加内容成功');
                        $this->err->set_data('forward', $this->mklink('gybj/shop/news:index'));
                    }
                }
        } else {
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'gybj/shop/news/create.html';
        }
    }
            ...
            if (! $detail = K::M('shop/news')->detail($news_id)) {
                ...
                if ($shop['shop_id'] != $detail['shop_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'gybj/shop/news/edit.html';
                    }
    }
