...
class Ctl_Gzsjs extends Ctl{
    public static $ctl_old="designer";
    public static $ctl_new="gzsjs";
    public static $tpl_gzsjs="gzsjs";
    ...
    public function items($page = 1){
        if(strpos($this->request['uri'],"esigner")){
            $uri=str_replace(Ctl_Gzsjs::$ctl_old, Ctl_Gzsjs::$ctl_new, $this->request['uri']);
            header("Location:{$this->request['city']['siteurl']}/{$uri}");die;
        }
        $this->request['uri']=  str_replace(Ctl_Gzsjs::$ctl_new, Ctl_Gzsjs::$ctl_old, $this->request['uri']);
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $v["link"]=str_replace(Ctl_Gzsjs::$ctl_old,Ctl_Gzsjs::$ctl_new,$v["link"]);
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vv["link"]=str_replace(Ctl_Gzsjs::$ctl_old,Ctl_Gzsjs::$ctl_new,$vv["link"]);
                $v['values'][$kk] = $vv;
            }
            $attr_values[$k] = $v;
        }
        ...
        $area_all_link=str_replace(Ctl_Gzsjs::$ctl_old,Ctl_Gzsjs::$ctl_new,$area_all_link);
        ...
        if ($items = K::M('designer/designer')->items_by_attr($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['pagebar']=str_replace(Ctl_Gzsjs::$ctl_old,Ctl_Gzsjs::$ctl_new,$pager['pagebar']);
        ...
        foreach($order_list as $k=>$v){
            ...
            $order_list[$k]["link"]=str_replace(Ctl_Gzsjs::$ctl_old,Ctl_Gzsjs::$ctl_new,$order_list[$k]["link"]);
        }
		...
		if(is_main()){
			empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('gzhuangsjs_items', $seo):$this->seo->init('gzhuangsjs_items_attr', $seo);
		}else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_gzhuangsjs_items', $seo):$this->seo->init('fenzhan_gzhuangsjs_items_attr', $seo);
		}
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/designer/";
        $this->tmpl = Ctl_Gzsjs::$tpl_gzsjs.'/items.html';
    }
            ...
            if (! $detail = K::M('designer/designer')->detail($uid)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($this->checksubmit('data') || $flag) {
                        ...
                        $this->tmpl = Ctl_Gzsjs::$tpl_gzsjs.'/yuyue.html';
                    }
                }
    }
