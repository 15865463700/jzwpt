 ...
 * $Id$
 */
class Ctl_Dcenter_Gz_Money extends Ctl_Dcenter
{

    public function gz($type = 'all', $page = 1)
    {
        $gz = $this->ucenter_gz();
        $filter = $pager = array();
        $filter['uid'] = $gz['uid'];
        ...
        $this->tmpl = 'dcenter/gz/money/items.html';
    }

    public function tixian()
    {
        $gz = $this->ucenter_gz();
        $member = K::M('member/member')->detail($gz['uid']);
        if ($data = $this->checksubmit('data')) {
            ...
            $this->tmpl = 'dcenter/gz/money/tixian.html';
        }
    }
}