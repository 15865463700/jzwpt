...
class Ctl_Dcenter_Mechanic extends Ctl_Dcenter
{
            ...
            if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                ...
                if (! $mechanic['mechanic_id']) {
                    ...
                    if ($mechanic_id = K::M('mechanic/mechanic')->update($mechanic['uid'], $data)) {
                        K::M('member/member')->update($mechanic['uid'], array(
    ...
    public function refresh()
    {
        ...
        if ($this->GP('fromid')) {
            ...
            $data['uid'] = $this->uid;
            $data['from'] = 'mechanic';
            $data['itemId'] = $mechanic['uid'];
            if ($isrefresh && K::M('member/flush')->create($data)) {
                K::M('mechanic/mechanic')->update($mechanic['uid'], array(
                    'flushtime' => __TIME
                ));
                $this->err->add('刷新成功');
            }
            ...
            $this->tmpl = 'dcenter/mechanic/refresh/look.html';
        }
    }
    ...
    public function attr()
    {
        ...
        if ($attr = $this->checksubmit('attr')) {
            ...
            $this->tmpl = 'dcenter/mechanic/attr.html';
        }
    }
}