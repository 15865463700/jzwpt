 ...
 * $Id$
 */
class Ctl_Dcenter_Designer_Case extends Ctl_Dcenter
{

    protected $_allower_fields = 'home_id,home_name,huxing_id,title,intro,seo_title,seo_keywords,seo_description';
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'dcenter/designer/case/items.html';
    }
        ...
        if (K::M('system/integral')->check('case', $this->MEMBER) === false) {
            ...
            if ($data = $this->checksubmit('data')) {
                ...
                if ($allow_case < 0) {
                    ...
                    if (! $detail = K::M('home/home')->detail($data['home_id'])) {
                        ...
                        if ($case_id = K::M('case/case')->create($data)) {
                            ...
                            if ($home_id = (int) $data['home_id']) {
                                ...
                                K::M('home/home')->update($home_id, array(
                                    'last_case_designer_id' => $designer['designer_id']
                                ));
                            }
                            K::M('member/integral')->commit('case', $this->MEMBER, '发布案例');
                            $this->err->set_data('forward', $this->mklink('dcenter/designer/case:detail', array(
                                $case_id
                            )));
                            $this->err->add('添加案例成功');
                        }
                ...
                $this->pagedata['pager'] = $pager;
                $this->tmpl = 'dcenter/designer/case/create.html';
            }
    }
    ...
    public function edit($case_id = null)
    {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('case/case')->detail($case_id)) {
                ...
                if ($data = $this->checksubmit('data')) {
                    ...
                    $pager['city_id'] = $designer['city_id'];
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'dcenter/designer/case/edit.html';
                }
    }
    ...
    public function detail($case_id = null, $page = 1)
    {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $detail = K::M('case/case')->detail($case_id)) {
                ...
                if ($detail['uid'] != $designer['designer_id']) {
                    ...
                    if ($items = K::M('case/photo')->items_by_case($case_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("dcenter/designer/case:detail", array(
                            $case_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'dcenter/designer/case/detail.html';
                }
    }
                        ...
                        if (UPLOAD_ERR_OK != $attach['error']) {
                            ...
                            if ($data = K::M('case/photo')->upload($case_id, $attach)) {
                                if ($allow_case != $case['audit'] && empty($case['photos'])) {
                                    K::M('case/case')->update($case_id, array(
                                        'audit' => $allow_case
                                    ));
                                }
    ...
    public function delete($case_id = null)
    {
        ...
        if (! ($case_id = (int) $case_id) && ! ($case_id = (int) $this->GP('case_id'))) {
            ...
            if (! $case = K::M('case/case')->detail($case_id)) {
                ...
                if ($case['uid'] != $designer['designer_id']) {
                    $this->err->add('不许越权管理别人的内容', 212);
                } else 
    ...
    public function defaultphoto($photo_id = null)
    {
        ...
        if (! ($photo_id = (int) $photo_id) && ! ($photo_id = (int) $this->GP('photo_id'))) {
            ...
            if (! $detail = K::M('case/photo')->detail($photo_id)) {
                ...
                if (! $case = K::M('case/case')->detail($detail['case_id'])) {
                    $this->err->add('案例不存在或已经删除', 212);
                } else 
                    if ($case['uid'] != $designer['designer_id']) {
                        $this->err->add('不许越权管理别人的内容', 213);
                    } else {
                        if (K::M('case/case')->update($detail['case_id'], array(
                            'photo' => $detail['photo']
                        ))) {
                            $this->err->add('修改封面成功');
                        }
