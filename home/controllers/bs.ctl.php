<?php
class Ctl_Bs extends Ctl{
    public static $ctl_bs_old="gs";
    public static $ctl_bs_new="bs";
    public static $tpl_bs="bs";
    ...
    public function items($page = 1){
        ...
        if (preg_match('/items(-[\d\-]+)?(-(\d+)).html/i', $uri, $m)||preg_match('/'.Ctl_Bs::$ctl_bs_new.'(-[\d\-]+)?(-(\d+))(\.html)*/i', $uri, $m)) {
            $page = (int) $m[3];
            if ($m[1]) {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
                $area_id,
                $group_id,
                implode('-', $vids)
            ));
            ...
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
                    $area_id,
                    $group_id,
                    implode('-', $vids)
                ));
        ...
        if ($group_list = K::M('member/group')->items_by_from('company')) {
            $group_all_link = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
                $area_id,
                0,
        ...
        $area_all_link = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
            0,
            $group_id,
            implode('-', $attr_value_ids),
            $order,
        ...
        $order_list[0]['link'] = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            0,
        ...
        $order_list[1]['link'] = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            1,
        ...
        $order_list[2]['link'] = $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            2,
        ...
        if ($items = K::M('company/company')->items($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(Ctl_Bs::$ctl_bs_new.':items', array(
                $area_id,
                $group_id,
                implode('-', $attr_value_ids),
                $order,
            ...
            $pager['pagebar']=str_replace(Ctl_Bs::$ctl_bs_old,Ctl_Bs::$ctl_bs_new,$pager['pagebar']);
            foreach($items as $k=>$v){
                $items[$k]['company_url']=preg_replace("/\/company-/","/company/",$v['company_url']);
                $items[$k]['company_url']=  str_replace(Ctl_Bs::$ctl_bs_old, Ctl_Bs::$ctl_bs_new, $items[$k]['company_url']);
            }
        ...
        foreach($order_list as $k=>$v){
            $order_list[$k]["link"] = preg_replace("/gs-items/",Ctl_Bs::$ctl_bs_new,$v["link"]);
            $order_list[$k]["link"] = preg_replace("/gs-0-0-0-0-0-1\./",Ctl_Bs::$ctl_bs_new.".",$order_list[$k]["link"]);
            $order_list[$k]["link"] = preg_replace("/\.html/","/",$order_list[$k]["link"]);
            $order_list[$k]["link"] = preg_replace(Ctl_Bs::$ctl_bs_old,Ctl_Bs::$ctl_bs_new,$order_list[$k]["link"]);
        }
        ...
        if(is_main()){
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('bshu_items', $seo):$this->seo->init('bshu_items_attr', $seo);
        }else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_bshu_items', $seo):$this->seo->init('fenzhan_bshu_items_attr', $seo);
        }
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/".Ctl_Bs::$ctl_bs_new."/";
        $this->tmpl = Ctl_Bs::$tpl_bs.'/items.html';
    }
    ...
    public function map()
    {
        ...
        $this->tmpl = Ctl_Bs::$tpl_bs.'/maps.html';
    }
            ...
            if (! $detail = K::M('company/company')->detail($company_id)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($this->checksubmit('data')) {
                        ...
                        $this->tmpl = Ctl_Bs::$tpl_bs.'/yuyue.html';
                    }
                }
    }
}