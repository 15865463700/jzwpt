...
class Ctl_Mechanic extends Ctl
{
    ...
    public function items($page = 1)
    {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('mechanic:items', array(
                $area_id,
                $group_id,
                implode('-', $vids)
            ));
            ...
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('mechanic:items', array(
                    $area_id,
                    $group_id,
                    implode('-', $vids)
                ));
        ...
        if ($group_list = K::M('member/group')->items_by_from('mechanic')) {
            $group_all_link = $this->mklink('mechanic:items', array(
                $area_id,
                0,
        ...
        $area_all_link = $this->mklink('mechanic:items', array(
            0,
            $group_id,
            implode('-', $attr_value_ids),
            $order,
        ...
        $order_list[0]['link'] = $this->mklink('mechanic:items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            0,
        ...
        $order_list[1]['link'] = $this->mklink('mechanic:items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            1,
        ...
        $order_list[2]['link'] = $this->mklink('mechanic:items', array(
            $area_id,
            $group_id,
            implode('-', $attr_value_ids),
            2,
        ...
        if ($filter['attrs']) {
            if ($items = K::M('mechanic/mechanic')->items_by_attr($filter, $orderby, $page, $limit, $count)) {
                $pager['count'] = $count;
                $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('mechanic:items', array(
                    $area_id,
                    $group_id,
                    implode('-', $attr_value_ids),
                    $order,
    ...
    public function detail($uid)
    {
        if (! ($uid = (int) $uid) && ! ($uid = $this->GP('uid'))) {
            ...
            if (! $detail = K::M('mechanic/mechanic')->detail($uid)) {
                ...
                K::M('mechanic/mechanic')->update($uid, array(
