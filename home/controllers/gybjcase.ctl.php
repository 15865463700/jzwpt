 ...
 * $Id$
 */
class Ctl_Gybjcase extends Ctl
{
    ...
    public function items($page = 1)
    {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('gybjcase:items', array(
                implode('-', $vids)
            ));
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('gybjcase:items', array(
                    implode('-', $vids)
                ));
                $v['values'][$kk] = $vv;
            }
        ...
        $order_list[0]['link'] = $this->mklink('gybjcase:items', array(
            implode('-', $attr_value_ids),
            0,
            1
        ));
        $order_list[1]['link'] = $this->mklink('gybjcase:items', array(
            implode('-', $attr_value_ids),
            1,
            1
        ));
        $order_list[2]['link'] = $this->mklink('gybjcase:items', array(
            implode('-', $attr_value_ids),
            2,
            1
        ));
        ...
        // $orderby = NULL;
        // }
        $items = K::M('gybjcase/case')->items($filter, $orderby, $page, $limit, $count);
        if ($items) {
            $lastphotos = array();
            ...
            $this->pagedata['photos'] = K::M('gybjcase/photo')->items_by_ids($lastphotos);
            // }
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('gybjcase:items', array(
                implode('-', $attr_value_ids),
                $order,
                '{page}'
            ), $params));
        ...
        $this->seo->init('gybjcase', $seo);
        $this->tmpl = 'gybjcase/items.html';
    }
    ...
    public function album($page = 1)
    {
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $vids['order'] = $order;
            $vids['page'] = 1;
            $v['link'] = $this->mklink('gybjcase:album', array(
                implode('-', $vids)
            ));
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vids[$k] = $kk;
                $vv['link'] = $this->mklink('gybjcase:album', array(
                    implode('-', $vids)
                ));
                $v['values'][$kk] = $vv;
            }
        ...
        $order_list[0]['link'] = $this->mklink('gybjcase:album', array(
            implode('-', $attr_value_ids),
            0,
            1
        ));
        $order_list[1]['link'] = $this->mklink('gybjcase:album', array(
            implode('-', $attr_value_ids),
            1,
            1
        ));
        $order_list[2]['link'] = $this->mklink('gybjcase:album', array(
            implode('-', $attr_value_ids),
            2,
            1
        ));
        ...
        if ($items = K::M('gybjcase/case')->items($filter, $orderby, $page, $limit, $count)) {
            $lastphotos = array();
            foreach ($items as $k => $val) {
                if ($val['lastphotos']) {
                    $lastphotos[] = $val['lastphotos'];
            ...
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('gybjcase:album', array(
                implode('-', $attr_value_ids),
                $order,
                '{page}'
            ), $params));
    ...
    public function detail($case_id, $page = 1)
    {
        ...
        K::M('gybjcase/case')->update_count($case_id, 'views', 1);
        if ($company_id = $case['company_id']) {
            $this->pagedata['company'] = K::M('gybjcompany/company')->detail($case['company_id']);
        } else 
        ...
        if ($attr_values = K::M('gybjcase/attr')->attrs_by_case($case_id)) {
            foreach ($attr_values as $k => $v) {
                $case['attrvalues'][$k] = $v['attr_value_id'];
            }
        }
        $this->pagedata['photos'] = K::M('gybjcase/photo')->items_by_case($case_id, 1, 50);
        $filter = $pager = array();
        $pager['page'] = max(intval($page), 1);
        $pager['limit'] = $limit = 5;
        $filter['gybjcase_id'] = $case_id;
        if ($items = K::M('gybjcase/comment')->items($filter, array(
            'dateline' => 'desc'
        ), $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('gybjcase:detail', array(
                $case_id,
                '{page}'
            )));
            $uids = array();
        ...
        $this->seo->init('gybjcase_detail', array(
            'title' => $case['title'],
        ...
        $this->tmpl = 'gybjcase/detail.html';
    }
    ...
    public function comment($case_id)
    {
        if (! $this->check_login()) {
            ...
            if ($verifycode_success) {
                ...
                K::M('gybjcase/comment')->create($data);
                $this->err->add('评论成功！');
            }
        }
    }
        ...
        if (! $case_id = (int) $case_id) {
            ...
            if (! $case = K::M('gybjcase/case')->detail($case_id)) {
                ...
                if (! $case['audit']) {
                    ...
                    if (K::M('gybjcase/like')->is_like($case_id, __IP)) {
                        ...
                        K::M('gybjcase/like')->create($data);
                        K::M('gybjcase/case')->update_count($case_id, 'likes', 1);
                        $this->err->add('喜欢成功');
                    }
    }
}
