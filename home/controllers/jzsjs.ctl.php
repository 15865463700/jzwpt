...
class Ctl_Jzsjs extends Ctl{
    public static $ctl_old="designer";
    public static $ctl_new="jzsjs";
    public static $tpl_jzsjs="jzsjs";
    ...
    public function items($page = 1){
        if(strpos($this->request['uri'],"esigner")){
            $uri=str_replace(Ctl_Jzsjs::$ctl_old, Ctl_Jzsjs::$ctl_new, $this->request['uri']);
            header("Location:{$this->request['city']['siteurl']}/{$uri}");die;
        }
        $this->request['uri']=  str_replace(Ctl_Jzsjs::$ctl_new, Ctl_Jzsjs::$ctl_old, $this->request['uri']);
        ...
        foreach ($attr_values as $k => $v) {
            ...
            $v["link"]=str_replace(Ctl_Jzsjs::$ctl_old,Ctl_Jzsjs::$ctl_new,$v["link"]);
            $v['checked'] = true;
            foreach ($v['values'] as $kk => $vv) {
                ...
                $vv["link"]=str_replace(Ctl_Jzsjs::$ctl_old,Ctl_Jzsjs::$ctl_new,$vv["link"]);
                $v['values'][$kk] = $vv;
            }
            $attr_values[$k] = $v;
        }
        ...
        $area_all_link=str_replace(Ctl_Jzsjs::$ctl_old,Ctl_Jzsjs::$ctl_new,$area_all_link);
        ...
        if ($items = K::M('designer/designer')->items_by_attr($filter, $orderby, $page, $limit, $count)) {
            ...
            $pager['pagebar']=str_replace(Ctl_Jzsjs::$ctl_old,Ctl_Jzsjs::$ctl_new,$pager['pagebar']);
        ...
        foreach($order_list as $k=>$v){
            ...
            $order_list[$k]["link"]=str_replace(Ctl_Jzsjs::$ctl_old,Ctl_Jzsjs::$ctl_new,$order_list[$k]["link"]);
        }
		...
		if(is_main()){
			empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('jzhuangsjs_items', $seo):$this->seo->init('jzhuangsjs_items_attr', $seo);
		}else{
            empty($seo['attr'])&&empty($seo['area_name'])?$this->seo->init('fenzhan_jzhuangsjs_items', $seo):$this->seo->init('fenzhan_jzhuangsjs_items_attr', $seo);
		}
        $this->pagedata['canonical'] = "{$_SERVER['HTTP_HOST']}/jzsjs/";
        $this->tmpl = Ctl_Jzsjs::$tpl_jzsjs.'/items.html';
    }
            ...
            if (! $detail = K::M('designer/designer')->detail($uid)) {
                ...
                if (empty($detail['audit'])) {
                    ...
                    if ($this->checksubmit('data') || $flag) {
                        ...
                        $this->tmpl = Ctl_Jzsjs::$tpl_jzsjs.'/yuyue.html';
                    }
                }
    }
