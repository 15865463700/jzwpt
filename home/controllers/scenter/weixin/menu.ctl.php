...
class Ctl_Scenter_Weixin_Menu extends Ctl_Scenter
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'scenter/weixin/menu/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, 'title,parent_id,type,reply_id,content,link,orderby')) {
                ...
                if ($menu_id = K::M('weixin/menu')->create($data)) {
                    $this->err->set_data('forward', $this->mklink('scenter/weixin/menu:index'));
                    $this->err->add('添加微信菜单成功');
                }
            }
        } else {
            $this->pagedata['wx_menu_list'] = K::M('weixin/menu')->items_by_weixin($weixin['wx_id']);
            $this->tmpl = 'scenter/weixin/menu/create.html';
        }
    }
            ...
            if (! $detail = K::M('weixin/menu')->detail($menu_id)) {
                ...
                if ($detail['wx_id'] != $weixin['wx_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->pagedata['wx_menu_list'] = K::M('weixin/menu')->items_by_weixin($weixin['wx_id']);
                        $this->tmpl = 'scenter/weixin/menu/edit.html';
                    }
    }
