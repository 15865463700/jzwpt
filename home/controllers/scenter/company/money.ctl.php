 ...
 * $Id$
 */
class Ctl_Scenter_Company_Money extends Ctl_Scenter
{
    ...
    public function company($type = 'all', $page = 1)
    {
        ...
        $this->tmpl = 'scenter/company/money/items.html';
    }
    ...
    public function tixian()
    {
        ...
        if ($data = $this->checksubmit('data')) {
            if (! is_numeric($data['money'])) {
                ...
                if (! $money = round($data['money'], 2)) {
                    ...
                    if ($money > $member['truste_money']) {
                        $this->err->add(sprintf('提现金额不能大于你的账户余额%s>%s,%s',$money,$member['truste_money'],json_encode($member)), 213);
                    } else 
            ...
            $this->tmpl = 'scenter/company/money/tixian.html';
        }
    }
}