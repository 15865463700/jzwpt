...
class Ctl_Scenter_Company_Photo extends Ctl_Scenter
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'scenter/company/photo/items.html';
    }
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                ...
                if ($pic_id = K::M('company/photo')->create($data)) {
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('scenter/company/photo:index'));
                }
            }
        } else {
            $this->pagedata['type_list'] = K::M('company/photo')->get_type_means();
            $this->tmpl = 'scenter/company/photo/create.html';
        }
    }
