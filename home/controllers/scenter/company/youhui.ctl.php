...
class Ctl_Scenter_Company_Youhui extends Ctl_Scenter
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'scenter/company/youhui/items.html';
    }
                ...
                if ($allow_youhui < 0) {
                    ...
                    if (! $data = $this->check_fields($data, $this->_allow_fields)) {
                        ...
                        if ($coupon_id = K::M('company/youhui')->create($data)) {
                            ...
                            $this->err->set_data('forward', $this->mklink('scenter/company/youhui:index'));
                        }
                    }
            } else {
                $this->tmpl = 'scenter/company/youhui/create.html';
            }
    }
    ...
    public function refresh($youhui_id = null)
    {
        ...
        if ($fromid = $this->GP('fromid')) {
            ...
            if (K::M('member/flush')->create($data)) {
                K::M('company/youhui')->update($fromid, array(
                    'flushtime' => __TIME
                ));
                $this->err->add('刷新成功');
            }
            ...
            if (! $detail = K::M('company/youhui')->detail($youhui_id)) {
                ...
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    if ($data = $this->checksubmit('data')) {
                        ...
                        $this->tmpl = 'scenter/company/youhui/edit.html';
                    }
    }
    ...
    public function sign($page = 1)
    {
        ...
        $pager['limit'] = $limit = 20;
        $pager['count'] = $count = 0;
        if ($items = K::M('company/sign')->items(array(
            'company_id' => $company['company_id']
        ), $page, $limit, $count)) {
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array(
        ...
        $this->tmpl = 'scenter/company/youhui/sign.html';
    }
    ...
    public function youhuiSign($youhui_id = null, $page = 1)
    {
        ...
        if (! $youhui_id = (int) $youhui_id) {
            ...
            if (! $youhui = K::M('company/youhui')->detail($youhui_id)) {
                ...
                if ($youhui['company_id'] != $company['company_id']) {
                    ...
                    $this->pagedata['youhui'] = $youhui;
                    $this->tmpl = 'scenter/company/youhui/youhuiSign.html';
                }
    }
    ...
    public function signDetail($sign_id = null)
    {
        ...
        if (! $sign_id = (int) $sign_id) {
            ...
            if (! $detail = K::M('company/sign')->detail($sign_id)) {
                $this->err->add('您要查看的内容不存在或已经删除', 212);
            } else 
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    $this->pagedata['detail'] = $detail;
                    $this->tmpl = 'scenter/company/youhui/signDetail.html';
                }
    }
    ...
    public function signSave()
    {
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $data = $this->check_fields($data, 'status,remark')) {
                ...
                if (! $sign_id = (int) $this->GP('sign_id')) {
    ...
    public function detail($youhui_id = null, $page = 1)
    {
        $company = $this->ucenter_company();
        $pager = array();
        if (! ($youhui_id = (int) $youhui_id) && ! ($youhui_id = (int) $this->GP('youhui_id'))) {
            ...
            if (! $detail = K::M('company/youhui')->detail($youhui_id)) {
                ...
                if ($detail['company_id'] != $company['company_id']) {
                    ...
                    $pager['youhui_id'] = $youhui_id;
                    $pager['page'] = (int) $page;
                    $pager['limit'] = $limit = 20;
                    $pager['count'] = $count = 0;
                    $this->pagedata['detail'] = $detail;
                    if ($items = K::M('company/yhphoto')->items_by_youhui($youhui_id, $page, $limit, $count)) {
                        $this->pagedata['items'] = $items;
                        $pager['count'] = $count;
                        $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("scenter/company/case:detail", array(
                            $youhui_id,
                            '{page}'
                        )));
                    }
                    $this->pagedata['pager'] = $pager;
                    $this->tmpl = 'scenter/company/youhui/detail.html';
                }
    }

    public function upload($youhui_id = null)
    {
        $company = $this->ucenter_company();
        $allow_case = K::M('member/group')->check_priv($company['group_id'], 'allow_case');
        if ($allow_case < 0) {
            $this->err->add('您是【' . $company['group_name'] . '】没有权限上传优惠图片', 333);
        } else{
            if (K::M('member/integral')->check('case', $this->MEMBER) === false) {
                ...
                if (! $youhui_id = (int) $this->GP('youhui_id')) {
                    ...
                    if (! $case = K::M('company/youhui')->detail($youhui_id)) {
                        $this->err->add('优惠不存在或已经删除', 202);
                            ...
                            if (UPLOAD_ERR_OK != $attach['error']) {
                                ...
                                if ($data = K::M('company/yhphoto')->upload($youhui_id, $attach)) {
                                    ...
                                    if ($allow_case != $case['audit'] && empty($case['photos'])) {
                                        K::M('company/youhui')->update($youhui_id, array(
                                            'audit' => $allow_case
                                        ));
//                                        K::M('member/integral')->commit('case', $this->MEMBER, '发布案例');
    ...
    public function deletephoto($photo_id = null)
    {
        ...
        if (! ($photo_id = (int) $photo_id) && ! ($photo_id = (int) $this->GP('photo_id'))) {
            ...
            if (! $detail = K::M('company/yhphoto')->detail($photo_id)) {
                $this->err->add('优惠不存在或已经删除', 211);
            } else{
                if (! $case = K::M('company/youhui')->detail($detail['youhui_id'])) {
                    $this->err->add('优惠不存在或已经删除', 212);
                } else{
                    if ($case['company_id'] != $company['company_id']) {
                        $this->err->add('不许越权管理别人的内容', 213);
                    } else {
                        if (K::M('company/yhphoto')->delete($photo_id,true)) {
