...
class Ctl_Scenter_Company_Team extends Ctl_Scenter
{
    ...
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'scenter/company/team/items.html';
    }
    ...
    public function bind()
    {
        ...
        if ($data = $this->checksubmit('data')) {
            if (! $member = K::M('member/member')->member($data['uname'], 'uname')) {
                ...
                if ($member['passwd'] != md5($data['passwd'])) {
                    $this->err->add('用户名密码错误', 201);
                } else 
                    if ($member['from'] != 'designer') {
                        ...
                        if (! $designer = K::M('designer/designer')->detail($member['uid'])) {
                            ...
                            if (! empty($designer['company_id'])) {
                                $this->err->add('该设计师已经被绑定过了', 201);
                            } else {
                                if (K::M('designer/designer')->update($member['uid'], array(
            ...
            $this->tmpl = 'scenter/company/team/bind.html';
        }
    }
    ...
    public function unbind($uid = null)
    {
        ...
        if (! ($uid = (int) $uid) && ! ($uid = (int) $this->GP('uid'))) {
            ...
            if (! $detail = K::M('designer/designer')->detail($uid)) {
                ...
                if (K::M('designer/designer')->update($uid, array(
                    'company_id' => 0
                ), true)) {
                    $this->err->add('操作成功');
                } else {
    ...
    public function create(){
          $this->tmpl = 'scenter/company/team/create.html';
    }
    public function insert(){
        $diary_id = K::M('company/designer')->create($data);
